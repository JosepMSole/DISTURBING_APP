...
const localAvatarKey='disturbing_user_avatar_v1', unlockedAvatarsKey='disturbing_unlocked_avatars_v1', avatarRewardSeenKey='disturbing_avatar_reward_seen_v1', userNameCacheKey='disturbing_user_name_v1', achievementEarnedKey='disturbing_achievements_v1', consumedMediaKey='disturbing_consumed_story_media_v1';
...
function bindReaderMedia(){bindBrokenImages();bindStoryMediaPause();const mediaType=storyMediaTypeFromCurrentTab();$$('.reader-image:not(.linked) img').forEach(img=>img.onclick=()=>openLightbox(img.src,img.alt));$$('#reader audio,#reader video').forEach(m=>m.addEventListener('play',()=>{if(mediaType)markStoryMediaConsumed(mediaType,currentStory?.id)},{once:true}));$$('#reader .reader-media iframe').forEach(m=>m.addEventListener('pointerdown',()=>{if(mediaType)markStoryMediaConsumed(mediaType,currentStory?.id)},{once:true,passive:true}))}
...
function markUnread(id){const a=readList().filter(x=>String(x)!==String(id));if(a.length===readList().length)return false;store.set(readKey,a);const m=readingStateMap();delete m[id];store.set(readingStateKey,m);const p=readProgressMap();delete p[id];store.set(readProgressKey,p);const pill=$('#readStatusPill');if(pill)pill.classList.add('hidden');updateAppBadge();queueUserVaultSync();if(routeName()==='user')renderUser();return true}
...
function consumedMediaMap(){return store.get(consumedMediaKey,{audio:[],video:[]})||{audio:[],video:[]}}
function mediaConsumed(type,storyId){const m=consumedMediaMap(),rows=Array.isArray(m?.[type])?m[type]:[];return rows.map(String).includes(String(storyId))}
function markStoryMediaConsumed(type,storyId){if(!['audio','video'].includes(type)||!storyId)return false;const m=consumedMediaMap();m.audio=Array.isArray(m.audio)?m.audio:[];m.video=Array.isArray(m.video)?m.video:[];if(m[type].map(String).includes(String(storyId)))return false;m[type].push(String(storyId));store.set(consumedMediaKey,m);evaluateAchievements(true);queueUserVaultSync();return true}
...
function earnedAchievementCount(){const got=earnedAchievementIds();return achievementRows().filter(a=>a?.id&&got.has(String(a.id))).length}
...
function evaluateAchievements(showReward=false){const got=earnedAchievementIds(),newOnes=[];for(const a of achievementRows()){const id=String(a?.id||'');if(!id||got.has(id)||!achievementRequirementMet(a))continue;got.add(id);newOnes.push(a)}if(newOnes.length){store.set(achievementEarnedKey,[...got]);queueUserVaultSync();if(showReward)queueAchievementRewards(newOnes);updateUserChrome();if(routeName()==='user')renderUser()}return got}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.7.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
