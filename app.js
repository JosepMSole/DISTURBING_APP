...
const view=$('#view');
let indexData=null,currentStory=null,currentTab='read',installPrompt=null,qrScanner=null,qrScanBusy=false,qrLiveStream=null,qrLiveTimer=0,qrLiveDecodeBusy=false,qrFrameDecoder=null,resumeOnOpen=false,storyInitialOpen=false,lastReadingSaveAt=0,bookAssets={};
...
function routeFromHash(){persistCurrentReadingPosition();stopQrScanner();stopRandomSpin(false);const p=currentHash().replace(/^#\//,'').split('/'),route=p[0]||'home',storyMode=route==='story'?(p[2]||''):'',isResume=storyMode==='resume',storyTab=(!isResume&&storyMode)?decodeURIComponent(storyMode):'';if(!isResume)scrollTo({top:0,behavior:'auto'});setNav(route);updateBackButton();$('#toTopHeaderBtn').classList.remove('hidden');if(route==='home')return renderHome();if(route==='stories')return renderStories('all');if(route==='unlocked')return renderMyStories();if(route==='collection')return renderCollection(decodeURIComponent(p[1]||''));if(route==='player')return renderPlayer();if(route==='story')return renderStory(decodeURIComponent(p[1]||''),isResume,storyTab);if(route==='sagas')return renderSagas();if(route==='saga')return renderSaga(decodeURIComponent(p[1]||''));if(route==='timeline')return renderTimeline();if(route==='random')return renderRandom();if(route==='cassettes')return renderMediaCollection('audio');if(route==='tapes')return renderMediaCollection('video');if(route==='extras')return renderExtras();if(route==='extra')return renderExtra(decodeURIComponent(p[1]||''));if(route==='games')return renderFuturePlaceholder('GAMES','rosa');if(route==='micro-pesadillas')return renderFuturePlaceholder('MICRO-PESADILLAS','rojo-blanco');renderHome()}
function sectionIndicatorMarkup(route){const storyRoutes=new Set(['stories','story','sagas','saga','timeline','random','cassettes','tapes','extras','extra','games','micro-pesadillas']);if(route==='unlock')return `<span class="section-indicator-qr" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M4 4h6v6H4zM14 4h6v6h-6zM4 14h6v6H4zM14 14h2v2h-2zM18 14h2v2h-2zM14 18h2v2h-2zM17 17h3v3h-3z"/></svg></span>`;if(route==='home')return `<span class="section-indicator-home" aria-hidden="true">⌂</span>`;if(route==='unlocked')return `<span class="section-indicator-watcher" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M3.2 12s3.2-5.2 8.8-5.2S20.8 12 20.8 12s-3.2 5.2-8.8 5.2S3.2 12 3.2 12Z"/><circle cx="12" cy="12" r="2.15"/><circle class="watcher-pupil" cx="12.7" cy="11.3" r=".45"/></svg></span>`;if(route==='collection')return `<span class="section-indicator-books" aria-hidden="true"><i></i><i></i><i></i><i></i></span>`;if(route==='player')return `<span class="section-indicator-player" aria-hidden="true">♫</span>`;if(storyRoutes.has(route))return `<span class="section-indicator-grid" aria-hidden="true">${'<i></i>'.repeat(9)}</span>`;return ''}
...
function lockedTeaserHtml(s){const raw=s?.publicTeaser||s?.teaserUrl||s?.teaser||s?.mediaYoutube?.teaser||'';const src=typeof raw==='string'?raw:(raw?.src||raw?.url||'');const embed=youtubeEmbedUrl(src);if(!embed)return'';return `<section class="locked-teaser"><div class="eyebrow">TEASER</div><div class="locked-teaser-video"><iframe src="${escAttr(embed)}" title="Teaser Story ${escAttr(s.id)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen loading="lazy"></iframe></div></section>`}
function renderLocked(s,msg=''){const book=String(Number(s.book)||s.book||'—');return `<section class="locked-story-page"><div class="locked-main-lock" aria-label="Story bloqueada">${lockSvg(false)}</div>${storyFeaturedMedia(s,true)}${msg?`<p class="locked-error-note">${escapeHtml(msg)}</p>`:''}<section class="unlock-panel locked-inline-unlock"><div class="scanner-instruction story-specific"><strong>SÓLO EN EL LIBRO ${escapeHtml(book)}</strong><p>Esta Story requiere el QR que aparece junto a este relato en el libro físico. Escanéalo para desbloquearla.</p></div><div class="scan-zone"><div id="qr-reader" class="qr-reader qr-reader-live"><div class="qr-placeholder">CÁMARA</div></div><div id="qr-file-reader" class="qr-file-reader" aria-hidden="true"></div><button class="cta" id="startQr">ACTIVAR ESCÁNER QR</button><button class="secondary-btn" id="photoQr" style="margin-top:10px">CARGAR IMAGEN DESDE TU ÁLBUM</button><input id="qrFile" class="qr-native-input" type="file" accept="image/*"><p id="unlockStatus" class="note" style="margin-top:12px"></p></div>${lockedTeaserHtml(s)}</section></section>`}
...
async function ensureQrLibrary(){if(typeof Html5Qrcode!=='undefined')return true;unlockStatus('Preparando lector QR…');return new Promise(resolve=>{let settled=false;const finish=ok=>{if(settled)return;settled=true;resolve(!!ok)};const old=document.querySelector('script[data-qr-lib]');if(old){old.addEventListener('load',()=>finish(typeof Html5Qrcode!=='undefined'),{once:true});old.addEventListener('error',()=>finish(false),{once:true});setTimeout(()=>finish(typeof Html5Qrcode!=='undefined'),4500);return}const sc=document.createElement('script');sc.src='https://cdnjs.cloudflare.com/ajax/libs/html5-qrcode/2.3.8/html5-qrcode.min.js';sc.async=true;sc.dataset.qrLib='1';sc.onload=()=>finish(typeof Html5Qrcode!=='undefined');sc.onerror=()=>finish(false);document.head.append(sc);setTimeout(()=>finish(typeof Html5Qrcode!=='undefined'),4500)})}
function qrCameraTimeout(ms=15000){return new Promise((_,reject)=>setTimeout(()=>reject(new Error('camera-timeout')),ms))}
function qrLiveVideoMarkup(){return `<video id="qrLiveVideo" class="qr-live-video" autoplay muted playsinline webkit-playsinline></video><div class="qr-live-overlay" aria-hidden="true"><i></i><i></i><i></i><i></i><span></span></div>`}
async function startQrCamera(prefill=''){
  if(qrScanBusy)return;
  if(!window.isSecureContext){unlockStatus('El escáner en directo necesita HTTPS. Abre la PWA publicada en GitHub Pages.');return}
  if(!navigator.mediaDevices?.getUserMedia){unlockStatus('Este dispositivo no permite abrir la cámara desde la PWA.');return}
  const reader=$('#qr-reader'),btn=$('#startQr');if(!reader)return;
  // MUY IMPORTANTE: getUserMedia se invoca antes del primer await para conservar el gesto directo del usuario en iOS/PWA.
  reader.innerHTML=qrLiveVideoMarkup();
  const streamRequest=navigator.mediaDevices.getUserMedia({audio:false,video:{facingMode:{ideal:'environment'},width:{ideal:1280},height:{ideal:1280}}});
  const libRequest=ensureQrLibrary();
  qrScanBusy=true;unlockStatus('Abriendo cámara…');if(btn){btn.disabled=true;btn.textContent='ABRIENDO CÁMARA…'}
  try{
    await stopQrScanner(false);qrScanBusy=true;
    const stream=await Promise.race([streamRequest,qrCameraTimeout()]);
    if(!qrScanBusy){stream.getTracks().forEach(t=>t.stop());return}
    qrLiveStream=stream;
    const video=$('#qrLiveVideo');if(!video){stream.getTracks().forEach(t=>t.stop());throw new Error('video-missing')}
    video.srcObject=stream;video.muted=true;video.autoplay=true;video.playsInline=true;video.setAttribute('playsinline','');video.setAttribute('webkit-playsinline','');
    await new Promise((resolve,reject)=>{let done=false;const ok=()=>{if(done)return;done=true;resolve()};const fail=()=>{if(done)return;done=true;reject(new Error('video-error'))};if(video.readyState>=2)ok();else{video.addEventListener('loadedmetadata',ok,{once:true});video.addEventListener('canplay',ok,{once:true});video.addEventListener('error',fail,{once:true});setTimeout(ok,1800)}});
    try{await video.play()}catch(e){console.warn('QR video play',e)}
    const libOk=await libRequest;if(!libOk)throw new Error('qr-library');
    qrFrameDecoder=new Html5Qrcode('qr-file-reader');
    unlockStatus('ESCÁNER ACTIVO · coloca el QR delante de la cámara. Se detectará automáticamente.');if(btn){btn.textContent='ESCÁNER ACTIVO';btn.disabled=true}
    scheduleQrLiveFrame(prefill,80);
  }catch(e){console.warn('QR direct camera',e);try{await stopQrScanner()}catch{};reader.innerHTML='<div class="qr-placeholder">CÁMARA</div>';unlockStatus(e?.message==='camera-timeout'?'La cámara no ha respondido. Cierra esta pantalla, vuelve a entrar y pulsa de nuevo ACTIVAR ESCÁNER QR.':'No se pudo abrir la cámara en directo. Revisa el permiso de cámara y pulsa REINTENTAR.');if(btn){btn.textContent='REINTENTAR ESCÁNER QR';btn.disabled=false}qrScanBusy=false}
}
function scheduleQrLiveFrame(prefill='',delay=360){clearTimeout(qrLiveTimer);if(!qrScanBusy)return;qrLiveTimer=setTimeout(()=>scanQrLiveFrame(prefill),delay)}
async function scanQrLiveFrame(prefill=''){
  if(!qrScanBusy||qrLiveDecodeBusy||!qrLiveStream){scheduleQrLiveFrame(prefill,220);return}
  const video=$('#qrLiveVideo');if(!video||video.readyState<2||!video.videoWidth){scheduleQrLiveFrame(prefill,180);return}
  qrLiveDecodeBusy=true;
  try{
    // Priorizamos el cuadrado central que coincide con la guía visual del scanner. Así el QR conserva más resolución en vídeo vertical de iPhone.
    const canvas=document.createElement('canvas'),side=Math.max(1,Math.min(video.videoWidth,video.videoHeight)*.84),sx=Math.max(0,(video.videoWidth-side)/2),sy=Math.max(0,(video.videoHeight-side)/2),out=Math.min(760,Math.max(420,Math.round(side)));canvas.width=out;canvas.height=out;const ctx=canvas.getContext('2d',{willReadFrequently:false});ctx.drawImage(video,sx,sy,side,side,0,0,out,out);
    let decoded='';
    if('BarcodeDetector' in window){try{const detector=new BarcodeDetector({formats:['qr_code']});const found=await detector.detect(canvas);decoded=found?.[0]?.rawValue||''}catch{}}
    if(!decoded&&qrFrameDecoder){const blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/jpeg',.9));if(blob){const file=new File([blob],'qr-live.jpg',{type:'image/jpeg'});try{decoded=await qrFrameDecoder.scanFile(file,false)}catch{}}}
    if(decoded&&qrScanBusy){unlockStatus('QR detectado. Validando acceso…');qrScanBusy=false;await stopQrScanner();await tryUnlock(decoded,prefill);return}
  }catch(e){console.debug('QR frame',e)}finally{qrLiveDecodeBusy=false}
  scheduleQrLiveFrame(prefill,360)
}
...
async function stopQrScanner(resetBusy=true){clearTimeout(qrLiveTimer);qrLiveTimer=0;qrLiveDecodeBusy=false;if(resetBusy)qrScanBusy=false;const stream=qrLiveStream;qrLiveStream=null;if(stream)try{stream.getTracks().forEach(t=>t.stop())}catch{};const d=qrFrameDecoder;qrFrameDecoder=null;if(d)try{await d.clear()}catch{};const s=qrScanner;qrScanner=null;if(s){try{if(s.isScanning)await s.stop()}catch{}try{await s.clear()}catch{}}}
...
function showRandomResult(s){const el=$('#randomResult');if(!el)return;el.innerHTML=`<article class="random-result random-result-v25"><div><div class="eyebrow">RESULTADO</div><h2>STORY ${escapeHtml(s.id)} · ${escapeHtml(s.title||'')}</h2><p>${escapeHtml(timelineYearLabel(parseStoryYear(s)))}</p></div><div class="random-actions"><button class="cta" id="randomRead">LEER STORY</button><button class="secondary-btn" id="randomAgain">VOLVER A GIRAR</button></div></article>`;el.classList.remove('reward-in');void el.offsetWidth;el.classList.add('reward-in');randomResultHaptic();$('#randomRead').onclick=()=>storyGo(s.id);$('#randomAgain').onclick=()=>$('#spinRandom').click()}
...
function renderFuturePlaceholder(title,tone){currentStory=null;view.innerHTML=`<section class="section v2-section future-placeholder ${escAttr(tone)}"><div class="eyebrow">PREPARADO PARA FUTURA ITERACIÓN</div><h1>${escapeHtml(title)}</h1><div class="future-placeholder-box"><strong>PRÓXIMAMENTE</strong><p>La entrada visual ya forma parte de Disturbing Stories App v2.6. Su contenido se desarrollará en una iteración posterior.</p></div></section>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=2.6.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
