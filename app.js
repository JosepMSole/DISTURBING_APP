...
async function boot(){
  ...
  try{
    ...
    [sagasData,timelineData,extrasData,playerCatalogData,homeData,homeHighlightsData,avatarsData,userConfigData,microsData,gamesData]=await Promise.all([
      ...
      loadOptionalJson('./data/games.json',{schema:1,games:[],heroImage:'',heroVersion:'',heroText:'Algunos están diseñados especialmente para ordenador; consulta la compatibilidad antes de jugar.'})
    ]);
    initialiseNewState();
    initUserSystem();
...
function sagaPartRow(part,n,total){const st=part.storyId?indexData.stories.find(s=>s.id===String(part.storyId)):null;if(!st)return `<div class="saga-part future-part"><strong>PARTE ${n}: PRÓXIMAMENTE</strong></div>`;const cover=displayCover(st),read=isRead(st.id),icons=sagaPartFeatureIcons(st);return `<article class="saga-part published-part ${read?'is-read':''}" data-story-id="${escAttr(st.id)}"><span class="saga-part-order">PARTE ${n} DE ${total}</span>${cover?`<div class="saga-part-poster"><img src="${escAttr(cover)}" alt="">${exclusiveBadge(st,'cover')}</div>`:''}<div class="saga-part-copy"><strong>${escapeHtml(st.id)} · ${escapeHtml(st.title||'')}</strong><em>${bookLabel(st.book||'—')}</em>${read?'<b class="saga-read-mark">✓ LEÍDA</b>':''}</div><div class="saga-part-icons">${icons}</div></article>`}
