...
function startPlayerArtworkForCurrent(){clearTimeout(playerArtworkTimer);playerArtworkTimer=0;stopPlayerArtworkMotion();const box=$('#playerArtwork'),track=playerTracks[playerIndex];if(!box||!track)return;const images=playerTrackImages(track),key=String(track.file||'');playerArtworkTrack=key;playerArtworkIndex=0;if(!images.length){box.innerHTML='';box.classList.add('empty');return}box.classList.remove('empty');box.innerHTML=images.map((src,i)=>`<div class="player-artwork-slide ${i===0?'active artwork-initial':''}"><img class="motion-run motion-v${i%4}" src="${escAttr(src)}" alt="" referrerpolicy="no-referrer"></div>`).join('')+'<span class="player-artwork-signal" aria-hidden="true"></span>';$$('.player-artwork-slide img',box).forEach(playerArtworkFit);activatePlayerArtwork(0,true);if(images.length>1){const rotate=()=>{if(routeName()!=='player'||playerArtworkTrack!==String(playerTracks[playerIndex]?.file||'')){playerArtworkTimer=0;return}activatePlayerArtwork(playerArtworkIndex+1,false);playerArtworkTimer=setTimeout(rotate,15000)};playerArtworkTimer=setTimeout(rotate,10000)}}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.29.1',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
