...
const localAvatarKey='disturbing_user_avatar_v1', unlockedAvatarsKey='disturbing_unlocked_avatars_v1', avatarRewardSeenKey='disturbing_avatar_reward_seen_v1', userNameCacheKey='disturbing_user_name_v1';
...
function updateUserChrome(){const name=registeredDisplayName(),lab=$('#userMenuLabel'),hello=$('#userGreeting');if(lab)lab.textContent=registeredUser?(name||'Usuario Registrado'):'Usuario No Registrado';if(hello){const show=!!registeredUser&&!!name;hello.textContent=show?`Hola ${name}`:'';hello.classList.toggle('hidden',!show)}}
...
function renderUser(){currentStory=null;ensureFreeAvatars();const registered=!!registeredUser,avatar=avatarById(selectedAvatarId())||avatarRows()[0],name=registeredDisplayName();view.innerHTML=`<section class="section user-page"><div class="user-hero"><div class="user-avatar-current">${avatar?avatarImg(avatar,'user-avatar-img'):'<span>?</span>'}</div><div><div class="eyebrow">${registered?'USUARIO REGISTRADO':'Usuario No Registrado'}</div><h1>${escapeHtml(registered?(name||'Usuario Registrado'):'Usuario No Registrado')}</h1>${registered?`<p class="user-email">${escapeHtml(registered.email||'')}</p>`:'<p>Tu progreso permanece en este dispositivo. Regístrate para guardarlo y sincronizarlo con tu cuenta.</p>'}</div></div><div class="user-avatar-section"><h2>AVATAR</h2><div class="avatar-grid">${avatarRows().map(avatarChoiceHtml).join('')||'<div class="empty">Los avatares se cargan desde GitHub y se configuran desde el Importer.</div>'}</div></div>${registered?`<div class="user-account-panel"><strong>SINCRONIZACIÓN ACTIVA</strong><p>Tu progreso, desbloqueos y preferencias se sincronizan automáticamente con tu cuenta cada vez que cambian.</p><button id="userLogout" class="secondary-btn" type="button">CERRAR SESIÓN</button><p id="userStatus" class="note"></p></div>`:registrationPanelHtml()}</section>`;$$('[data-avatar-id]').forEach(b=>b.onclick=()=>chooseAvatar(b.dataset.avatarId));if(registered){$('#userLogout').onclick=logoutUser}else bindRegistrationPanel();bindBrokenImages();updateUserChrome()}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.1.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
