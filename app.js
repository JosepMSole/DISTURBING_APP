...
// v6.4 · Sistema de TUTORIALES. TUTORIAL 1: PRIMER CONTACTO.
...
const TUTORIAL1_STEPS=[
  {step:1,route:'home',title:'BIENVENID@',text:'Bienvenid@ a Disturbing Stories App. Aquí podrás leer, explorar y descubrir todo el universo de Disturbing Stories.',tk:'tk_t01_p01.webm',tkLayout:'square',composition:'center-tk-text',centerOffsetY:{desktop:-90,mobile:-68},tkPlacement:{desktop:{width:'380px',height:'380px'},mobile:{width:'284px',height:'284px'}},bubblePlacement:{desktop:{width:'390px'},mobile:{width:'calc(100vw - 28px)'}},dim:.66},
  {step:2,route:'home',title:'TODAS LAS STORIES',text:'Empieza aquí. En TODAS LAS STORIES encontrarás todas las Stories publicadas.',tk:'tk_t01_p02.webm',tkLayout:'vertical',target:'.nav-btn[data-route="stories"]',focusTarget:'.nav-btn[data-route="stories"] .nav-icon',liftTargetSelector:'.nav-btn[data-route="stories"] .nav-icon',composition:'target-stack',focusStrong:true,focusDelay:2000,liftTarget:true,tkPlacement:{desktop:{width:'300px',height:'460px'},mobile:{width:'236px',height:'356px'}},bubblePlacement:{desktop:{width:'390px'},mobile:{width:'calc(100vw - 28px)'}},dim:.70},
...
function tutorialOverlayHtml(cfg){return `<div id="tutorialOverlay" class="tutorial-overlay-v61 tutorial-step-${cfg.step}${cfg.focusStrong?' tutorial-has-strong-focus':''}" style="--tutorial-dim:${cfg.dim??.64}"><div class="tutorial-dim-v61"></div><div class="tutorial-focus-ring-v61 hidden" aria-hidden="true"></div><div class="tutorial-tk-stage-v61 tutorial-tk-${cfg.tkLayout||'square'}"><div class="tutorial-tk-card"><video src="./assets/tutorials/tk/${escAttr(cfg.tk)}?v=${encodeURIComponent(APP_VERSION)}" autoplay muted loop playsinline preload="auto" aria-label="T.K." onerror="this.classList.add('missing')"></video><div class="tutorial-tk-fallback" aria-hidden="true"><span>TK</span></div></div></div>${tutorialLockMarkup(cfg.lockMode,cfg)}<section class="tutorial-bubble-v61" role="dialog" aria-modal="true" aria-label="Tutorial · ${escAttr(cfg.title)}"><button type="button" class="tutorial-close-v61" data-tutorial-close aria-label="Cerrar tutorial">×</button><div class="tutorial-step-label"><i aria-hidden="true"></i>PASO ${cfg.step}/5</div><h2>${escapeHtml(cfg.title)}</h2><p class="tutorial-main-text">${escapeHtml(cfg.text)}</p>${cfg.secondary?`<p class="tutorial-secondary-text">${escapeHtml(cfg.secondary)}</p>`:''}${cfg.micro?`<div class="tutorial-micro-rule">${escapeHtml(cfg.micro)}</div>`:''}${tutorialButtonMarkup(cfg.step)}</section></div>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=6.4.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
