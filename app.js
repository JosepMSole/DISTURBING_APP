...
let readingRestoreSession=null;
...
const storiesCleanPostersKey='disturbing_stories_clean_posters_v1';
...
const UI_SOUND_FILES={open:'open.mp3',openb:'openb.mp3',pages:'pages.mp3',back:'back.mp3',tap:'tap.mp3',expand:'expand.mp3',collapse:'collapse.mp3',confirm:'confirm.mp3',blocked:'blocked.mp3',home:'home.mp3',risa:'risa.mp3',ruletagira:'ruletagira.mp3',ruletaresultado:'ruletaresultado.mp3'};
const UI_SOUND_VOLUME={open:.32,openb:.32,pages:.32,back:.32,tap:.24,expand:.28,collapse:.28,confirm:.34,blocked:.31,home:.34,risa:.40,ruletagira:.38,ruletaresultado:.46};
...
function startRandomSpinSound(){
  if(!uiSoundsEnabled()||document.body.classList.contains('micro-preview-mode'))return;
  const a=uiSoundEngine('ruletagira');if(!a)return;
  try{a.pause();a.currentTime=0;a.loop=true;a.volume=UI_SOUND_VOLUME.ruletagira??.38;const p=a.play();if(p&&typeof p.catch==='function')p.catch(()=>{})}catch{}
}
function stopRandomSpinSound(){
  const a=uiSoundEngines.get('ruletagira');if(!a)return;
...
function renderHome(){
  ...
  bindStoryCards();bindBrokenImages();updateInstallButton();startHomeNextStoryCountdown();setupHomeNextStoryReveal();setupHomeProgressReveal();bindHomeInteractionFx();maybeAutoTutorial('home');
}
function bindHomeInteractionFx(){
  if(view.dataset.homeInteractionFxBound==='1')return;view.dataset.homeInteractionFxBound='1';
  const selector='button,a,[role="button"],[data-story-id]';
  const pulse=el=>{if(!view.classList.contains('home-view')||!el||el.disabled||el.getAttribute?.('aria-disabled')==='true')return;el.classList.remove('home-interaction-glow');void el.offsetWidth;el.classList.add('home-interaction-glow');clearTimeout(el._dsHomeGlowTimer);el._dsHomeGlowTimer=setTimeout(()=>el.classList.remove('home-interaction-glow'),430)};
  view.addEventListener('pointerdown',e=>pulse(e.target?.closest?.(selector)),{passive:true});
  view.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' ')pulse(e.target?.closest?.(selector))});
}
function renderStories(initial='all'){
  currentStory=null;
  const savedView=store.get(storiesViewKey,'covers'),validViews=['covers','grid','list'];
  let viewMode=validViews.includes(savedView)?savedView:'covers',cleanPosters=store.get(storiesCleanPostersKey,false)===true,currentFilter=initial,search='',sortMode='published-desc',feature='all',category='all',book='all';
  ...
  const draw=()=>{
    ...
    const list=$('#storiesList');list.className=`stories-view stories-view-${viewMode}${cleanPosters?' stories-clean-posters':''}`;list.innerHTML=a.length?a.map(s=>storyCard(s,viewMode,sortMode,{filteredResults:true,clean:cleanPosters})).join(''):'<div class="empty">No hay Stories con estos criterios.</div>';
  ...
  $$('.view-mode[data-view]').forEach(b=>b.onclick=()=>{viewMode=b.dataset.view;store.set(storiesViewKey,viewMode);draw()});
  $('#togglePosterOverlays').onclick=()=>{cleanPosters=!cleanPosters;store.set(storiesCleanPostersKey,cleanPosters);draw()};
...
function storyHeaderInfoPills(s){const bits=[];const rt=storyReadTimeText(s);if(rt)bits.push(`<span class="pill" title="Tiempo de lectura">◷ ${escapeHtml(rt)}</span>`);const book=String(s?.book||'').trim();if(book)bits.push(`<span class="pill" title="Libro de la colección">BOOK#${escapeHtml(String(Number(book)||book))}</span>`);return bits.join('')}
...
function storyCard(s,mode='list',sortMode='',opts={}){
  const filteredResults=!!opts.filteredResults,clean=!!opts.clean;
  const locked=isLocked(s),read=isRead(s.id),cover=displayCover(s),pr=storyProgress(s.id),ex=exclusiveBadge(s,'cover'),media=storyMediaBadges(s),sortBadge=filteredResults?storySortBadge(s,sortMode):'',readCorner=readCornerBadge(s),fresh=filteredResults?'':newStoryBadge(s),rtBadge=filteredResults?'':storyReadTimeBadge(s);
  ...
  const overlayClass=clean?' story-overlays-clean':'';
  const thumb=`<div class="story-thumb-wrap${overlayClass}">${image}${readCorner}${sortBadge}${fresh}${ex}${media}${rtBadge}</div>`;
  if(mode==='covers')return `<article class="story-cover-card${overlayClass}" data-story-id="${escAttr(s.id)}" aria-label="Story ${escAttr(s.id)} · ${escAttr(s.title||'')}">${image}${readCorner}${sortBadge}${fresh}${!filteredResults&&(read?'<span class="cover-read">✓ LEÍDA</span>':pr>.015?`<span class="cover-progress">${pct(pr)}</span>`:'')}${ex}${media}${rtBadge}</article>`;
  if(mode==='grid')return `<article class="story-grid-card${overlayClass}" data-story-id="${escAttr(s.id)}"><div class="story-grid-poster${overlayClass}">${image}${readCorner}${sortBadge}${fresh}${ex}${media}${rtBadge}</div><div class="story-grid-copy"><div class="story-number">STORY ${escapeHtml(s.id)}</div><h3>${escapeHtml(s.title)}</h3><p>${s.access==='exclusive'?(locked?'EXCLUSIVA · BLOQUEADA':'EXCLUSIVA · DESBLOQUEADA'):'GRATIS'}${read?' · LEÍDA':''}</p></div></article>`;
  return `<article class="story-card${overlayClass}" data-story-id="${escAttr(s.id)}">${thumb}<div class="story-copy"><div class="story-number">STORY ${escapeHtml(s.id)} · ${formatDate(s.published)}</div><h3>${escapeHtml(s.title)}</h3><p>${s.access==='exclusive'?(locked?'Exclusiva · bloqueada':'Exclusiva · desbloqueada'):'Gratis'}${read?' · Leída':''}</p></div><div class="story-state">${read?'✓':'›'}</div></article>`;
}
...
function extraNumber(x){const raw=x?.number??x?.id??'';const direct=Number(raw);if(Number.isFinite(direct)&&direct>0)return direct;const m=String(raw).match(/(\d+)/);return m?Number(m[1]):0}
function extraForBook(bookId){const n=Number(bookId);if(!Number.isFinite(n)||n<=0)return null;return (extrasData?.extras||[]).find(x=>extraNumber(x)===n)||null}
function bindBookExtraCards(){$$('#bookStoriesGrid [data-extra-id]').forEach(b=>{const open=()=>runCatalogSelectFx(b,()=>go(`extra/${encodeURIComponent(b.dataset.extraId)}`));b.onclick=open})}
function renderCollection(bookId=''){
  ...
  if(bookId&&books.has(bookId)){
    const stories=[...books.get(bookId)].sort((a,b)=>new Date(b.published)-new Date(a.published)),bookExtra=extraForBook(bookId),read=stories.filter(s=>readIds.has(s.id)).length,available=stories.filter(accessibleStory).length,bookCompleteRead=read>=15,asset=bookAsset(bookId),hasAudio=stories.some(s=>storyHasFeature(s,'audio')),hasVideo=stories.some(s=>storyHasFeature(s,'video')),hasExclusive=stories.some(s=>s.access==='exclusive');
...
function readingPositionForScroll(reader,y=scrollY){
  if(!reader)return 0;const rect=reader.getBoundingClientRect(),top=scrollY+rect.top,h=Math.max(1,reader.offsetHeight-innerHeight*.65);return Math.max(0,Math.min(1,(Number(y)-top+innerHeight*.3)/h))
}
function readingScrollForPosition(reader,position){
  if(!reader)return 0;const rect=reader.getBoundingClientRect(),top=scrollY+rect.top,h=Math.max(1,reader.offsetHeight-innerHeight*.65),raw=top+Math.max(0,Math.min(1,Number(position)||0))*h-innerHeight*.3,max=Math.max(0,Math.max(document.documentElement?.scrollHeight||0,document.body?.scrollHeight||0)-innerHeight);return Math.max(0,Math.min(max,raw))
}
function readingRestoreIsActive(id=''){return !!readingRestoreSession&&readingRestoreSession.active&&(!id||String(readingRestoreSession.id)===String(id))}
function persistCurrentReadingPosition(){
  const r=$('#reader');if(!r||!currentStory||currentTab!=='read'||readingRestoreIsActive(currentStory.id))return;const x=readingPositionForScroll(r),maxProgress=Math.max(storyProgress(currentStory.id),x);saveReadingState(currentStory.id,maxProgress,x,r);
}
...
function updateProgress(){
  ...
  const x=readingPositionForScroll(r);p.style.width=`${x*100}%`;
  // Durante una reanudación automática solo se actualiza la UI. Nunca se sobrescribe
  // el punto guardado con posiciones intermedias del propio scroll automático.
  if(readingRestoreIsActive(currentStory.id))return;
...
function saveReadingState(id,progress,position,reader){
  ...
  kids.forEach((el,i)=>{const er=el.getBoundingClientRect(),d=Math.abs(er.top-target);if(er.top<=target+80&&d<best){best=d;anchor=i}});
  const states=readingStateMap();states[id]={progress:Math.round(progress*10000)/10000,position:Math.round(position*10000)/10000,anchor,updated:Date.now()};store.set(readingStateKey,states);
}
...
function stopReadingRestore({keepCurrent=false}={}){
  const session=readingRestoreSession;if(!session)return;session.active=false;if(session.raf)cancelAnimationFrame(session.raf);if(session.settleRaf)cancelAnimationFrame(session.settleRaf);if(session.timer)clearTimeout(session.timer);try{session.resizeObserver?.disconnect()}catch{};for(const [type,fn] of session.listeners||[])window.removeEventListener(type,fn,true);if(session.reader){session.reader.style.overflowAnchor=session.prevReaderAnchor||''}document.documentElement.style.overflowAnchor=session.prevHtmlAnchor||'';if(document.body)document.body.style.overflowAnchor=session.prevBodyAnchor||'';readingRestoreSession=null;if(keepCurrent&&currentStory&&session.reader?.isConnected){const x=readingPositionForScroll(session.reader);saveReadingState(currentStory.id,Math.max(storyProgress(currentStory.id),x),x,session.reader);requestAnimationFrame(updateProgress)}}
function restoreReadPosition(id){
  const r=$('#reader');if(!r)return;stopReadingRestore();
  const state=readingStateMap()[id]||{progress:storyProgress(id),position:storyProgress(id)},stored=Math.max(0,Math.min(1,Number(state.progress??state.position)||0));
  const images=[...r.querySelectorAll('img')];images.forEach(img=>{try{img.loading='eager';img.decoding='sync';img.setAttribute('fetchpriority','high')}catch{}});
  const session={id:String(id),position:stored,reader:r,active:true,raf:0,settleRaf:0,timer:0,resizeObserver:null,listeners:[],lastLayoutChange:performance.now(),prevReaderAnchor:r.style.overflowAnchor,prevHtmlAnchor:document.documentElement.style.overflowAnchor,prevBodyAnchor:document.body?.style.overflowAnchor||''};readingRestoreSession=session;
  r.style.overflowAnchor='none';document.documentElement.style.overflowAnchor='none';if(document.body)document.body.style.overflowAnchor='none';
  const targetY=()=>readingScrollForPosition(r,stored);
  const userCancel=()=>{if(readingRestoreSession!==session)return;stopReadingRestore({keepCurrent:true})};
  for(const type of ['wheel','touchstart','pointerdown','keydown']){window.addEventListener(type,userCancel,{passive:true,capture:true});session.listeners.push([type,userCancel])}
  if('ResizeObserver'in window){session.resizeObserver=new ResizeObserver(()=>{session.lastLayoutChange=performance.now()});session.resizeObserver.observe(r)}
  for(const img of images.filter(i=>!i.complete)){const changed=()=>{session.lastLayoutChange=performance.now()};img.addEventListener('load',changed,{once:true});img.addEventListener('error',changed,{once:true})}
  const finishWhenStable=()=>{
    if(readingRestoreSession!==session||!session.active)return;const dest=targetY(),delta=Math.abs(scrollY-dest);if(delta>.75)scrollTo({top:dest,behavior:'auto'});const allImagesDone=images.every(i=>i.complete),stableFor=performance.now()-session.lastLayoutChange;
    if((allImagesDone&&stableFor>650)||performance.now()-session.startedAt>8000){scrollTo({top:targetY(),behavior:'auto'});const maxProgress=Math.max(storyProgress(id),stored);saveReadingState(id,maxProgress,stored,r);stopReadingRestore();requestAnimationFrame(updateProgress);return}
    session.settleRaf=requestAnimationFrame(finishWhenStable)
  };
  const begin=()=>{
    if(readingRestoreSession!==session||!session.active)return;session.startedAt=performance.now();const from=scrollY,duration=820;
    const step=now=>{if(readingRestoreSession!==session||!session.active)return;const t=Math.min(1,(now-session.startedAt)/duration),ease=1-Math.pow(1-t,3),dest=targetY();scrollTo({top:from+(dest-from)*ease,behavior:'auto'});if(t<1)session.raf=requestAnimationFrame(step);else{scrollTo({top:targetY(),behavior:'auto'});session.lastLayoutChange=performance.now();session.settleRaf=requestAnimationFrame(finishWhenStable)}};
    session.raf=requestAnimationFrame(step)
  };
  session.timer=setTimeout(begin,300)
}
function qrScanControlsHtml(){
  return `<div class="scan-zone qr-photo-only"><div id="qr-reader" class="qr-reader"><div class="qr-photo-mode"><div class="qr-app-demo-wrap" aria-hidden="true"><img class="qr-app-demo" src="./assets/qr_app.png?v=${encodeURIComponent(APP_VERSION)}" alt=""></div><span>Haz una foto clara y centrada del código</span></div></div><div id="qr-file-reader" class="qr-file-reader" aria-hidden="true"></div><button class="cta" id="photoQr">HACER FOTO DEL QR</button><button class="secondary-btn qr-gallery-btn" id="galleryQr" style="margin-top:8px">ELEGIR FOTO DE TU GALERÍA</button><input id="qrCameraFile" class="qr-native-input" type="file" accept="image/*" capture="environment"><input id="qrFile" class="qr-native-input" type="file" accept="image/*"><p id="unlockStatus" class="note" style="margin-top:12px"></p></div>`
}
...
function bindQrControls(prefill=''){
  ...
  if(photo)photo.onclick=()=>{
    unlockStatus('');
    if(!openQrNativeCamera())unlockStatus('No se pudo abrir la cámara. Usa «ELEGIR FOTO DE TU GALERÍA».')
  };
...
function achievementRewardSignature(a){const r=a?.reward||{};if(r.type==='unlock_story'&&r.storyId)return`${String(a?.id||'')}|unlock_story|${String(r.storyId)}`;if(r.type==='unlock_avatar'&&r.avatarId)return`${String(a?.id||'')}|unlock_avatar|${String(r.avatarId)}`;return''}
...
async function unlockAvatarAchievementReward(a,{showModal=true}={}){const r=a?.reward||{},avatarId=String(r.avatarId||'').trim(),avatar=avatarById(avatarId);if(!avatar)throw new Error(`Avatar ${avatarId||'?'} no disponible`);const ok=await unlockAvatarById(avatarId,showModal);if(!ok)throw new Error(`No se pudo desbloquear el avatar ${avatarId}`);rememberAchievementReward(a);return true}
async function executeAchievementReward(a,{showModal=true}={}){if(!achievementRewardPending(a))return false;const r=a?.reward||{};if(r.type==='unlock_story')return unlockStoryAchievementReward(a,{showModal});if(r.type==='unlock_avatar')return unlockAvatarAchievementReward(a,{showModal});return false}
...
function animateRegisteredUserHero(){
  ...
  const avatar=hero.querySelector('.user-avatar-current'),level=hero.querySelector('.user-hero-level'),star=hero.querySelector('.user-hero-level-star'),name=hero.querySelector('.user-hero-name'),status=hero.querySelector('.user-hero-status'),email=hero.querySelector('.user-email-box strong'),edit=hero.querySelector('.user-edit-name-btn'),levelNumber=hero.querySelector('.user-hero-level-number');
...
function vaultOwnerId(){return String(localStorage.getItem(vaultOwnerKey)||'')}
function setVaultOwnerId(id){if(id)localStorage.setItem(vaultOwnerKey,String(id));else localStorage.removeItem(vaultOwnerKey)}
function pendingAnonMerge(){try{const x=JSON.parse(localStorage.getItem(pendingAnonMergeKey)||'null');return x&&typeof x==='object'?x:null}catch{return null}}
function setPendingAnonMerge(x){localStorage.setItem(pendingAnonMergeKey,JSON.stringify(x||null))}
function clearPendingAnonMerge(){localStorage.removeItem(pendingAnonMergeKey)}
function anonymousVaultBackup(){try{const x=JSON.parse(localStorage.getItem(anonymousVaultBackupKey)||'{}');return x&&typeof x==='object'?x:{}}catch{return {}}}
function setAnonymousVaultBackup(vault){localStorage.setItem(anonymousVaultBackupKey,JSON.stringify(vault&&typeof vault==='object'?vault:{}))}
function clearLocalVaultData(){const keys=[];for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith('disturbing_'))keys.push(k)}keys.forEach(k=>localStorage.removeItem(k))}
function replaceLocalVault(vault,owner=''){clearLocalVaultData();applyVault(vault||{});setVaultOwnerId(owner||'anon')}
function restoreAnonymousVault(){const backup=anonymousVaultBackup();replaceLocalVault(backup,'anon');ensureFreeAvatars();initialiseNewState();updateAppBadge()}
...
function applyVault(vault){if(!vault||typeof vault!=='object')return;for(const [k,v] of Object.entries(vault)){if(!String(k).startsWith('disturbing_'))continue;localStorage.setItem(k,JSON.stringify(v))}}
function pendingMergeMatchesUser(pending,user){if(!pending||!user)return false;const pid=String(pending.userId||''),uid=String(user.id||''),pe=String(pending.email||'').trim().toLowerCase(),ue=String(user.email||'').trim().toLowerCase();return (!!pid&&pid===uid)||(!pid&&!!pe&&pe===ue)}
async function loadRegisteredProfileAndMerge(){
  if(!registeredUser||!supabaseClient)return;const uid=String(registeredUser.id||'');if(userVaultLoadPromise&&userVaultLoadUserId===uid)return userVaultLoadPromise;
  userVaultLoadUserId=uid;userVaultLoadPromise=(async()=>{userSyncBusy=true;try{
    const profileQuery=supabaseClient.from('profiles').select('*').eq('id',uid),vaultQuery=supabaseClient.from('user_vaults').select('*').eq('user_id',uid),profileReq=typeof profileQuery.maybeSingle==='function'?profileQuery.maybeSingle():profileQuery.single(),vaultReq=typeof vaultQuery.maybeSingle==='function'?vaultQuery.maybeSingle():vaultQuery.single();
    const [{data:profile,error:pe},{data:vaultRow,error:ve}]=await Promise.all([profileReq,vaultReq]);if(pe&&String(pe.code||'')!=='PGRST116')throw pe;if(ve&&String(ve.code||'')!=='PGRST116')throw ve;registeredVaultRowExists=!!vaultRow;
    userProfile=profile||{id:uid,display_name:String(registeredUser.user_metadata?.display_name||''),avatar_id:registeredUser.user_metadata?.avatar_id||null};
    const remote=vaultRow?.vault&&typeof vaultRow.vault==='object'?vaultRow.vault:{},owner=vaultOwnerId(),pending=pendingAnonMerge(),pendingMatch=pendingMergeMatchesUser(pending,registeredUser);let nextVault;
    if(pendingMatch){nextVault=mergeVault(remote,pending.vault||{});clearPendingAnonMerge();setAnonymousVaultBackup({})}
    else if(owner===uid){nextVault=mergeVault(remote,localVaultSnapshot())}
    else{if(owner==='anon')setAnonymousVaultBackup(localVaultSnapshot());nextVault=remote}
    replaceLocalVault(nextVault,uid);if(userProfile?.display_name)store.set(userNameCacheKey,userProfile.display_name);const chosen=userProfile?.avatar_id||store.get(localAvatarKey,'avatar1');if(chosen)store.set(localAvatarKey,chosen);ensureFreeAvatars();initialiseNewState();
    const vaultNow=localVaultSnapshot(),vaultPayload={user_id:uid,vault:vaultNow,vault_schema:1,updated_at:new Date().toISOString()};let writeResult;if(registeredVaultRowExists)writeResult=await supabaseClient.from('user_vaults').update({vault:vaultNow,vault_schema:1,updated_at:vaultPayload.updated_at}).eq('user_id',uid);else writeResult=await supabaseClient.from('user_vaults').upsert(vaultPayload,{onConflict:'user_id'});if(writeResult?.error)console.warn('User vault write',writeResult.error);else registeredVaultRowExists=true;
    await evaluateAvatarUnlocks(false);evaluateAchievements(false);updateUserChrome();updateAppBadge();if(routeName()==='user')renderUser()
  }catch(e){console.warn('User sync',e)}finally{userSyncBusy=false}})();try{return await userVaultLoadPromise}finally{if(userVaultLoadUserId===uid){userVaultLoadPromise=null;userVaultLoadUserId=''}}}
let userSyncTimer=0;function queueUserVaultSync(){if(!registeredUser||!supabaseClient||vaultOwnerId()!==String(registeredUser.id||''))return;clearTimeout(userSyncTimer);userSyncTimer=setTimeout(()=>syncUserVault(),650)}
async function syncUserVault(){if(!registeredUser||!supabaseClient||userSyncBusy||vaultOwnerId()!==String(registeredUser.id||''))return;userSyncBusy=true;try{const uid=String(registeredUser.id),vault=localVaultSnapshot(),updated_at=new Date().toISOString();let result;if(registeredVaultRowExists)result=await supabaseClient.from('user_vaults').update({vault,vault_schema:1,updated_at}).eq('user_id',uid);else result=await supabaseClient.from('user_vaults').upsert({user_id:uid,vault,vault_schema:1,updated_at},{onConflict:'user_id'});if(result?.error)throw result.error;registeredVaultRowExists=true}catch(e){console.warn('Automatic user sync',e)}finally{userSyncBusy=false}}
...
// v6.5 · Sistema de TUTORIALES. TUTORIAL 1: PRIMER CONTACTO.
...
const TUTORIAL1_STEPS=[
  {step:1,route:'home',title:'BIENVENID@',text:'Bienvenid@ a Disturbing Stories App. Aquí podrás leer, explorar y descubrir todo este universo de relatos cortos de terror y ciencia ficción.',tk:'tk_t01_p01.webm',voice:'tk_t01_p01.mp3',tkLayout:'square',composition:'center-tk-text',centerOffsetY:{desktop:-116,mobile:-90},tkPlacement:{desktop:{width:'380px',height:'380px'},mobile:{width:'284px',height:'284px'}},bubblePlacement:{desktop:{width:'390px'},mobile:{width:'calc(100vw - 28px)'}},dim:.66},
  {step:2,route:'home',title:'TODAS LAS STORIES',text:'Empieza aquí. En TODAS LAS STORIES encontrarás todas las Stories publicadas.',tk:'tk_t01_p02.webm',voice:'tk_t01_p02.mp3',tkLayout:'vertical',target:'.nav-btn[data-route="stories"]',focusTarget:'.nav-btn[data-route="stories"] .nav-icon',liftTargetSelector:'.nav-btn[data-route="stories"] .nav-icon',composition:'target-stack',targetGap:27,focusStrong:true,focusDelay:2000,liftTarget:true,tkPlacement:{desktop:{width:'300px',height:'460px'},mobile:{width:'236px',height:'356px'}},bubblePlacement:{desktop:{width:'390px'},mobile:{width:'calc(100vw - 28px)'}},dim:.70},
  {step:3,route:'stories',title:'PÚBLICAS Y EXCLUSIVAS',text:'Algunas Stories puedes leerlas directamente. Otras son exclusivas y aparecerán bloqueadas.',tk:'tk_t01_p03.webm',voice:'tk_t01_p03.mp3',tkLayout:'horizontal',composition:'center-stack-lock',tkPlacement:{desktop:{width:'410px',height:'218px'},mobile:{width:'calc(100vw - 32px)',height:'174px'}},bubblePlacement:{desktop:{width:'410px'},mobile:{width:'calc(100vw - 32px)'}},lockMode:'locked',lockEntryDelay:2120,ambientScroll:'start',dim:.54},
  {step:4,route:'stories',title:'DESBLOQUEO POR QR',text:'Cada Story exclusiva tiene su propio QR. Lo encontrarás junto a esa Story en uno de los libros físicos de Disturbing Stories. Escanéalo y quedará desbloqueada.',micro:'1 QR = 1 STORY',tk:'tk_t01_p04.webm',voice:'tk_t01_p04.mp3',tkLayout:'square',composition:'center-stack-lock',tkPlacement:{desktop:{width:'240px',height:'240px'},mobile:{width:'190px',height:'190px'}},bubblePlacement:{desktop:{width:'430px'},mobile:{width:'calc(100vw - 32px)'}},lockMode:'unlock',unlockDelay:3700,ambientScroll:'continue',dim:.54},
...
let tutorialTimers=[],tutorialScrollRaf=0,tutorialScrollLast=0,tutorialScrollSpeed=12,tutorialReturnRaf=0,tutorialResizeHandler=null,tutorialAutoScheduled=false,tutorialVoiceAudio=null,tutorialVoiceGestureRetry=null;
function stopTutorialVoice(){if(tutorialVoiceGestureRetry){removeEventListener('pointerdown',tutorialVoiceGestureRetry,true);removeEventListener('keydown',tutorialVoiceGestureRetry,true);tutorialVoiceGestureRetry=null}if(tutorialVoiceAudio){try{tutorialVoiceAudio.pause();tutorialVoiceAudio.removeAttribute('src');tutorialVoiceAudio.load()}catch{}tutorialVoiceAudio=null}}
function playTutorialVoice(cfg){stopTutorialVoice();const file=String(cfg?.voice||'').trim();if(!file)return;const a=document.createElement('audio');a.preload='auto';a.volume=.96;a.setAttribute('aria-hidden','true');let remoteTried=false;const clearRetry=()=>{if(!tutorialVoiceGestureRetry)return;removeEventListener('pointerdown',tutorialVoiceGestureRetry,true);removeEventListener('keydown',tutorialVoiceGestureRetry,true);tutorialVoiceGestureRetry=null};const armRetry=()=>{if(tutorialVoiceGestureRetry)return;tutorialVoiceGestureRetry=()=>{clearRetry();if(tutorialVoiceAudio===a&&tutorialActive?.step===cfg.step){try{a.currentTime=0}catch{}const p=a.play();if(p&&typeof p.catch==='function')p.catch(()=>{})}};addEventListener('pointerdown',tutorialVoiceGestureRetry,true);addEventListener('keydown',tutorialVoiceGestureRetry,true)};const attempt=()=>{if(tutorialVoiceAudio!==a||tutorialActive?.step!==cfg.step)return;try{const p=a.play();if(p&&typeof p.catch==='function')p.catch(armRetry)}catch{armRetry()}};a.onerror=()=>{if(remoteTried)return;remoteTried=true;a.src=PUBLIC_APP_ASSET_BASE+'tutorials/audio/'+encodeURIComponent(file)+'?v='+encodeURIComponent(APP_VERSION);try{a.load()}catch{};tutorialLater(attempt,80)};tutorialVoiceAudio=a;a.src='./assets/tutorials/audio/'+encodeURIComponent(file)+'?v='+encodeURIComponent(APP_VERSION);try{a.load()}catch{};tutorialLater(attempt,170)}
...
function tutorialScrollTop(){return Math.max(0,window.scrollY||window.pageYOffset||tutorialScrollElement().scrollTop||0)}
function tutorialSetScroll(y){const el=tutorialScrollElement(),v=Math.max(0,Number(y)||0);try{window.scrollTo({top:v,left:0,behavior:'auto'})}catch{window.scrollTo(0,v)}el.scrollTop=v;if(document.documentElement&&document.documentElement!==el)document.documentElement.scrollTop=v;if(document.body&&document.body!==el)document.body.scrollTop=v}
...
function tutorialMaxScroll(){const root=document.documentElement,body=document.body;return Math.max(0,Math.max(root?.scrollHeight||0,body?.scrollHeight||0,tutorialScrollElement().scrollHeight||0)-innerHeight)}
function tutorialStoriesStartTop(){const list=document.querySelector('#storiesList'),header=document.querySelector('.topbar');if(!list)return 0;const headerH=header?.getBoundingClientRect().height||70;return Math.max(0,tutorialScrollTop()+list.getBoundingClientRect().top-headerH-8)}
function startTutorialStoryScroll(reset=false){if(tutorialReducedMotion())return;if(tutorialReturnRaf){cancelAnimationFrame(tutorialReturnRaf);tutorialReturnRaf=0}if(reset)tutorialSetScroll(tutorialStoriesStartTop());if(tutorialScrollRaf)return;tutorialScrollLast=performance.now();tutorialScrollSpeed=innerWidth<=600?42:54;const tick=now=>{if(!tutorialActive||![3,4].includes(tutorialActive.step)||routeName()!=='stories'){tutorialScrollRaf=0;return}const dt=Math.min(.05,Math.max(0,(now-tutorialScrollLast)/1000));tutorialScrollLast=now;const current=tutorialScrollTop(),max=tutorialMaxScroll();if(current<max-2)tutorialSetScroll(Math.min(max,current+tutorialScrollSpeed*dt));tutorialScrollRaf=requestAnimationFrame(tick)};tutorialScrollRaf=requestAnimationFrame(tick)}
function animateTutorialScrollTo(target,duration=2000){if(tutorialReturnRaf)cancelAnimationFrame(tutorialReturnRaf);const start=tutorialScrollTop(),delta=target-start,t0=performance.now();return new Promise(resolve=>{const tick=now=>{if(!tutorialActive||tutorialActive.step!==5){tutorialReturnRaf=0;resolve();return}const p=Math.min(1,(now-t0)/duration),e=p<.5?4*p*p*p:1-Math.pow(-2*p+2,3)/2;tutorialSetScroll(start+delta*e);if(p<1)tutorialReturnRaf=requestAnimationFrame(tick);else{tutorialReturnRaf=0;resolve()}};tutorialReturnRaf=requestAnimationFrame(tick)})}
function finishTutorialStoryScroll(){if(tutorialReducedMotion()){stopTutorialStoryScroll();tutorialSetScroll(0);return}if(tutorialScrollRaf)cancelAnimationFrame(tutorialScrollRaf);tutorialScrollRaf=0;const start=performance.now(),v0=tutorialScrollSpeed||42,last={t:start};const brake=now=>{if(!tutorialActive||tutorialActive.step!==5)return;const p=Math.min(1,(now-start)/800),dt=Math.min(.05,(now-last.t)/1000);last.t=now;const velocity=v0*Math.pow(1-p,2),current=tutorialScrollTop(),max=tutorialMaxScroll();if(current<max-1)tutorialSetScroll(Math.min(max,current+velocity*dt));if(p<1)requestAnimationFrame(brake);else tutorialLater(()=>{if(tutorialActive?.step===5)animateTutorialScrollTo(0,2000)},2000)};requestAnimationFrame(brake)}
...
let tutorialLockBridge=null;
function clearTutorialLockBridge(){tutorialLockBridge?.remove();tutorialLockBridge=null}
function createTutorialLockBridge(){clearTutorialLockBridge();const source=document.querySelector('#tutorialOverlay .tutorial-lock-stage');if(!source)return false;const r=source.getBoundingClientRect(),bridge=document.createElement('div');bridge.className='tutorial-lock-bridge-v64';bridge.innerHTML=lockSvg(false);Object.assign(bridge.style,{left:`${r.left}px`,top:`${r.top}px`,width:`${r.width}px`,height:`${r.height}px`});document.body.append(bridge);tutorialLockBridge=bridge;return true}
function landTutorialLockBridge(){const bridge=tutorialLockBridge,target=$('#tutorialOverlay .tutorial-lock-stage');if(!bridge||!target)return;target.classList.add('is-bridge-target');requestAnimationFrame(()=>requestAnimationFrame(()=>{if(!tutorialLockBridge||!target.isConnected)return;const r=target.getBoundingClientRect();bridge.classList.add('is-moving');Object.assign(bridge.style,{left:`${r.left}px`,top:`${r.top}px`,width:`${r.width}px`,height:`${r.height}px`});tutorialLater(()=>{if(target.isConnected){target.classList.remove('is-bridge-target');target.classList.add('is-bridged')}clearTutorialLockBridge()},360)}))}
function positionTutorialComposition(cfg){const tk=$('.tutorial-tk-stage-v61'),bubble=$('.tutorial-bubble-v61'),lock=$('.tutorial-lock-stage');if(!tk||!bubble)return;const gap=12,edge=12;if(cfg.composition==='center-tk-text'){const tr=tk.getBoundingClientRect(),br=bubble.getBoundingClientRect(),rawOffset=innerWidth<=600?cfg.centerOffsetY?.mobile:cfg.centerOffsetY?.desktop,offset=Number(rawOffset)||0;tutorialSetBox(tk,(innerWidth-tr.width)/2,(innerHeight-tr.height)/2+offset);const fresh=tk.getBoundingClientRect();tutorialSetBox(bubble,tutorialClamp((innerWidth-br.width)/2,edge,innerWidth-br.width-edge),fresh.bottom+gap);return}if(cfg.composition==='target-stack'){const target=document.querySelector(cfg.target),tr=tk.getBoundingClientRect(),br=bubble.getBoundingClientRect();if(!target)return;const rr=target.getBoundingClientRect(),desiredBottom=rr.top-(Number(cfg.targetGap)||16),bTop=desiredBottom-br.height,tTop=bTop-gap-tr.height,groupShift=tTop<edge?edge-tTop:0;tutorialSetBox(tk,tutorialClamp((innerWidth-tr.width)/2,edge,innerWidth-tr.width-edge),tTop+groupShift);tutorialSetBox(bubble,tutorialClamp((innerWidth-br.width)/2,edge,innerWidth-br.width-edge),bTop+groupShift);return}if(cfg.composition==='center-stack-lock'&&lock){const tr=tk.getBoundingClientRect(),br=bubble.getBoundingClientRect(),lr=lock.getBoundingClientRect(),total=lr.height+gap+tr.height+gap+br.height,top=Math.max(edge,(innerHeight-total)/2);tutorialSetBox(lock,(innerWidth-lr.width)/2,top);tutorialSetBox(tk,(innerWidth-tr.width)/2,top+lr.height+gap);tutorialSetBox(bubble,tutorialClamp((innerWidth-br.width)/2,edge,innerWidth-br.width-edge),top+lr.height+gap+tr.height+gap);return}if(cfg.composition==='center-tk-text-final'){const tr=tk.getBoundingClientRect(),br=bubble.getBoundingClientRect(),total=tr.height+gap+br.height,top=Math.max(edge,(innerHeight-total)/2);tutorialSetBox(tk,(innerWidth-tr.width)/2,top);tutorialSetBox(bubble,tutorialClamp((innerWidth-br.width)/2,edge,innerWidth-br.width-edge),top+tr.height+gap)}}
function placeTutorialElements(cfg,{activateFocus=false}={}){applyTutorialPlacement($('.tutorial-tk-stage-v61'),cfg.tkPlacement);applyTutorialPlacement($('.tutorial-bubble-v61'),cfg.bubblePlacement);positionTutorialLock(cfg);const focusSelector=cfg.focusTarget||cfg.target,liftSelector=cfg.liftTargetSelector||cfg.target,focusReady=!cfg.focusDelay||activateFocus;positionTutorialFocus(focusSelector,cfg.focusStrong&&focusReady,focusReady);positionTutorialTargetClone(liftSelector,cfg.liftTarget,focusReady);requestAnimationFrame(()=>positionTutorialComposition(cfg))}
function destroyTutorialOverlay({keepScroll=false}={}){clearTutorialTimers();stopTutorialVoice();document.querySelector('#tutorialOverlay')?.remove();if(tutorialResizeHandler){removeEventListener('resize',tutorialResizeHandler);tutorialResizeHandler=null}if(!keepScroll)stopTutorialStoryScroll()}
function bindTutorialInteractionFx(root){if(!root)return;root.querySelectorAll('button,[role="button"]').forEach(el=>{if(el.dataset.tutorialFxBound==='1')return;el.dataset.tutorialFxBound='1';el.addEventListener('pointerdown',()=>{if(tutorialReducedMotion())return;el.classList.remove('tutorial-interaction-flash');void el.offsetWidth;el.classList.add('tutorial-interaction-flash');setTimeout(()=>el.classList.remove('tutorial-interaction-flash'),460)},{passive:true})})}
function startTutorialTypewriter(root){if(!root||tutorialReducedMotion())return;const nodes=[...root.querySelectorAll('[data-tutorial-typewrite]')];if(!nodes.length)return;const speeds={step:22,title:27,main:14,secondary:16,micro:24};let delay=90;for(const node of nodes){const full=node.textContent||'',kind=node.dataset.tutorialTypewrite||'main',speed=speeds[kind]||15;const box=node.getBoundingClientRect();node.style.minHeight=`${Math.ceil(box.height)}px`;if(node.matches('h2,.tutorial-micro-rule'))node.style.minWidth=`${Math.ceil(box.width)}px`;node.textContent='';node.classList.add('tutorial-typewrite-pending');tutorialLater(()=>{if(!node.isConnected)return;node.classList.remove('tutorial-typewrite-pending');node.classList.add('tutorial-typewrite-writing');let i=0;const tick=()=>{if(!node.isConnected)return;i+=1;node.textContent=full.slice(0,i);if(i<full.length)tutorialLater(tick,speed);else node.classList.remove('tutorial-typewrite-writing')};tick()},delay);delay+=Math.max(180,full.length*speed)+105}}
function renderTutorialStep(){if(!tutorialActive||tutorialActive.id!=='tutorial1')return;const cfg=TUTORIAL1_STEPS[tutorialActive.step-1];if(!cfg)return;destroyTutorialOverlay({keepScroll:[3,4].includes(tutorialActive.step)});const host=document.createElement('div');host.innerHTML=tutorialOverlayHtml(cfg);document.body.append(host.firstElementChild);requestAnimationFrame(()=>{placeTutorialElements(cfg);requestAnimationFrame(()=>{const currentOverlay=$('#tutorialOverlay');currentOverlay?.classList.add('is-visible');startTutorialTypewriter(currentOverlay);playTutorialVoice(cfg);if(cfg.step===4&&tutorialLockBridge)landTutorialLockBridge()})});tutorialResizeHandler=()=>placeTutorialElements(cfg,{activateFocus:!cfg.focusDelay||$('#tutorialOverlay')?.classList.contains('tutorial-focus-active')});addEventListener('resize',tutorialResizeHandler,{passive:true});const overlay=$('#tutorialOverlay');bindTutorialInteractionFx(overlay);$('[data-tutorial-close]')?.addEventListener('click',()=>closeTutorial1());$('[data-tutorial-next]')?.addEventListener('click',()=>moveTutorial1(1));$('[data-tutorial-back]')?.addEventListener('click',()=>moveTutorial1(-1));$('[data-tutorial-start]')?.addEventListener('click',()=>finishTutorial1('start'));$('[data-tutorial-more]')?.addEventListener('click',()=>finishTutorial1('more'));if(cfg.focusDelay)tutorialLater(()=>{if(tutorialActive?.step!==cfg.step)return;overlay?.classList.add('tutorial-focus-active');placeTutorialElements(cfg,{activateFocus:true})},tutorialReducedMotion()?40:cfg.focusDelay);if(cfg.ambientScroll==='start')tutorialLater(()=>startTutorialStoryScroll(true),120);else if(cfg.ambientScroll==='continue')tutorialLater(()=>startTutorialStoryScroll(false),80);else if(cfg.ambientScroll==='finish')tutorialLater(()=>finishTutorialStoryScroll(),80);if(cfg.lockMode==='unlock')tutorialLater(()=>$('.tutorial-lock-stage')?.classList.add('is-unlocked'),tutorialReducedMotion()?40:(cfg.unlockDelay||3700))}
...
function showTutorialStep(step){if(!tutorialActive)return;const cfg=TUTORIAL1_STEPS[step-1];if(!cfg)return;const previousStep=tutorialActive.step,bridgeTransition=previousStep===3&&step===4;if(bridgeTransition)createTutorialLockBridge();else clearTutorialLockBridge();tutorialActive.step=step;const current=routeName();if(current===cfg.route){renderTutorialStep();return}destroyTutorialOverlay({keepScroll:false});const target=`#/${cfg.route}`;if(currentHash()!==target)navigateHash(target);else routeFromHash();waitTutorialRoute(cfg.route,renderTutorialStep)}
...
function closeTutorial1(){if(!tutorialActive)return;const viewed=tutorialActive.step>=5;if(viewed)markTutorialViewed('tutorial1');tutorialActive=null;clearTutorialLockBridge();destroyTutorialOverlay()}
function finishTutorial1(action='start'){if(!tutorialActive)return;markTutorialViewed('tutorial1');tutorialActive=null;clearTutorialLockBridge();destroyTutorialOverlay();if(action==='more'){if(currentHash()!=='#/tutorials')navigateHash('#/tutorials');else renderTutorials();return}if(action==='start'){playUiSound('risa');tutorialSetScroll(0);if(currentHash()==='#/stories'){routeFromHash();requestAnimationFrame(()=>scrollTo({top:0,behavior:'auto'}))}else navigateHash('#/stories')}}
function renderTutorials(){currentStory=null;const seen=tutorialViewed('tutorial1');view.innerHTML=`<section class="section tutorials-page-v61"><div class="section-title"><div><div class="eyebrow">GUÍA DE LA APP</div><h1>Tutoriales</h1></div></div><p class="tutorials-copy-v61">Elige el tutorial que quieras ver. Puedes repetirlos tantas veces como necesites.</p><div class="tutorial-list-v61"><button type="button" data-start-tutorial="tutorial1"><span class="tutorial-list-number">01</span><span class="tutorial-list-copy"><strong>PRIMER CONTACTO</strong><small>Lo esencial para empezar a explorar Disturbing Stories App.</small></span>${seen?'<em class="tutorial-viewed-badge">VISTO</em>':'<em class="tutorial-new-badge">VER</em>'}</button></div></section>`;bindTutorialInteractionFx(view);$('[data-start-tutorial="tutorial1"]')?.addEventListener('click',()=>startTutorial1({manual:true}))}
...
function gamesForStoryHtml(s){const rows=gamesForStory(s);if(!rows.length)return'';const title=rows.length===1?'GAME RELACIONADO':'GAMES RELACIONADOS';return `<section id="storyGameRelations" class="story-game-relations"><div class="eyebrow">${title}</div><div class="story-game-relation-grid">${rows.map(g=>{const img=gameAssetUrl(g.listImage||g.cover);return `<button type="button" data-related-game="${escAttr(g.id)}">${img?`<img src="${escAttr(img)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:''}<span><b>GAME #${String(Number(g.number)||0).padStart(2,'0')}</b><strong>${escapeHtml(g.title||g.id)}</strong></span><em aria-hidden="true">›</em></button>`}).join('')}</div></section>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=6.17.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
