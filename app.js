...
// v6.1 · Sistema de TUTORIALES. TUTORIAL 1: PRIMER CONTACTO.
...
const TUTORIAL1_STEPS=[
  {step:1,route:'home',title:'BIENVENIDO',text:'Bienvenido a Disturbing Stories App. Aquí podrás leer, explorar y descubrir todo el universo de Disturbing Stories.',tk:'tk_t01_p01.webm',tkLayout:'square',tkPlacement:{desktop:{left:'5vw',top:'22vh',width:'190px',height:'190px',scale:1},mobile:{left:'18px',top:'16vh',width:'142px',height:'142px',scale:1}},bubblePlacement:{desktop:{right:'5vw',bottom:'14vh',width:'390px'},mobile:{left:'18px',right:'18px',bottom:'104px'}},dim:.66},
  {step:2,route:'home',title:'TODAS LAS STORIES',text:'Empieza aquí. En TODAS LAS STORIES encontrarás todas las Stories publicadas.',tk:'tk_t01_p02.webm',tkLayout:'vertical',target:'.nav-btn[data-route="stories"]',tkPlacement:{desktop:{left:'7vw',bottom:'15vh',width:'150px',height:'230px',scale:1},mobile:{left:'18px',top:'15vh',width:'118px',height:'178px',scale:1}},bubblePlacement:{desktop:{right:'5vw',bottom:'14vh',width:'390px'},mobile:{left:'154px',right:'14px',bottom:'106px'}},dim:.68},
  {step:3,route:'stories',title:'PÚBLICAS Y EXCLUSIVAS',text:'Algunas Stories puedes leerlas directamente. Otras son exclusivas y aparecerán bloqueadas.',tk:'tk_t01_p03.webm',tkLayout:'horizontal',tkPlacement:{desktop:{left:'5vw',top:'18vh',width:'280px',height:'148px',scale:1},mobile:{left:'16px',top:'12vh',width:'210px',height:'112px',scale:1}},bubblePlacement:{desktop:{right:'5vw',bottom:'11vh',width:'410px'},mobile:{left:'16px',right:'16px',bottom:'96px'}},lockPlacement:{desktop:{right:'10vw',top:'20vh'},mobile:{right:'18px',top:'29vh'}},lockMode:'locked',ambientScroll:'start',dim:.56},
  {step:4,route:'stories',title:'DESBLOQUEO POR QR',text:'Cada Story exclusiva tiene su propio QR. Lo encontrarás junto a esa Story en uno de los libros físicos de Disturbing Stories. Escanéalo y quedará desbloqueada.',micro:'1 QR = 1 STORY',tk:'tk_t01_p04.webm',tkLayout:'vertical',tkPlacement:{desktop:{left:'5vw',top:'17vh',width:'155px',height:'232px',scale:1},mobile:{left:'16px',top:'11vh',width:'118px',height:'178px',scale:1}},bubblePlacement:{desktop:{right:'5vw',bottom:'9vh',width:'430px'},mobile:{left:'16px',right:'16px',bottom:'92px'}},lockPlacement:{desktop:{right:'13vw',top:'18vh'},mobile:{right:'22px',top:'28vh'}},lockMode:'unlock',ambientScroll:'continue',dim:.56},
  {step:5,route:'stories',title:'YA PUEDES EMPEZAR',text:'Eso es todo lo que necesitas para empezar.',secondary:'Puedes descubrir el resto cuando quieras.',tk:'tk_t01_p05.webm',tkLayout:'horizontal',tkPlacement:{desktop:{left:'6vw',top:'21vh',width:'300px',height:'158px',scale:1},mobile:{left:'50%',top:'12vh',width:'230px',height:'122px',scale:1,translateX:'-50%'}},bubblePlacement:{desktop:{right:'5vw',bottom:'10vh',width:'450px'},mobile:{left:'14px',right:'14px',bottom:'86px'}},ambientScroll:'finish',dim:.62}
];
...
function tutorialMaxScroll(){return Math.max(0,document.documentElement.scrollHeight-innerHeight)}
function startTutorialStoryScroll(reset=false){if(tutorialReducedMotion())return;if(tutorialReturnRaf){cancelAnimationFrame(tutorialReturnRaf);tutorialReturnRaf=0} if(reset)scrollTo({top:0,behavior:'auto'});if(tutorialScrollRaf)return;tutorialScrollLast=performance.now();tutorialScrollSpeed=innerWidth<=600?10:13;const tick=now=>{if(!tutorialActive||![3,4].includes(tutorialActive.step)||routeName()!=='stories'){tutorialScrollRaf=0;return}const dt=Math.min(.05,Math.max(0,(now-tutorialScrollLast)/1000));tutorialScrollLast=now;const max=tutorialMaxScroll();if(scrollY<max-2)scrollTo({top:Math.min(max,scrollY+tutorialScrollSpeed*dt),behavior:'auto'});tutorialScrollRaf=requestAnimationFrame(tick)};tutorialScrollRaf=requestAnimationFrame(tick)}
function animateTutorialScrollTo(target,duration=2000){if(tutorialReturnRaf)cancelAnimationFrame(tutorialReturnRaf);const start=scrollY,delta=target-start,t0=performance.now();return new Promise(resolve=>{const tick=now=>{if(!tutorialActive||tutorialActive.step!==5){tutorialReturnRaf=0;resolve();return}const p=Math.min(1,(now-t0)/duration),e=p<.5?4*p*p*p:1-Math.pow(-2*p+2,3)/2;scrollTo({top:start+delta*e,behavior:'auto'});if(p<1)tutorialReturnRaf=requestAnimationFrame(tick);else{tutorialReturnRaf=0;resolve()}};tutorialReturnRaf=requestAnimationFrame(tick)})}
function finishTutorialStoryScroll(){if(tutorialReducedMotion()){stopTutorialStoryScroll();scrollTo({top:0,behavior:'auto'});return}if(tutorialScrollRaf)cancelAnimationFrame(tutorialScrollRaf);tutorialScrollRaf=0;const start=performance.now(),v0=tutorialScrollSpeed||12,last={t:start};const brake=now=>{if(!tutorialActive||tutorialActive.step!==5)return;const p=Math.min(1,(now-start)/800),dt=Math.min(.05,(now-last.t)/1000);last.t=now;const velocity=v0*Math.pow(1-p,2);const max=tutorialMaxScroll();if(scrollY<max-1)scrollTo({top:Math.min(max,scrollY+velocity*dt),behavior:'auto'});if(p<1)requestAnimationFrame(brake);else tutorialLater(()=>{if(tutorialActive?.step===5)animateTutorialScrollTo(0,2000)},2000)};requestAnimationFrame(brake)}
...
function tutorialButtonMarkup(step){if(step===1)return `<div class="tutorial-controls tutorial-controls-first"><span></span><button type="button" class="tutorial-next" data-tutorial-next>SIGUIENTE <b>›</b></button></div>`;if(step===5)return `<div class="tutorial-controls tutorial-controls-final"><button type="button" class="tutorial-back" data-tutorial-back>‹ ATRÁS</button><div class="tutorial-final-actions"><button type="button" class="tutorial-start" data-tutorial-start>EMPEZAR</button><button type="button" class="tutorial-more" data-tutorial-more>¿QUIERES SABER MÁS?</button></div></div>`;return `<div class="tutorial-controls"><button type="button" class="tutorial-back" data-tutorial-back>‹ ATRÁS</button><button type="button" class="tutorial-next" data-tutorial-next>SIGUIENTE <b>›</b></button></div>`}
function tutorialOverlayHtml(cfg){return `<div id="tutorialOverlay" class="tutorial-overlay-v61" style="--tutorial-dim:${cfg.dim??.64}"><div class="tutorial-dim-v61"></div><div class="tutorial-focus-ring-v61 hidden" aria-hidden="true"></div><div class="tutorial-tk-stage-v61 tutorial-tk-${cfg.tkLayout||'square'}"><div class="tutorial-tk-card"><video src="./assets/tutorials/tk/${escAttr(cfg.tk)}?v=${encodeURIComponent(APP_VERSION)}" autoplay muted loop playsinline preload="auto" aria-label="T.K." onerror="this.classList.add('missing')"></video><div class="tutorial-tk-fallback" aria-hidden="true"><span>TK</span></div></div></div>${tutorialLockMarkup(cfg.lockMode)}<section class="tutorial-bubble-v61" role="dialog" aria-modal="true" aria-label="Tutorial · ${escAttr(cfg.title)}"><button type="button" class="tutorial-close-v61" data-tutorial-close aria-label="Cerrar tutorial">×</button><div class="tutorial-step-label"><i aria-hidden="true"></i>PASO ${cfg.step}/5</div><h2>${escapeHtml(cfg.title)}</h2><p class="tutorial-main-text">${escapeHtml(cfg.text)}</p>${cfg.secondary?`<p class="tutorial-secondary-text">${escapeHtml(cfg.secondary)}</p>`:''}${cfg.micro?`<div class="tutorial-micro-rule">${escapeHtml(cfg.micro)}</div>`:''}${tutorialButtonMarkup(cfg.step)}</section></div>`}
function positionTutorialFocus(targetSelector){const ring=$('.tutorial-focus-ring-v61');if(!ring)return;if(!targetSelector){ring.classList.add('hidden');return}const target=document.querySelector(targetSelector);if(!target){ring.classList.add('hidden');return}const r=target.getBoundingClientRect(),pad=7;ring.classList.remove('hidden');ring.style.left=`${Math.max(5,r.left-pad)}px`;ring.style.top=`${Math.max(5,r.top-pad)}px`;ring.style.width=`${Math.min(innerWidth-10,r.width+pad*2)}px`;ring.style.height=`${r.height+pad*2}px`;ring.style.borderRadius=(parseFloat(getComputedStyle(target).borderRadius)||10)+pad+'px'}
function positionTutorialLock(cfg){const lock=$('.tutorial-lock-stage');if(!lock)return;applyTutorialPlacement(lock,cfg.lockPlacement);const tk=$('.tutorial-tk-stage-v61');if(tk){const r=tk.getBoundingClientRect(),size=Math.max(112,Math.min(220,Math.min(r.width||150,r.height||150)));lock.style.setProperty('--tutorial-lock-size',`${size}px`)}}
function placeTutorialElements(cfg){applyTutorialPlacement($('.tutorial-tk-stage-v61'),cfg.tkPlacement);applyTutorialPlacement($('.tutorial-bubble-v61'),cfg.bubblePlacement);positionTutorialFocus(cfg.target);positionTutorialLock(cfg)}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=6.1.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
