...
function showInstallGuide(kind='ios'){
  ...
  const steps=ios
    ...
    : android
      ? [`<div><b>1</b><span>Pulsa <strong>INSTALAR AHORA</strong> si aparece el cuadro del navegador.</span></div>`,`<div><b>2</b><span>Si no aparece, abre el menú <strong>⋮</strong>.</span></div>`,`<div><b>3</b><span>Elige <strong>Instalar app</strong> o <strong>Añadir a pantalla de inicio</strong>.</span></div>`,`<div><b>4</b><span>Acepta y abre Disturbing Stories App desde el nuevo icono.</span></div>`]
      : mac
        ? [`<div><b>1</b><span>Abre el menú del navegador.</span></div>`,`<div><b>2</b><span>Busca <strong>Instalar app</strong> o <strong>Crear acceso directo</strong>.</span></div>`,`<div><b>3</b><span>Confirma la instalación o la creación del acceso.</span></div>`,`<div><b>4</b><span>Abre Disturbing Stories App desde su icono o desde el Dock.</span></div>`]
        : windows
          ? [`<div><b>1</b><span>Pulsa <strong>INSTALAR AHORA</strong> si tu navegador muestra esa opción.</span></div>`,`<div><b>2</b><span>Si no aparece, abre el menú del navegador.</span></div>`,`<div><b>3</b><span>Selecciona <strong>Instalar app</strong>.</span></div>`,`<div><b>4</b><span>La app quedará instalada con su propia ventana e icono.</span></div>`]
          : [`<div><b>1</b><span>Abre el menú principal de tu navegador.</span></div>`,`<div><b>2</b><span>Busca <strong>Instalar app</strong> o <strong>Añadir a pantalla de inicio</strong>.</span></div>`,`<div><b>3</b><span>Confirma la instalación.</span></div>`,`<div><b>4</b><span>Después abre Disturbing Stories App desde su icono.</span></div>`];
  const summary=ios
    ? ['Usa Safari.', 'Pulsa Compartir.', 'Toca “Añadir a pantalla de inicio”.', 'Confirma con Añadir.']
    : android
      ? ['Si aparece, usa “Instalar ahora”.', 'Si no, abre el menú del navegador.', 'Toca “Instalar app” o “Añadir a pantalla de inicio”.', 'Acepta y abre la app desde el icono.']
      : mac
        ? ['Abre el menú del navegador.', 'Busca “Instalar app” o una opción equivalente.', 'Confirma la instalación.', 'Abre la app desde su acceso directo.']
        : windows
          ? ['Pulsa “Instalar ahora” si aparece.', 'Si no aparece, abre el menú del navegador.', 'Selecciona “Instalar app”.', 'Abre Disturbing Stories App desde su nuevo icono.']
          : ['Abre el menú del navegador.', 'Busca la opción de instalar.', 'Confirma.', 'Abre la app desde el nuevo icono.'];
  const helper=ios
    ? 'En dispositivos Apple la instalación se hace desde el menú Compartir. Solo tendrás que hacerlo una vez.'
    : canPrompt
...
function storyFeatures(s){
  ...
  const my=s?.mediaYoutube||{},md=s?.mediaDurations||{},sources=s?.mediaDurationSources||{};
  if((Array.isArray(my.audio)&&my.audio.length)||Number(md.audio)>0||String(sources.audio||'').toLowerCase()==='youtube')set.add('audio');
  if((Array.isArray(my.video)&&my.video.length)||Number(md.video)>0||String(sources.video||'').toLowerCase()==='youtube')set.add('video');
...
function renderHome(){
  ...
  const installPanelBtn=$('#homeInstallPanel');if(installPanelBtn)installPanelBtn.onclick=()=>showInstallGuide(detectInstallGuideKind());
...
function sagaCard(saga){const d=sagaProgressData(saga),banner=sagaListBanner(saga);return `<button class="saga-card" data-saga-id="${escAttr(saga.id)}">${banner?`<img src="${escAttr(banner)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'<div class="saga-banner-placeholder">SAGA</div>'}<div class="saga-card-copy"><span>${d.read} LEÍDAS · ${d.published}/${d.total} PUBLICADAS</span><h2>${escapeHtml(saga.name||'Saga')}</h2>${sagaProgressBars(d)}<strong><em class="read">${d.readPct}% LEÍDA</em> · <em class="published">${d.publishedPct}% PUBLICADA</em></strong></div></button>`}
...
function renderMediaCollection(type){currentStory=null;const isAudio=type==='audio',items=(indexData.stories||[]).filter(s=>storyHasFeature(s,type)),durations=items.map(s=>mediaDuration(s,type)).filter(x=>x>0),total=durations.reduce((a,b)=>a+b,0);view.innerHTML=`<section class="section v2-section media-collection ${isAudio?'cassettes':'tapes'}"><div class="media-section-head"><div class="media-title-no-icon"><div><h1>${isAudio?'Disturbing Cassettes':'Disturbing Tapes'}</h1></div></div><div class="media-summary"><div><span>TOTAL</span><strong>${items.length}</strong><em>${isAudio?'CASSETTES':'TAPES'}</em></div><div><span>DURACIÓN TOTAL</span><strong>${total?fmtTotalDuration(total):'PENDIENTE'}</strong></div></div></div><div class="media-catalog">${items.length?items.map(s=>mediaCollectionItem(s,type)).join(''):'<div class="empty">No hay contenido disponible en esta sección.</div>'}</div></section>`;$$('[data-media-story]').forEach(el=>el.onclick=()=>navigateHash(`#/story/${encodeURIComponent(el.dataset.mediaStory)}/${encodeURIComponent(el.dataset.mediaTab)}`));bindBrokenImages()}
function mediaCollectionItem(s,type){const cover=displayCover(s),d=mediaDuration(s,type),read=isRead(s.id);return `<article class="media-collection-card ${isLocked(s)?'is-locked':''} ${read?'is-read':''}" data-media-story="${escAttr(s.id)}" data-media-tab="${escAttr(type)}">${cover?`<div class="media-collection-poster"><img src="${escAttr(cover)}" alt="">${readCornerBadge(s)}</div>`:''}<div class="media-collection-copy"><span>STORY ${escapeHtml(s.id)}</span><strong>${escapeHtml(s.title||'')}</strong><div class="media-duration-row"><em>${d?fmtLong(d):'DURACIÓN PENDIENTE'}</em>${read?'<b class="media-read-bubble">LEÍDA</b>':''}</div></div><div class="media-collection-icons">${mediaCollectionIcons(s)}</div></article>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=3.0.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
