...
const TUTORIAL1_STEPS=[
  {step:1,route:'home',title:'BIENVENID@',text:'Bienvenid@ a Disturbing Stories App. Aquí podrás leer, explorar y descubrir todo este universo de relatos cortos de terror y ciencia ficción.',tk:'tk_t01_p01.webm',tkLayout:'square',composition:'center-tk-text',centerOffsetY:{desktop:-116,mobile:-90},tkPlacement:{desktop:{width:'380px',height:'380px'},mobile:{width:'284px',height:'284px'}},bubblePlacement:{desktop:{width:'390px'},mobile:{width:'calc(100vw - 28px)'}},dim:.66},
  {step:2,route:'home',title:'TODAS LAS STORIES',text:'Empieza aquí. En TODAS LAS STORIES encontrarás todas las Stories publicadas.',tk:'tk_t01_p02.webm',tkLayout:'vertical',target:'.nav-btn[data-route="stories"]',focusTarget:'.nav-btn[data-route="stories"] .nav-icon',liftTargetSelector:'.nav-btn[data-route="stories"] .nav-icon',composition:'target-stack',targetGap:27,focusStrong:true,focusDelay:2000,liftTarget:true,tkPlacement:{desktop:{width:'300px',height:'460px'},mobile:{width:'236px',height:'356px'}},bubblePlacement:{desktop:{width:'390px'},mobile:{width:'calc(100vw - 28px)'}},dim:.70},
  ...
  {step:4,route:'stories',title:'DESBLOQUEO POR QR',text:'Cada Story exclusiva tiene su propio QR. Lo encontrarás junto a esa Story en uno de los libros físicos de Disturbing Stories. Escanéalo y quedará desbloqueada.',micro:'1 QR = 1 STORY',tk:'tk_t01_p04.webm',tkLayout:'square',composition:'center-stack-lock',tkPlacement:{desktop:{width:'240px',height:'240px'},mobile:{width:'190px',height:'190px'}},bubblePlacement:{desktop:{width:'430px'},mobile:{width:'calc(100vw - 32px)'}},lockMode:'unlock',unlockDelay:3700,ambientScroll:'continue',dim:.54},
  {step:5,route:'stories',title:'YA PUEDES EMPEZAR',text:'Eso es todo lo que necesitas para empezar.',secondary:'Puedes descubrir el resto cuando quieras.',tk:'tk_t01_p05.webm',tkLayout:'horizontal',composition:'center-tk-text-final',tkPlacement:{desktop:{width:'450px',height:'238px'},mobile:{width:'calc(100vw - 28px)',height:'190px'}},bubblePlacement:{desktop:{width:'450px'},mobile:{width:'calc(100vw - 28px)'}},ambientScroll:'finish',dim:.62}
];
...
function finishTutorial1(action='start'){if(!tutorialActive)return;markTutorialViewed('tutorial1');tutorialActive=null;clearTutorialLockBridge();destroyTutorialOverlay();if(action==='more'){if(currentHash()!=='#/tutorials')navigateHash('#/tutorials');else renderTutorials();return}if(action==='start'){tutorialSetScroll(0);if(currentHash()==='#/stories'){routeFromHash();requestAnimationFrame(()=>scrollTo({top:0,behavior:'auto'}))}else navigateHash('#/stories')}}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=6.5.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
