...
function storyManualReadButtonHtml(s,read=false){return `<div class="story-manual-read-row"><button id="manualMarkRead" class="story-manual-read-btn ${read?'is-read':''}" type="button" ${read?'disabled':''}>${read?'✓ LEÍDA':'MARCAR COMO LEÍDA'}</button></div>`}
function bindManualReadButton(id){const b=$('#manualMarkRead');if(!b)return;b.onclick=()=>{if(markRead(id)){b.textContent='✓ LEÍDA';b.classList.add('is-read');b.disabled=true}}}
...
function sagaStoryLink(s){const m=sagaMembership(s?.id);return m?`<button class="story-saga-link" type="button" data-saga-link="${escAttr(m.saga.id)}">PERTENECE A: ${escapeHtml(m.saga.name)} · PARTE ${m.part} DE ${(m.saga.parts||[]).length}</button>`:''}
...
function sagaProgressData(saga){const parts=Array.isArray(saga?.parts)?saga.parts:[],total=parts.length,publishedStories=parts.map(p=>p.storyId?indexData.stories.find(s=>s.id===String(p.storyId)):null).filter(Boolean),published=publishedStories.length,read=publishedStories.filter(s=>isRead(s.id)).length;return{total,published,read,publishedPct:total?Math.round(published/total*100):0,readPct:total?Math.round(read/total*100):0}}
...
/* ===== v4.6 · USER / AVATARES / LOGROS / SINCRONIZACIÓN AUTOMÁTICA ===== */
...
function evaluateAchievements(showReward=false){const got=earnedAchievementIds(),newOnes=[];for(const a of achievementRows()){const id=String(a?.id||'');if(!id||got.has(id)||!achievementRequirementMet(a))continue;got.add(id);newOnes.push(a)}if(newOnes.length){store.set(achievementEarnedKey,[...got]);queueUserVaultSync();if(showReward)queueAchievementRewards(newOnes);if(routeName()==='user')renderUser()}return got}
...
function earnedAchievementsLatestFirst(){const ids=(store.get(achievementEarnedKey,[])||[]).map(String),pos=new Map(ids.map((id,i)=>[id,i]));return earnedAchievements().sort((a,b)=>(pos.get(String(b.id))??-1)-(pos.get(String(a.id))??-1)||(Number(b.number)||0)-(Number(a.number)||0))}
function achievementCardHtml(a){const p=achievementRequirementProgress(a);return `<article class="achievement-earned-card"><span class="achievement-medal" aria-hidden="true">★</span><div><strong>${escapeHtml(a.name||'LOGRO')}</strong>${a.description||p.label?`<small>${escapeHtml(a.description||p.label)}</small>`:''}</div></article>`}
function achievementsPanelHtml(){const rows=earnedAchievementsLatestFirst();return `<div class="user-achievements-section"><div class="user-box-title">LOGROS CONSEGUIDOS</div><div id="achievementEarnedList" class="achievement-earned-list">${rows.length?rows.map(achievementCardHtml).join(''):'<p class="achievement-empty">Todavía no has conseguido ningún logro.</p>'}</div>${rows.length>1?`<button id="achievementListToggle" class="achievement-list-toggle hidden" type="button" aria-expanded="false">VER TODOS LOS LOGROS</button>`:''}</div>`}
function setupAchievementCollapser(){const list=$('#achievementEarnedList'),toggle=$('#achievementListToggle');if(!list||!toggle)return;const cards=[...list.querySelectorAll('.achievement-earned-card')];if(cards.length<2)return;requestAnimationFrame(()=>{cards.forEach(c=>c.classList.remove('achievement-row-hidden'));const firstTop=cards[0]?.offsetTop??0,extra=cards.filter(c=>c.offsetTop>firstTop+2);if(!extra.length){toggle.classList.add('hidden');return}extra.forEach(c=>c.classList.add('achievement-row-hidden'));toggle.classList.remove('hidden');toggle.dataset.collapsed='1';toggle.textContent=`VER TODOS LOS LOGROS · ${cards.length}`;toggle.onclick=()=>{const collapsed=toggle.dataset.collapsed==='1';extra.forEach(c=>c.classList.toggle('achievement-row-hidden',!collapsed));toggle.dataset.collapsed=collapsed?'0':'1';toggle.setAttribute('aria-expanded',String(collapsed));toggle.textContent=collapsed?'OCULTAR LOGROS':`VER TODOS LOS LOGROS · ${cards.length}`}})}
...
function updateUserChrome(){const name=registeredDisplayName(),lab=$('#userMenuLabel'),hello=$('#userGreeting'),head=$('#userHeaderAvatar'),headImg=$('#userHeaderAvatarImg'),drawerIcon=$('#userMenuIcon'),drawerImg=$('#userMenuAvatarImg'),a=avatarById(selectedAvatarId());if(lab)lab.textContent=registeredUser?(name||'Usuario Registrado'):'Usuario No Registrado';const show=!!registeredUser&&!!name;if(hello){hello.textContent=show?`Hola ${name}`:'';hello.classList.toggle('hidden',!show);bindUserHeaderShortcut(hello)}if(head){head.classList.toggle('hidden',!show);bindUserHeaderShortcut(head);if(headImg&&show&&a){headImg.src=avatarImageUrl(a);headImg.alt=avatarDisplayName(a)||name||'Avatar'}}if(drawerIcon){const useAvatar=!!registeredUser&&!!a;drawerIcon.classList.toggle('has-avatar',useAvatar);if(drawerImg&&useAvatar){drawerImg.src=avatarImageUrl(a);drawerImg.alt=avatarDisplayName(a)||name||'Avatar'}}}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.6.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
