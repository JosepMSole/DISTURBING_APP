...
function stopPlayerArtwork(){clearInterval(playerArtworkTimer);playerArtworkTimer=0;playerArtworkTrack='';playerArtworkIndex=0}
...
function activatePlayerArtwork(index,initial=false){const box=$('#playerArtwork');if(!box)return;const slides=$$('.player-artwork-slide',box);if(!slides.length)return;playerArtworkIndex=((index%slides.length)+slides.length)%slides.length;slides.forEach((el,i)=>el.classList.toggle('active',i===playerArtworkIndex));box.classList.toggle('single',slides.length===1);if(initial)requestAnimationFrame(()=>slides[playerArtworkIndex]?.classList.add('active'))}
...
function startPlayerArtworkForCurrent(){clearInterval(playerArtworkTimer);playerArtworkTimer=0;const box=$('#playerArtwork'),track=playerTracks[playerIndex];if(!box||!track)return;const images=playerTrackImages(track),key=String(track.file||'');playerArtworkTrack=key;playerArtworkIndex=0;if(!images.length){box.innerHTML='';box.classList.add('empty');return}box.classList.remove('empty');box.innerHTML=images.map((src,i)=>`<div class="player-artwork-slide ${i===0?'active':''}"><img src="${escAttr(src)}" alt="" referrerpolicy="no-referrer"></div>`).join('')+'<span class="player-artwork-signal" aria-hidden="true"></span>';$$('.player-artwork-slide img',box).forEach(playerArtworkFit);if(images.length>1)playerArtworkTimer=setInterval(()=>{if(routeName()!=='player'||playerArtworkTrack!==String(playerTracks[playerIndex]?.file||'')){clearInterval(playerArtworkTimer);playerArtworkTimer=0;return}activatePlayerArtwork(playerArtworkIndex+1)},15000)}
...
function storyMediaIdentityForSource(mediaType,src='',fallbackLocale='es'){const mediaId=youtubeMediaId(src),fallback=['es','en','ca'].includes(fallbackLocale)?fallbackLocale:'es';if(!mediaId)return{locale:fallback,mediaId:''};const meta=currentStory||{},tag=(Array.isArray(meta?.mediaAssetTags)?meta.mediaAssetTags:[]).find(t=>String(t?.mediaId||'')===mediaId&&String(t?.kind||'')===mediaType);if(tag&&['es','en','ca'].includes(String(tag.locale||'')))return{locale:String(tag.locale),mediaId};const sets={es:Array.isArray(meta?.mediaYoutube?.[mediaType])?meta.mediaYoutube[mediaType]:[],en:Array.isArray(meta?.mediaYoutubeLocales?.[mediaType]?.en)?meta.mediaYoutubeLocales[mediaType].en:[],ca:Array.isArray(meta?.mediaYoutubeLocales?.[mediaType]?.ca)?meta.mediaYoutubeLocales[mediaType].ca:[]},order=[fallback,...['es','en','ca'].filter(x=>x!==fallback)];for(const locale of order)if((sets[locale]||[]).some(x=>mediaEntryId(x)===mediaId))return{locale,mediaId};return{locale:fallback,mediaId}}
...
function sagaCard(saga){const d=sagaProgressData(saga),banner=sagaListBanner(saga);return `<button class="saga-card" data-saga-id="${escAttr(saga.id)}">${banner?`<img src="${escAttr(banner)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'<div class="saga-banner-placeholder">SAGA</div>'}${sagaStatusBubble(d)}<div class="saga-card-copy"><span>${d.read} ${d.read===1?'LEÍDA':'LEÍDAS'} · ${d.published}/${d.total} PUBLICADAS</span><h2>${escapeHtml(saga.name||'Saga')}</h2>${sagaProgressBars(d)}<strong><em class="read">${d.readPct}% LEÍDA</em> · <em class="published">${d.publishedPct}% PUBLICADA</em></strong></div></button>`}
...
function animateRegisteredUserHero(){
  ...
  hero.classList.add('user-hero-v422');
  ...
  if(reduced){if(levelNumber)levelNumber.textContent=String(target);hero.classList.add('user-hero-v422-complete');return}
  ...
  typeTimed(name,rows.find(r=>r.el===name)?.text||'',240,350);
  typeTimed(status,rows.find(r=>r.el===status)?.text||'',520,250);
  typeTimed(email,rows.find(r=>r.el===email)?.text||'',745,330);
  setTimeout(()=>{if(!levelNumber)return;if(target<=0){levelNumber.textContent=String(target);return}const t0=performance.now(),dur=520,frame=now=>{const p=Math.min(1,(now-t0)/dur),e=1-Math.pow(1-p,3);levelNumber.textContent=String(Math.min(target,Math.round(target*e)));if(p<1)requestAnimationFrame(frame);else levelNumber.textContent=String(target)};requestAnimationFrame(frame)},620);
  setTimeout(()=>{if(edit){edit.animate?.([{opacity:0,transform:'translateY(7px)'},{opacity:1,transform:'translateY(0)'}],{duration:270,easing:'ease-out',fill:'both'});edit.style.opacity='1';edit.style.transform='none'}},1320);
  // Hard final state: the whole composition is complete before the 2-second limit.
  setTimeout(()=>hero.classList.add('user-hero-v422-complete'),1720);
}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.22.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
