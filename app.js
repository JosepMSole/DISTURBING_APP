...
function bindReaderMedia(){bindBrokenImages();bindStoryMediaPause();const mediaType=storyMediaTypeFromCurrentTab(),mediaLocale=mediaType?currentStoryMediaLocale(mediaType):'es';$$('.reader-image:not(.linked) img').forEach(img=>img.onclick=()=>openLightbox(img.src,img.alt));$$('#reader audio,#reader video').forEach(m=>m.addEventListener('play',()=>{if(mediaType)markStoryMediaConsumed(mediaType,currentStory?.id,mediaLocale)},{once:true}));$$('#reader .reader-media iframe').forEach(m=>m.addEventListener('pointerdown',()=>{if(mediaType)markStoryMediaConsumed(mediaType,currentStory?.id,mediaLocale)},{once:true,passive:true}))}
...
function consumedMediaMap(){const m=store.get(consumedMediaKey,{audio:[],video:[],audioLocales:{es:[],en:[],ca:[]},videoLocales:{es:[],en:[],ca:[]}})||{};m.audio=Array.isArray(m.audio)?m.audio:[];m.video=Array.isArray(m.video)?m.video:[];for(const type of ['audio','video']){const k=`${type}Locales`;m[k]=m[k]&&typeof m[k]==='object'?m[k]:{};for(const locale of ['es','en','ca'])m[k][locale]=Array.isArray(m[k][locale])?m[k][locale]:[]}return m}
function mediaConsumed(type,storyId,locale=''){const m=consumedMediaMap(),id=String(storyId),base=Array.isArray(m?.[type])?m[type].map(String):[];if(!locale)return base.includes(id)||['es','en','ca'].some(l=>(m?.[`${type}Locales`]?.[l]||[]).map(String).includes(id));const l=['es','en','ca'].includes(locale)?locale:'es',rows=(m?.[`${type}Locales`]?.[l]||[]).map(String);return rows.includes(id)||(l==='es'&&base.includes(id))}
function markStoryMediaConsumed(type,storyId,locale='es'){if(!['audio','video'].includes(type)||!storyId)return false;locale=['es','en','ca'].includes(locale)?locale:'es';const m=consumedMediaMap(),id=String(storyId),lk=`${type}Locales`,localeRows=m[lk][locale].map(String);let changed=false;if(!localeRows.includes(id)){m[lk][locale].push(id);changed=true}if(locale==='es'&&!m[type].map(String).includes(id)){m[type].push(id);changed=true}if(!changed)return false;store.set(consumedMediaKey,m);evaluateAchievements(true);queueUserVaultSync();return true}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.10.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
