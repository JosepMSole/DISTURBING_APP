...
/* ===== v4.3 · USER / AVATARES / LOGROS / SINCRONIZACIÓN AUTOMÁTICA ===== */
...
function updateUserChrome(){const name=registeredDisplayName(),lab=$('#userMenuLabel'),hello=$('#userGreeting'),head=$('#userHeaderAvatar'),headImg=$('#userHeaderAvatarImg'),a=avatarById(selectedAvatarId());if(lab)lab.textContent=registeredUser?(name||'Usuario Registrado'):'Usuario No Registrado';const show=!!registeredUser&&!!name;if(hello){hello.textContent=show?`Hola ${name}`:'';hello.classList.toggle('hidden',!show)}if(head){head.classList.toggle('hidden',!show);if(headImg&&show&&a){headImg.src=avatarImageUrl(a);headImg.alt=avatarDisplayName(a)||name||'Avatar'}}}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.3.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
