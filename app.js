...
const UI_SOUND_FILES={open:'open.mp3',back:'back.mp3',tap:'tap.mp3',expand:'expand.mp3',collapse:'collapse.mp3',confirm:'confirm.mp3',blocked:'blocked.mp3'};
const UI_SOUND_VOLUME={open:.32,back:.32,tap:.24,expand:.28,collapse:.28,confirm:.34,blocked:.31};
...
function uiSoundForControl(el){
  ...
  if(['miniPlayerOpen','userMenuBtn','scanBtn','brandBtn','openLatest','openAllStories','continueReading','gamePlayBtn'].includes(id))return'open';
...
function uiSoundPreferenceHtml(){const on=uiSoundsEnabled();return `<div class="user-sounds-panel"><div><strong>SONIDOS DE INTERFAZ</strong><small>Sonidos breves para abrir, volver, desplegar y confirmar acciones.</small></div><button id="userUiSoundToggle" class="user-sounds-toggle ${on?'is-on':'is-off'}" type="button" data-ui-sound="none" aria-pressed="${on?'true':'false'}"><span>${on?'ON':'OFF'}</span></button></div>`}
...
async function boot(){
  ...
  try{
    ...
    initialiseNewState();
    initUserSystem();
    bindShell();bindUiSoundSystem();initMotionSystem();initPlayer();
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=5.17.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
