...
async function boot(){
  ...
  try{
    ...
    [sagasData,timelineData,extrasData,playerCatalogData,homeData,avatarsData,userConfigData,microsData,gamesData]=await Promise.all([
      ...
      loadOptionalJson('./data/games.json',{schema:1,games:[]})
    ]);
    initialiseNewState();
    initUserSystem();
...
function gameCompatibilityLabel(v){return({mobile_desktop:'MOBILE + DESKTOP',desktop_recommended:'DESKTOP RECOMENDADO',desktop_only:'SOLO DESKTOP'})[String(v||'')]||'MOBILE + DESKTOP'}
...
function gameCardHtml(g){const cover=gameAssetUrl(g.cover),st=gameStory(g);return `<button class="game-card" type="button" data-game-id="${escAttr(g.id)}">${cover?`<img src="${escAttr(cover)}" alt="${escAttr(g.title||'Game')}" loading="lazy" referrerpolicy="no-referrer">`:'<div class="game-card-placeholder">GAME</div>'}<div class="game-card-copy"><span>${escapeHtml(gameTypeLabel(g.type))}</span><h2>${escapeHtml(g.title||g.id)}</h2><small>${escapeHtml(gameCompatibilityLabel(g.compatibility))}</small>${st?`<em>STORY ${escapeHtml(st.id)} · ${escapeHtml(st.title||'')}</em>`:''}</div></button>`}
function bindGameCards(){$$('[data-game-id]').forEach(b=>b.onclick=()=>go(`game/${encodeURIComponent(b.dataset.gameId)}`))}
function renderGamesCatalog(){currentStory=null;currentMicro=null;currentGame=null;if(!featurePortalEnabled('games'))return renderFuturePlaceholder('GAMES','rosa');const rows=[...(gamesData?.games||[])].filter(g=>g&&g.published!==false).sort((a,b)=>new Date(b.publicationDate||0)-new Date(a.publicationDate||0)||String(a.title||'').localeCompare(String(b.title||'')));view.innerHTML=`<section class="section games-section"><div class="games-hero"><div>${animatedFeatureIcon('game','portal-custom-svg gamepad-portal-svg')}</div><span>JUEGA EN EL UNIVERSO DE</span><h1>GAMES</h1><p>Minijuegos de Disturbing Stories. Algunos están diseñados especialmente para ordenador; consulta la compatibilidad antes de jugar.</p></div><div class="games-summary"><strong>${rows.length}</strong><span>${rows.length===1?'GAME DISPONIBLE':'GAMES DISPONIBLES'}</span></div><div class="games-grid">${rows.length?rows.map(gameCardHtml).join(''):'<div class="empty">Todavía no hay Games publicados.</div>'}</div></section>`;bindGameCards();bindBrokenImages()}
...
function renderGameDetail(id){currentStory=null;currentMicro=null;const g=gameById(id);currentGame=g;if(!g||g.published===false){view.innerHTML='<div class="empty">Game no encontrado.</div>';return}const cover=gameAssetUrl(g.cover),compat=gameCompatibilityLabel(g.compatibility),orientation=gameOrientationLabel(g.orientation);view.innerHTML=`<section class="section game-detail"><div class="game-detail-head">${cover?`<img src="${escAttr(cover)}" alt="${escAttr(g.title||'Game')}" class="game-detail-cover">`:''}<div class="game-detail-copy"><span>${escapeHtml(gameTypeLabel(g.type))}</span><h1>${escapeHtml(g.title||g.id)}</h1>${g.publicationDate?`<small>${formatDate(g.publicationDate)}</small>`:''}<div class="game-badges"><b>${escapeHtml(compat)}</b><b>${escapeHtml(orientation)}</b></div></div></div>${g.description?`<p class="game-description">${escapeHtml(g.description)}</p>`:''}${g.instructions?`<div class="game-instructions"><strong>INSTRUCCIONES</strong><p>${escapeHtml(g.instructions).replace(/\n/g,'<br>')}</p></div>`:''}<div class="game-actions"><button id="gamePlayBtn" class="game-play-btn" type="button">JUGAR</button></div>${gameStoryLinkHtml(g)}</section>`;$('#gamePlayBtn').onclick=()=>go(`game/${encodeURIComponent(g.id)}/play`);$$('[data-game-story]').forEach(b=>b.onclick=()=>go(`story/${encodeURIComponent(b.dataset.gameStory)}`));bindBrokenImages()}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=5.8.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
