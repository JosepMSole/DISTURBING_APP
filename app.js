...
function stopPlayerArtwork(){clearTimeout(playerArtworkTimer);playerArtworkTimer=0;playerArtworkTrack='';playerArtworkIndex=0}
function playerArtworkFit(img){if(!img)return;const apply=()=>{const box=img.closest('.player-artwork-bg');if(!box||!img.naturalWidth||!img.naturalHeight)return;const br=box.getBoundingClientRect(),imageRatio=img.naturalWidth/img.naturalHeight,boxRatio=Math.max(.01,br.width/br.height);img.classList.toggle('pan-x',imageRatio>boxRatio*1.08);img.classList.toggle('pan-y',imageRatio<boxRatio*.92);img.classList.toggle('pan-soft',imageRatio>=boxRatio*.92&&imageRatio<=boxRatio*1.08)};if(img.complete)apply();else img.addEventListener('load',apply,{once:true})}
function activatePlayerArtwork(index,initial=false){const box=$('#playerArtwork');if(!box)return;const slides=$$('.player-artwork-slide',box);if(!slides.length)return;const previous=slides[playerArtworkIndex];playerArtworkIndex=((index%slides.length)+slides.length)%slides.length;const next=slides[playerArtworkIndex];slides.forEach(el=>{if(el!==next)el.classList.remove('active')});if(previous&&previous!==next)previous.classList.add('leaving');next.classList.remove('leaving');next.classList.add('active');box.classList.toggle('single',slides.length===1);const img=next.querySelector('img');if(img&&slides.length>1){img.classList.remove('motion-run');void img.offsetWidth;img.classList.add('motion-run')}if(previous&&previous!==next)setTimeout(()=>previous.classList.remove('leaving'),5100);if(initial)requestAnimationFrame(()=>next.classList.add('active'))}
function cleanupPlayerVolumeArtifacts(){const box=$('.player-console');if(!box)return;$$('.player-console .player-volume,.player-console .player-volume-label,.player-console .player-volume-control:not(.ds-volume-control-v422),.player-console .player-volume-track,.player-console .player-volume-thumb:not(.ds-volume-thumb-v422),.player-console .player-mobile-volume-note,.player-console .player-volume-note,.player-console [data-player-volume-note],.player-console .volume-output,.player-console output').forEach(n=>n.remove())}
...
async function renderPlayer(){currentStory=null;scrollTo({top:0,behavior:'auto'});view.innerHTML='<section class="section player-section"><div class="player-title-row"><div><div class="eyebrow">Música del proyecto</div><h1>PLAYER</h1></div><div class="player-summary"><div class="player-summary-row"><span>PISTAS</span><strong>—</strong></div><div class="player-summary-row"><small>TOTAL</small><b>—</b></div></div></div><div class="player-loading">Cargando playlist…</div></section>';try{await ensurePlayerManifest()}catch(e){view.innerHTML=`<section class="section player-section"><h1>PLAYER</h1><div class="empty">No se pudo cargar la playlist.<br>${escapeHtml(e.message)}</div></section>`;return}const a=$('#globalPlayerAudio'),t=playerTracks[playerIndex];const waveBars=Array.from({length:46},(_,i)=>`<i style="--wave-i:${i}"></i>`).join('');const playlistHtml=playerTracks.map((x,i)=>`<button class="player-track ${i===playerIndex?'active':''}" data-player-track="${i}"><span>${String(i+1).padStart(2,'0')}</span><strong>${escapeHtml(x.name)}</strong><em>${playerSavedDuration(x.file)?fmtPlayer(playerSavedDuration(x.file)):'—'}</em></button>`).join('');view.innerHTML=`<section class="section player-section"><div class="player-title-row"><div><div class="eyebrow">Música del proyecto</div><h1>PLAYER</h1></div><div class="player-summary"><div class="player-summary-row"><span>PISTAS</span><strong>${playerTracks.length}</strong></div><div class="player-summary-row"><small>TOTAL</small><b id="playerTotalDuration">…</b></div></div></div><div class="player-console"><div id="playerArtwork" class="player-artwork-bg" aria-hidden="true"></div><div class="player-console-head"><span>NOW PLAYING</span><strong id="playerNowTrack">${escapeHtml(t?.name||'—')}</strong></div><div class="player-progress-meta"><div class="player-time" id="playerTime">${fmtPlayer(a.currentTime)} / ${fmtPlayer(a.duration)}</div><div id="playerWave" class="player-wave" aria-hidden="true">${waveBars}</div></div><button class="player-progress" id="playerProgress" aria-label="Progreso"><span id="playerProgressFill"></span></button><div class="player-controls player-controls-main"><button id="playerPrev" aria-label="Anterior"><svg class="player-skip-svg" viewBox="0 0 24 24" aria-hidden="true"><path d="M14.5 5.5 8 12l6.5 6.5"/></svg></button><button id="playerPlay" class="player-main-play ${a.paused?'is-play':'is-pause'}" aria-label="Play/Pause">${a.paused?'▶':'❚❚'}</button><button id="playerNext" aria-label="Siguiente"><svg class="player-skip-svg" viewBox="0 0 24 24" aria-hidden="true"><path d="m9.5 5.5 6.5 6.5-6.5 6.5"/></svg></button></div><div class="player-mode-controls"><button id="playerRepeatOne" class="player-mode ${playerRepeatMode==='one'?'active':''}" type="button"><span class="player-mode-icon" aria-hidden="true">↻</span><span>1</span></button><button id="playerRepeatAll" class="player-mode ${playerRepeatMode==='all'?'active':''}" type="button"><span class="player-mode-icon" aria-hidden="true">↻</span><span>TODOS</span></button><button id="playerShuffle" class="player-mode ${playerShuffle?'active':''}" type="button"><span class="player-mode-icon" aria-hidden="true">⤨</span><span>RANDOM</span></button></div><div class="ds-volume-v422"><div class="ds-volume-label-v422">VOL</div><div id="playerVolumeControl" class="ds-volume-control-v422" style="--volume-pct:${Math.round(a.volume*100)}%" role="slider" tabindex="0" aria-label="Volumen" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${Math.round(a.volume*100)}"><div class="ds-volume-thumb-v422" aria-hidden="true"></div></div></div></div>${spotifyCompanionHtml()}<button id="playerPlaylistToggle" class="player-playlist-toggle" type="button" aria-expanded="false"><span class="player-playlist-mark" aria-hidden="true"><svg class="player-playlist-icon" viewBox="0 0 24 24"><circle cx="5" cy="7" r="1.2"/><circle cx="5" cy="12" r="1.2"/><circle cx="5" cy="17" r="1.2"/><path d="M9 7h10M9 12h10M9 17h10"/></svg></span><span><strong>PLAYLIST</strong><small>Pulsa aquí para abrir</small></span><b class="player-playlist-chevron" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="m6 9 6 6 6-6"/></svg></b></button><div id="playerPlaylistPanel" class="player-playlist-panel hidden"><div class="player-list">${playlistHtml}</div><p class="player-note">La música continúa al navegar por la app. Si reproduces un audio o vídeo dentro de una Story, Player se pausa.</p></div></section>`;$('#playerPrev').onclick=()=>playerStep(-1,true);$('#playerPlay').onclick=()=>playerToggle();$('#playerNext').onclick=()=>playerStep(1,true);$('#playerRepeatOne').onclick=()=>setRepeatMode('one');$('#playerRepeatAll').onclick=()=>setRepeatMode('all');$('#playerShuffle').onclick=()=>{playerShuffle=!playerShuffle;playerActivated=true;savePlayerState();syncPlayerUI()};bindPlayerVolumeControl();$('#playerProgress').onclick=e=>{const r=e.currentTarget.getBoundingClientRect();if(a.duration>0)a.currentTime=a.duration*((e.clientX-r.left)/r.width)};$$('[data-player-track]').forEach(b=>b.onclick=()=>{playerActivated=true;loadPlayerTrack(Number(b.dataset.playerTrack),true)});const toggle=$('#playerPlaylistToggle'),panel=$('#playerPlaylistPanel');if(toggle&&panel)toggle.onclick=()=>{const opening=panel.classList.contains('hidden');panel.classList.toggle('hidden',!opening);toggle.setAttribute('aria-expanded',String(opening));toggle.classList.toggle('open',opening);const small=toggle.querySelector('small');if(small)small.textContent=opening?'Pulsa aquí para cerrar':'Pulsa aquí para abrir'};syncPlayerUI();cleanupPlayerVolumeArtifacts();startPlayerArtworkForCurrent();if(!a.paused)startPlayerWaveVisualizer();updateMiniPlayerVisibility('player');ensurePlaylistTotalDuration().then(total=>{const el=$('#playerTotalDuration');if(el)el.textContent=total>0?fmtPlayer(total):'PENDIENTE'}).catch(()=>{})}
...
function setupUserPageReveal(){const page=$('.user-page');if(!page)return;const boxes=[...page.querySelectorAll('.user-hero,.user-achievements-section,.user-avatar-section,.user-account-panel')];page.classList.add('user-page-reveal-ready');let i=0;for(const box of boxes){box.style.setProperty('--user-box-i',i++);let j=0;for(const el of box.querySelectorAll(':scope > *, .achievement-earned-card, .avatar-choice'))el.style.setProperty('--user-item-i',j++)}requestAnimationFrame(()=>requestAnimationFrame(()=>page.classList.add('user-page-reveal-in')))}
function animateRegisteredUserHero(){
  const hero=$('.user-hero.registered');if(!hero)return;
  hero.classList.add('user-hero-v423');
  const name=hero.querySelector('.user-hero-name'),status=hero.querySelector('.user-hero-status'),email=hero.querySelector('.user-email'),edit=hero.querySelector('.user-edit-name-btn'),levelNumber=hero.querySelector('.user-hero-level-number');
  ...
  if(reduced){if(levelNumber)levelNumber.textContent=String(target);hero.classList.add('user-hero-v423-complete');return}
  for(const row of rows)row.el.textContent='';
  if(levelNumber)levelNumber.textContent='0';
  if(edit){edit.style.opacity='0';edit.style.transform='translateY(7px)'}
  ...
  typeTimed(name,rows.find(r=>r.el===name)?.text||'',110,330);
  typeTimed(status,rows.find(r=>r.el===status)?.text||'',390,230);
  typeTimed(email,rows.find(r=>r.el===email)?.text||'',610,300);
  setTimeout(()=>{if(!levelNumber)return;if(target<=0){levelNumber.textContent=String(target);return}const t0=performance.now(),dur=500,frame=now=>{const p=Math.min(1,(now-t0)/dur),e=1-Math.pow(1-p,3);levelNumber.textContent=String(Math.min(target,Math.round(target*e)));if(p<1)requestAnimationFrame(frame);else levelNumber.textContent=String(target)};requestAnimationFrame(frame)},430);
  setTimeout(()=>{if(edit){edit.animate?.([{opacity:0,transform:'translateY(7px)'},{opacity:1,transform:'translateY(0)'}],{duration:250,easing:'ease-out',fill:'both'});edit.style.opacity='1';edit.style.transform='none'}},1240);
  setTimeout(()=>hero.classList.add('user-hero-v423-complete'),1640);
}
...
function startIntroWhiteHandoff(){const layer=$('#introWhiteHandoff');if(!layer||layer.classList.contains('hidden'))return;requestAnimationFrame(()=>layer.classList.add('fade-out'));setTimeout(()=>{layer.classList.add('hidden');layer.classList.remove('fade-out');layer.setAttribute('aria-hidden','true');layer.style.opacity=''},1080)}
async function playEntryIntro(){if(introPlayed)return false;introPlayed=true;const layer=$('#introLayer'),video=$('#introVideo');if(!layer||!video)return false;layer.classList.remove('hidden');layer.setAttribute('aria-hidden','false');let finished=false;try{video.currentTime=0}catch{}return new Promise(resolve=>{const done=(withWhite=true)=>{if(finished)return;finished=true;clearTimeout(fallback);try{video.pause()}catch{}if(withWhite)prepareIntroWhiteHandoff();layer.classList.add('hidden');layer.setAttribute('aria-hidden','true');resolve(!!withWhite)};const start=()=>{clearTimeout(fallback);const p=video.play();if(p&&typeof p.catch==='function')p.catch(()=>done(false))};const fallback=setTimeout(()=>done(false),8000);layer.onclick=()=>done(true);video.addEventListener('ended',()=>done(true),{once:true});video.addEventListener('error',()=>done(false),{once:true});if(video.readyState>=3)start();else video.addEventListener('canplay',start,{once:true})})}

async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.23.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
