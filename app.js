...
function renderLocked(s,msg=''){return `<section class="locked-story-page"><div class="locked-main-lock" aria-label="Story bloqueada">${lockSvg(false)}</div>${storyFeaturedMedia(s,true)}<div class="lock-screen"><h2>STORY EXCLUSIVA DEL LIBRO ${escapeHtml(String(Number(s.book)||s.book||'—'))}</h2><p>${msg?escapeHtml(msg):'Esta Story requiere el QR específico impreso en el libro físico. Escanéalo para desbloquearla.'}</p><button class="cta" id="unlockThis">DESBLOQUEAR STORY ${escapeHtml(s.id)}</button></div></section>`}
...
function showUnlock(prefill=''){
  const meta=prefill?indexData.stories.find(s=>s.id===prefill):null;
  view.innerHTML=`<section class="unlock-panel section"><div class="eyebrow">Acceso mediante QR</div><h1>Desbloquear Story${prefill?' '+escapeHtml(prefill):''}</h1><p class="scanner-copy">Activa el escáner y pasa el QR específico impreso junto al relato por delante de la cámara. La detección es automática: no hace falta hacer ninguna foto.</p><div id="qr-reader" class="qr-reader qr-reader-live"><div class="qr-placeholder">QR</div></div><div id="qr-file-reader" class="qr-file-reader" aria-hidden="true"></div><button class="cta" id="startQr">ESCANEAR QR CON LA CÁMARA</button><button class="secondary-btn" id="photoQr" style="margin-top:10px">CARGAR IMAGEN DESDE TU ÁLBUM</button><input id="qrFile" class="qr-native-input" type="file" accept="image/*"><p id="unlockStatus" class="note" style="margin-top:16px"></p><p class="scanner-help">Al activar el escáner, mantén el QR dentro del recuadro hasta que la app lo detecte. «Cargar imagen» queda como alternativa para una foto ya guardada.</p>${meta?lockedTeaserHtml(meta):''}</section>`;
  ...
  requestAnimationFrame(()=>startQrCamera(prefill));
}
...
async function startQrCamera(prefill=''){
  ...
  if(!window.isSecureContext){unlockStatus('El escáner en directo necesita una conexión HTTPS segura.');return}
  ...
  await stopQrScanner();
  qrScanBusy=true;unlockStatus('Solicitando acceso a la cámara…');
  const onDecoded=async decoded=>{if(!qrScanBusy)return;qrScanBusy=false;unlockStatus('QR detectado. Validando acceso…');await stopQrScanner();await tryUnlock(decoded,prefill)};
  const qrbox=(w,h)=>{const side=Math.max(150,Math.min(210,Math.floor(Math.min(w,h)*.68)));return{width:side,height:side}};
  const config={fps:15,qrbox,aspectRatio:1.0,disableFlip:false};
  try{
    const reader=$('#qr-reader');if(reader)reader.innerHTML='';
    qrScanner=new Html5Qrcode('qr-reader',{verbose:false});
    try{await qrScanner.start({facingMode:'environment'},config,onDecoded,()=>{})}
    catch(first){
      const cams=await Html5Qrcode.getCameras();if(!cams?.length)throw first;
    ...
    unlockStatus('Escáner activo. Pasa el QR por delante de la cámara.');
    const btn=$('#startQr');if(btn){btn.textContent='ESCÁNER ACTIVO';btn.disabled=true}
  }catch(e){
    try{await stopQrScanner()}catch{}
    unlockStatus('No se pudo abrir el escáner en directo. Revisa el permiso de cámara o usa «Cargar imagen desde tu álbum».');
    const btn=$('#startQr');if(btn){btn.textContent='REINTENTAR ESCÁNER QR';btn.disabled=false}
    qrScanBusy=false;
  }
}
...
async function showUnlockReward(id){
  ...
  await new Promise(r=>setTimeout(r,motionReduced()?40:760));
  el.querySelector('.unlock-reward-poster')?.classList.add('fade-poster');
  await new Promise(r=>setTimeout(r,motionReduced()?30:420));
...
function renderFuturePlaceholder(title,tone){currentStory=null;view.innerHTML=`<section class="section v2-section future-placeholder ${escAttr(tone)}"><div class="eyebrow">PREPARADO PARA FUTURA ITERACIÓN</div><h1>${escapeHtml(title)}</h1><div class="future-placeholder-box"><strong>PRÓXIMAMENTE</strong><p>La entrada visual ya forma parte de Disturbing Stories App v2.4. Su contenido se desarrollará en una iteración posterior.</p></div></section>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=2.4.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
