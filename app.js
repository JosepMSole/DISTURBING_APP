...
let sagasData={schema:1,sagas:[]},timelineData={schema:1,orders:{}},extrasData={schema:1,extras:[]},playerCatalogData={schema:1,tracks:[]},homeData={schema:1,nextStoryDate:'',portals:{micro:true,games:false}},avatarsData={schema:1,avatars:[]},userConfigData={schema:1,achievements:[{id:'achievement1',number:1,name:'PRIMERA STORY',description:'Lee tu primera Story.',requirement:{type:'read_stories',count:1}}]},microsData={schema:1,items:[]},currentMicro=null,gamesData={schema:1,games:[]},currentGame=null;
...
async function boot(){
  ...
  try{
    ...
    [sagasData,timelineData,extrasData,playerCatalogData,homeData,avatarsData,userConfigData,microsData,gamesData]=await Promise.all([
      ...
      loadOptionalJson('./data/games.json',{schema:1,games:[],heroImage:'',heroVersion:''})
    ]);
    initialiseNewState();
    initUserSystem();
...
function gameCardHtml(g,i=0){const cover=gameAssetUrl(g.cover),num=Math.max(0,Number(g.number)||0);return `<button class="game-card section-entry-item" style="--entry-i:${i}" type="button" data-game-id="${escAttr(g.id)}">${cover?`<img src="${escAttr(cover)}" alt="${escAttr(g.title||'Game')}" loading="lazy" referrerpolicy="no-referrer">`:'<div class="game-card-placeholder">GAME</div>'}<div class="game-card-copy"><span>${num?`GAME #${String(num).padStart(2,'0')} · `:''}${escapeHtml(gameTypeLabel(g.type))}</span><h2>${escapeHtml(g.title||g.id)}</h2>${gameCompatibilityIconHtml(g.compatibility,true)}</div></button>`}
function bindGameCards(){$$('[data-game-id]').forEach(b=>b.onclick=()=>go(`game/${encodeURIComponent(b.dataset.gameId)}`))}
...
function typeGamesIntro(){const el=$('#gamesIntroTyping');if(!el)return;const text=el.dataset.text||'';if(motionReduced()){el.textContent=text;return}el.textContent='';const duration=Math.max(900,Math.min(1900,text.length*22)),start=performance.now();const tick=now=>{const p=Math.min(1,(now-start)/duration),n=Math.max(0,Math.round(text.length*p));el.textContent=text.slice(0,n);if(p<1)requestAnimationFrame(tick)};requestAnimationFrame(tick)}
function renderGamesCatalog(){currentStory=null;currentMicro=null;currentGame=null;if(!featurePortalEnabled('games'))return renderFuturePlaceholder('GAMES','rosa');const rows=[...(gamesData?.games||[])].filter(g=>g&&g.published!==false).sort((a,b)=>(Number(b.number)||0)-(Number(a.number)||0)||new Date(b.publicationDate||0)-new Date(a.publicationDate||0));const hero=gameHeroUrl(),intro='Algunos están diseñados especialmente para ordenador; consulta la compatibilidad antes de jugar.';view.innerHTML=`<section class="section games-section section-entry-page"><div class="games-hero section-entry-block ${hero?'has-image':''}">${hero?`<img class="games-hero-image" src="${escAttr(hero)}" alt="" referrerpolicy="no-referrer">`:''}<p id="gamesIntroTyping" data-text="${escAttr(intro)}"></p></div><div class="games-summary section-entry-block"><strong id="gamesAvailableCount">0</strong><span>${rows.length===1?'GAME DISPONIBLE':'GAMES DISPONIBLES'}</span></div><div class="games-grid">${rows.length?rows.map((g,i)=>gameCardHtml(g,i)).join(''):'<div class="empty">Todavía no hay Games publicados.</div>'}</div></section>`;bindGameCards();bindBrokenImages();animateGameAvailableCount(rows.length);typeGamesIntro()}
...
function renderGameDetail(id){currentStory=null;currentMicro=null;const g=gameById(id);currentGame=g;if(!g||g.published===false){view.innerHTML='<div class="empty">Game no encontrado.</div>';return}const cover=gameAssetUrl(g.cover),orientation=gameOrientationLabel(g.orientation),num=Math.max(0,Number(g.number)||0),desc=String(g.description||'');view.innerHTML=`<section class="section game-detail section-entry-page"><div class="game-detail-poster section-entry-block">${cover?`<img src="${escAttr(cover)}" alt="${escAttr(g.title||'Game')}" class="game-detail-cover">`:''}</div><div class="game-detail-info section-entry-block"><div class="game-detail-topline"><span>${num?`GAME #${String(num).padStart(2,'0')} · `:''}${escapeHtml(gameTypeLabel(g.type))}</span>${g.publicationDate?`<small>${formatDate(g.publicationDate)}</small>`:''}</div>${gameCompatibilityIconHtml(g.compatibility,false)}<div class="game-badges"><b>${escapeHtml(orientation)}</b></div>${desc?`<div class="game-description">${escapeHtml(desc).replace(/\n/g,'<br>')}</div>`:''}</div>${g.instructions?`<div class="game-instructions section-entry-block"><strong>INSTRUCCIONES</strong><p>${escapeHtml(g.instructions).replace(/\n/g,'<br>')}</p></div>`:''}<div class="game-actions section-entry-block"><button id="gamePlayBtn" class="game-play-btn" type="button">JUGAR</button></div>${gameStoryLinkHtml(g)}${gameSagaLinkHtml(g)}<button id="backToGamesBtn" class="game-back-btn section-entry-block" type="button">VOLVER A GAMES</button></section>`;$('#gamePlayBtn').onclick=()=>go(`game/${encodeURIComponent(g.id)}/play`);$('#backToGamesBtn').onclick=()=>go('games');$$('[data-game-story]').forEach(b=>b.onclick=()=>go(`story/${encodeURIComponent(b.dataset.gameStory)}`));$$('[data-game-saga]').forEach(b=>b.onclick=()=>go(`saga/${encodeURIComponent(b.dataset.gameSaga)}`));bindBrokenImages()}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=5.9.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
