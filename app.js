...
function sectionIndicatorMarkup(route){
  ...
  if(route==='games')return `<span class="section-indicator-games" aria-hidden="true">${animatedFeatureIcon('game','portal-custom-svg gamepad-portal-svg')}</span>`;
  if(route==='micro-pesadillas')return `<span class="section-indicator-micro" aria-hidden="true">${animatedFeatureIcon('micro')}</span>`;
...
function startPlayerArtworkMotion(box,key){
  stopPlayerArtworkMotion();if(!box)return;
  const imgs=$$('.player-artwork-slide img',box);if(!imgs.length)return;
  const specs=imgs.map((img,i)=>({img,i,phase:[.37,2.13,4.41,5.72][i%4]+i*.61,px:[7.2,6.1,8.4,5.8][i%4],py:[5.6,7.5,4.7,6.8][i%4],period:[6.4,7.7,5.9,8.6][i%4]}));
  const started=performance.now();
  const frame=now=>{
    if(routeName()!=='player'||!box.isConnected||playerArtworkTrack!==key){playerArtworkMotionRaf=0;return}
    const t=(now-started)/1000;
    for(const spec of specs){
      const {img,i,phase,px,py,period}=spec;
      const a=(t/period)*Math.PI*2+phase,b=(t/(period*1.43))*Math.PI*2+phase*.63;
      const x=Math.sin(a)*px+Math.sin(b)*px*.24,y=Math.cos(a*.83)*py+Math.sin(b*.91)*py*.27;
      const scale=1.11+.035*Math.sin(a*.58+phase)+.018*Math.cos(b*.72);
      img.style.transform=`translate3d(${x.toFixed(2)}%,${y.toFixed(2)}%,0) scale(${scale.toFixed(4)})`;
      img.style.transformOrigin='50% 50%';
      img.style.willChange='transform';
    }
    playerArtworkMotionRaf=requestAnimationFrame(frame);
  };
  playerArtworkMotionRaf=requestAnimationFrame(frame);
}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.28.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
