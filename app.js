...
function lockedHeroMedia(s){const teaser=lockedTeaserHtml(s,true);if(teaser)return teaser;const poster=displayCover(s);return `<section class="locked-hero-teaser locked-poster-fallback"><div class="locked-teaser-video">${poster?`<img class="locked-teaser-poster" src="${escAttr(poster)}" alt="Póster Story ${escAttr(s.id)}" loading="eager" referrerpolicy="no-referrer">`:'<div class="locked-poster-empty"></div>'}</div></section>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=2.22.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
