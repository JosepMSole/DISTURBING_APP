...
function syncMyStoriesHeaderAvatar(){
  ...
  // v5.26 · el avatar de MIS STORIES ocupa aproximadamente la mitad de la altura
  // de la caja pendiente, igual de compacto que el avatar del menú lateral.
  const apply=()=>{const base=Math.round(pending.getBoundingClientRect().height||0),h=Math.max(26,Math.min(38,Math.round(base*.5)||32));avatar.style.width=`${h}px`;avatar.style.height=`${h}px`;avatar.style.flexBasis=`${h}px`};
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=5.26.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
