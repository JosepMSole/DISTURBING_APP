...
function publicTeaserSource(s){
  const candidates=[s?.publicTeaser,s?.teaserUrl,s?.teaser,s?.mediaYoutube?.teaser,s?.publicMedia?.teaser];
...
function lockedTeaserHtml(s){const src=publicTeaserSource(s),embed=youtubeEmbedUrl(src);if(!embed)return'';return `<section class="locked-teaser"><div class="eyebrow">TEASER</div><div class="locked-teaser-video"><iframe src="${escAttr(embed)}" title="Teaser Story ${escAttr(s.id)}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe></div></section>`}
function renderLocked(s,msg=''){const book=String(Number(s.book)||s.book||'—');return `<section class="locked-story-page"><div class="locked-main-lock" aria-label="Story bloqueada">${lockSvg(false)}</div>${storyFeaturedMedia(s,true)}${msg?`<p class="locked-error-note">${escapeHtml(msg)}</p>`:''}<section class="unlock-panel locked-inline-unlock"><div class="scanner-instruction story-specific"><strong>SÓLO EN EL LIBRO ${escapeHtml(book)}</strong><p>Esta Story requiere el QR que aparece junto a este relato en el libro físico. Escanéalo para desbloquearla.</p></div><div class="scan-zone"><div id="qr-reader" class="qr-reader qr-reader-live"><div class="qr-placeholder">CÁMARA</div></div><div id="qr-file-reader" class="qr-file-reader" aria-hidden="true"></div><button class="cta" id="startQr">ACTIVAR ESCÁNER QR</button><button class="secondary-btn" id="photoQr" style="margin-top:10px">CARGAR IMAGEN DESDE TU ÁLBUM</button><input id="qrFile" class="qr-native-input" type="file" accept="image/*"><p id="unlockStatus" class="note" style="margin-top:12px"></p></div></section>${lockedTeaserHtml(s)}</section>`}
...
async function ensureQrLibrary(){
  ...
  return new Promise(resolve=>{let settled=false;const finish=ok=>{if(settled)return;settled=true;resolve(!!ok)};const old=document.querySelector('script[data-qr-lib]');if(old){old.addEventListener('load',()=>finish(typeof Html5Qrcode!=='undefined'),{once:true});old.addEventListener('error',()=>finish(false),{once:true});setTimeout(()=>finish(typeof Html5Qrcode!=='undefined'),4500);return}const sc=document.createElement('script');sc.src='https://cdnjs.cloudflare.com/ajax/libs/html5-qrcode/2.3.8/html5-qrcode.min.js';sc.async=true;sc.dataset.qrLib='1';sc.onload=()=>finish(typeof Html5Qrcode!=='undefined');sc.onerror=()=>finish(false);document.head.append(sc);setTimeout(()=>finish(typeof Html5Qrcode!=='undefined'),4500)})
}
async function startQrCamera(prefill=''){
  if(qrScanBusy)return;
  if(!window.isSecureContext){unlockStatus('El escáner en directo necesita HTTPS. Abre la PWA publicada.');return}
  ...
  reader.innerHTML='';qrScanBusy=true;unlockStatus('Solicitando acceso a la cámara…');if(btn){btn.disabled=true;btn.textContent='ABRIENDO CÁMARA…'}
  ...
  try{
    ...
    if(btn){btn.disabled=true;btn.textContent='ESCÁNER ACTIVO'}
  }catch(e){
    console.warn('QR live scanner',e);
    ...
    qrScanBusy=false;if(btn){btn.disabled=false;btn.textContent='REINTENTAR ESCÁNER QR'}
  }
}
...
async function stopQrScanner(){qrScanBusy=false;clearTimeout(qrLiveTimer);qrLiveTimer=0;qrLiveDecodeBusy=false;const stream=qrLiveStream;qrLiveStream=null;if(stream)try{stream.getTracks().forEach(t=>t.stop())}catch{};const d=qrFrameDecoder;qrFrameDecoder=null;if(d)try{await d.clear()}catch{};const s=qrScanner;qrScanner=null;if(!s)return;try{if(s.isScanning)await s.stop()}catch{}try{await s.clear()}catch{}}
...
function renderFuturePlaceholder(title,tone){currentStory=null;view.innerHTML=`<section class="section v2-section future-placeholder ${escAttr(tone)}"><div class="eyebrow">PREPARADO PARA FUTURA ITERACIÓN</div><h1>${escapeHtml(title)}</h1><div class="future-placeholder-box"><strong>PRÓXIMAMENTE</strong><p>La entrada visual ya forma parte de Disturbing Stories App v2.7. Su contenido se desarrollará en una iteración posterior.</p></div></section>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=2.7.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
