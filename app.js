...
function mediaCollectionIcons(s){const a=[];if(storyHasFeature(s,'audio'))a.push(`<span class="media-list-icon audio" title="Audio">${mediaIconSvg('audio')}</span>`);if(storyHasFeature(s,'video'))a.push(`<span class="media-list-icon video" title="Vídeo">${mediaIconSvg('video')}</span>`);if(s?.access==='exclusive'){const open=!isLocked(s);a.push(`<span class="media-list-icon lock ${open?'open':'closed'}" title="${open?'Desbloqueada':'Bloqueada'}">${lockSvg(open)}</span>`)}return a.join('')}
function renderMediaCollection(type){currentStory=null;const isAudio=type==='audio',items=(indexData.stories||[]).filter(s=>storyHasFeature(s,type)),durations=items.map(s=>mediaDuration(s,type)),complete=items.length>0&&durations.every(x=>x>0),total=complete?durations.reduce((a,b)=>a+b,0):0;view.innerHTML=`<section class="section v2-section media-collection ${isAudio?'cassettes':'tapes'}"><div class="media-section-head"><div class="media-title-no-icon"><div><div class="eyebrow">DISTURBING STORIES</div><h1>${isAudio?'Disturbing Cassettes':'Disturbing Tapes'}</h1></div></div><div class="media-summary"><div><span>TOTAL</span><strong>${items.length}</strong><em>${isAudio?'CASSETTES':'TAPES'}</em></div><div><span>DURACIÓN TOTAL</span><strong>${complete?fmtTotalDuration(total):'PENDIENTE'}</strong></div></div></div><div class="media-catalog">${items.length?items.map(s=>mediaCollectionItem(s,type)).join(''):'<div class="empty">No hay contenido disponible en esta sección.</div>'}</div></section>`;$$('[data-media-story]').forEach(el=>el.onclick=()=>navigateHash(`#/story/${encodeURIComponent(el.dataset.mediaStory)}/${encodeURIComponent(el.dataset.mediaTab)}`));bindBrokenImages()}
function mediaCollectionItem(s,type){const cover=displayCover(s),d=mediaDuration(s,type);return `<article class="media-collection-card ${isLocked(s)?'is-locked':''}" data-media-story="${escAttr(s.id)}" data-media-tab="${escAttr(type)}">${cover?`<div class="media-collection-poster"><img src="${escAttr(cover)}" alt="">${readCornerBadge(s)}${newStoryBadge(s)}${exclusiveBadge(s,'cover')}</div>`:''}<div class="media-collection-copy"><span>STORY ${escapeHtml(s.id)}</span><strong>${escapeHtml(s.title||'')}</strong><em>${d?fmtLong(d):'DURACIÓN PENDIENTE'}</em></div><div class="media-collection-icons">${mediaCollectionIcons(s)}</div></article>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=2.23.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
