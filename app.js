...
function bindGlobalStormMediaStop(){document.addEventListener('play',e=>{const t=e.target;if(!(t instanceof HTMLMediaElement))return;if(t.id==='stormAudio'||t.id==='introVideo')return;stopStormForOtherMedia()},true)}
...
async function playEntryIntro(){if(introPlayed)return false;introPlayed=true;const gate=$('#entryGate'),gateBtn=$('#entryGateButton'),layer=$('#introLayer'),video=$('#introVideo');if(!layer||!video)return false;if(!gate||!gateBtn)return playEntryIntroDirect(layer,video);gate.classList.remove('hidden');gate.setAttribute('aria-hidden','false');return new Promise(resolve=>{let started=false;const enter=()=>{if(started)return;started=true;primeStormAudio();gateBtn.disabled=true;gate.classList.add('leaving');setTimeout(()=>{gate.classList.add('hidden');gate.classList.remove('leaving');gate.setAttribute('aria-hidden','true')},220);startIntroMedia(layer,video,resolve)};gateBtn.addEventListener('click',enter,{once:true})})}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.24.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
