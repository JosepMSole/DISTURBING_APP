...
function exclusiveBadge(s,kind='cover'){if(s?.access!=='exclusive')return'';const state=exclusiveState(s),open=state==='unlocked',mediaCount=(storyHasFeature(s,'audio')?1:0)+(storyHasFeature(s,'video')?1:0);return `<span class="ex-badge ex-${state} ex-after-${mediaCount} ${kind==='inline'?'ex-inline':''}" title="Exclusiva ${open?'desbloqueada':'bloqueada'}" aria-label="Exclusiva ${open?'desbloqueada':'bloqueada'}">${lockSvg(open)}</span>`}
...
function storyCard(s,mode='list',sortMode=''){
  const locked=isLocked(s),read=isRead(s.id),cover=displayCover(s),pr=storyProgress(s.id),ex=exclusiveBadge(s,'cover'),media=storyMediaBadges(s),sortBadge=storySortBadge(s,sortMode),readCorner=readCornerBadge(s);
  ...
  const thumb=`<div class="story-thumb-wrap">${image}${readCorner}${sortBadge}${ex}${media}</div>`;
  if(mode==='covers')return `<article class="story-cover-card" data-story-id="${escAttr(s.id)}" aria-label="Story ${escAttr(s.id)} · ${escAttr(s.title||'')}">${image}${readCorner}${sortBadge}${read?'<span class="cover-read">✓ LEÍDA</span>':pr>.015?`<span class="cover-progress">${pct(pr)}</span>`:''}${ex}${media}</article>`;
  if(mode==='grid')return `<article class="story-grid-card" data-story-id="${escAttr(s.id)}"><div class="story-grid-poster">${image}${readCorner}${sortBadge}${ex}${media}</div><div class="story-grid-copy"><div class="story-number">STORY ${escapeHtml(s.id)}</div><h3>${escapeHtml(s.title)}</h3><p>${s.access==='exclusive'?(locked?'EXCLUSIVA · BLOQUEADA':'EXCLUSIVA · DESBLOQUEADA'):'GRATIS'}${read?' · LEÍDA':''}</p></div></article>`;
...
function renderCollection(bookId=''){
  ...
  if(bookId&&books.has(bookId)){
    ...
    view.innerHTML=`<section class="section collection-section"><button class="collection-back" id="collectionBack">‹ COLECCIÓN</button><div class="book-detail-head"><div><div class="eyebrow">Tu colección</div><div class="book-detail-title-row"><h1>${escapeHtml(bookLabel(bookId))}</h1>${bookFeatureSummary(stories,true)}</div></div>${covers?`<div class="book-cover-spread ${asset.back?'':'single'}">${covers}</div>`:''}</div>${asset.amazon?`<a class="amazon-buy-btn" href="${escAttr(asset.amazon)}" target="_blank" rel="noopener noreferrer"><span class="amazon-buy-copy">COMPRAR EN <span class="amazon-wordmark">AMAZON</span></span></a>`:''}<div class="book-detail-stats"><div><strong>${read}</strong><span>LEÍDAS</span></div><div><strong>${available}</strong><span>DISPONIBLES</span></div><div><strong>${stories.length}</strong><span>PUBLICADAS</span></div></div><div class="collection-progress large"><span style="width:${pct(stories.length?read/stories.length:0)}"></span></div><div class="book-story-filters"><button class="book-story-filter active" data-book-feature="all">TODAS</button>${hasAudio?'<button class="book-story-filter" data-book-feature="audio">CON AUDIO</button>':''}${hasVideo?'<button class="book-story-filter" data-book-feature="video">CON VÍDEO</button>':''}</div><div id="bookStoriesGrid" class="stories-view stories-view-covers collection-story-grid"></div></section>`;
...
function renderLocked(s,msg=''){return `<section class="lock-screen"><div class="lock">▣</div><h2>STORY EXCLUSIVA DEL LIBRO ${escapeHtml(String(Number(s.book)||s.book||'—'))}</h2><p>${msg?escapeHtml(msg):'Esta Story requiere el QR específico impreso en el libro físico. Escanéalo para desbloquearla.'}</p><button class="cta" id="unlockThis">DESBLOQUEAR STORY ${escapeHtml(s.id)}</button></section>`}
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=1.17.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
