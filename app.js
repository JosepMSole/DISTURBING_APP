...
async function boot(){
  ...
  try{
    ...
    [sagasData,timelineData,extrasData,playerCatalogData,homeData,avatarsData,userConfigData,microsData]=await Promise.all([
...
function setNav(route){const storyRoutes=new Set(['stories','story','sagas','saga','timeline','random','cassettes','tapes','extras','extra','games','micro-pesadillas','micro','micro-preview']);$$('.nav-btn').forEach(b=>b.classList.toggle('active',(b.dataset.route==='stories'&&storyRoutes.has(route))||b.dataset.route===route));updateSectionIndicator(route);updateUserChrome();updateMiniPlayerVisibility(route)}
...
function markMicroRead(id){id=String(id||'').padStart(3,'0');const a=microReadList();if(a.includes(id))return false;a.push(id);store.set(microReadKey,a);refreshMicroReadUI(id,true);queueUserVaultSync();return true}
...
function bindMicroDetail(m,{preview=false}={}){bindBrokenImages();$$('.micro-image-block img').forEach(img=>img.onclick=()=>openLightbox(img.src,img.alt));$$('[data-micro-story]').forEach(b=>b.onclick=()=>storyGo(b.dataset.microStory,false,b));if(preview)return;const id=String(m.id||m.number||'').padStart(3,'0'),rb=$('#microMarkRead'),ub=$('#microMarkUnread');if(rb)rb.onclick=()=>{if(markMicroRead(id))refreshMicroReadUI(id,true)};if(ub)ub.onclick=()=>{if(markMicroUnread(id))refreshMicroReadUI(id,false)};requestAnimationFrame(updateMicroProgress)}
...
function updateMicroProgress(){if(routeName()!=='micro'||!currentMicro)return;const r=$('#microReader');if(!r)return;const bottomNav=$('.bottom-nav')?.getBoundingClientRect().height||72,mini=document.body.classList.contains('player-mini-visible')?($('#miniPlayer')?.getBoundingClientRect().height||58):0,reached=r.getBoundingClientRect().bottom<=innerHeight-bottomNav-mini+18;if(reached&&!isMicroRead(currentMicro.id))markMicroRead(currentMicro.id)}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=5.3.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
