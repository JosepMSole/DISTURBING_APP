...
function microIntroHtml(){return `<div class="micro-title"><img class="micro-title-logo" src="${escAttr(PUBLIC_APP_ASSET_BASE+'logomicros.png')}" alt="Micro-Pesadillas" loading="eager" referrerpolicy="no-referrer"><div class="micro-intro-panel" id="microIntroPanel"><p class="micro-intro-lead">Conoce una nueva subespecie de Disturbing Stories:</p><div id="microIntroMore" class="micro-intro-more"><strong>¡LAS MICRO-PESADILLAS!</strong><p>Relatos muy breves de terror y ciencia ficción que puedes leer en apenas un par de minutos.</p><strong>¡Descubre nuevas maneras de sorprenderte!</strong><p>Conoce nuevos horrores… y vuelve a encontrarte con viejos conocidos del universo de Disturbing Stories.</p><p>Las Micro-Pesadillas tienen su propia numeración, pero…</p><strong>¡CUIDADO!</strong><strong>¡PUEDEN APARECER EN CUALQUIER MOMENTO!</strong><p>Y recuerda: el hábitat exclusivo de las <b>MICRO-PESADILLAS</b> es Disturbing Stories App.</p><strong>¡SOLO PODRÁS LEERLAS AQUÍ!</strong></div><button id="microIntroToggle" class="micro-intro-toggle" type="button" aria-expanded="false" aria-controls="microIntroMore" aria-label="Mostrar información completa"><span aria-hidden="true">⌄</span></button></div></div>`}
function bindMicroIntroToggle(){const panel=$('#microIntroPanel'),btn=$('#microIntroToggle');if(!panel||!btn)return;btn.onclick=()=>{const open=panel.classList.toggle('expanded');btn.setAttribute('aria-expanded',open?'true':'false');btn.setAttribute('aria-label',open?'Ocultar información':'Mostrar información completa')}}
...
function celebrateMicroRead(id){const total=(microsData?.items||[]).length,count=microReadList().filter(x=>(microsData?.items||[]).some(m=>String(m.id||m.number||'').padStart(3,'0')===String(x).padStart(3,'0'))).length;document.querySelector('.read-celebration')?.remove();const el=document.createElement('div');el.className='read-celebration micro-read-celebration';el.innerHTML=`<button class="read-celebration-close" type="button" aria-label="Cerrar">×</button><div class="read-check">✓</div><div class="eyebrow">MICRO-PESADILLA #${escapeHtml(id)}</div><strong>LEÍDA</strong><small>${count} / ${total} Micro-Pesadillas publicadas</small>`;document.body.append(el);let timer;const close=()=>{clearTimeout(timer);el.classList.remove('show');setTimeout(()=>el.remove(),260)};el.querySelector('.read-celebration-close').onclick=close;setTimeout(()=>el.classList.add('show'),20);timer=setTimeout(close,3000)}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=5.4.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
