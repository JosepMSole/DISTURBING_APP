...
function uiSoundForControl(el){
  ...
  if(id==='gamePlayBtn'||el.matches?.('[data-game-id],[data-micro-id],[data-extra-id],[data-story-media-game],[data-story-media-micro],[data-related-game],[data-related-micro],[data-book-id],.extra-card'))return'openb';
...
function syncMyStoriesHeaderAvatar(){
  const title=$('.my-stories-title'),avatar=$('#myStoriesAvatar'),pending=title?.querySelector('.unread-callout');if(!title||!avatar||!pending)return;
  const apply=()=>{const h=Math.max(42,Math.round(pending.getBoundingClientRect().height||0));avatar.style.width=`${h}px`;avatar.style.height=`${h}px`;avatar.style.flexBasis=`${h}px`};
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=5.25.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
