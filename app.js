...
let sagasData={schema:1,sagas:[]},timelineData={schema:1,orders:{}},extrasData={schema:1,extras:[]},playerCatalogData={schema:1,tracks:[]},homeData={schema:1,nextStoryDate:''},avatarsData={schema:1,avatars:[]},userConfigData={schema:1,registeredHeroBackground:'',achievements:[{id:'achievement1',name:'PRIMERA STORY',description:'Lee tu primera Story.',requirement:{type:'read_stories',count:1}}]};
...
const localAvatarKey='disturbing_user_avatar_v1', unlockedAvatarsKey='disturbing_unlocked_avatars_v1', avatarRewardSeenKey='disturbing_avatar_reward_seen_v1', userNameCacheKey='disturbing_user_name_v1', achievementEarnedKey='disturbing_achievements_v1';
...
const store={get(k,f){try{return JSON.parse(localStorage.getItem(k))??f}catch{return f}},set(k,v){localStorage.setItem(k,JSON.stringify(v));if(String(k).startsWith('disturbing_'))queueUserVaultSync()}};
...
async function boot(){
  ...
  try{
    ...
    [sagasData,timelineData,extrasData,playerCatalogData,homeData,avatarsData,userConfigData]=await Promise.all([
      ...
      loadOptionalJson('./data/user.json',{schema:1,registeredHeroBackground:'',achievements:[{id:'achievement1',name:'PRIMERA STORY',description:'Lee tu primera Story.',requirement:{type:'read_stories',count:1}}]})
    ]);
    initialiseNewState();
    initUserSystem();
...
/* ===== v4.1 · USER / AVATARES / SINCRONIZACIÓN AUTOMÁTICA ===== */
...
function achievementRequirementProgress(a){const r=a?.requirement||{},target=Math.max(1,Number(r.count)||1);if(r.type==='complete_sagas')return{current:completedSagaCount(),target,label:`COMPLETA ${target} ${target===1?'SAGA':'SAGAS'}`};return{current:publishedReadCount(),target,label:`LEE ${target} ${target===1?'STORY':'STORIES'}`}}
function achievementRequirementMet(a){const p=achievementRequirementProgress(a);return p.current>=p.target}
function evaluateAchievements(showReward=false){const got=earnedAchievementIds(),before=got.size;for(const a of achievementRows()){if(a?.id&&achievementRequirementMet(a))got.add(String(a.id))}if(got.size!==before){store.set(achievementEarnedKey,[...got]);queueUserVaultSync();if(showReward&&routeName()==='user')renderUser()}return got}
function earnedAchievements(){const got=evaluateAchievements(false);return achievementRows().filter(a=>a?.id&&got.has(String(a.id)))}
function achievementCardHtml(a){const p=achievementRequirementProgress(a);return `<article class="achievement-earned-card"><span class="achievement-medal" aria-hidden="true">★</span><div><strong>${escapeHtml(a.name||'LOGRO')}</strong><small>${escapeHtml(a.description||p.label)}</small></div></article>`}
function achievementsPanelHtml(){const rows=earnedAchievements();return `<div class="user-achievements-section"><div class="user-box-title">LOGROS CONSEGUIDOS</div><div class="achievement-earned-list">${rows.length?rows.map(achievementCardHtml).join(''):'<p class="achievement-empty">Todavía no has conseguido ningún logro.</p>'}</div></div>`}
function registeredHeroBackground(){const u=String(userConfigData?.registeredHeroBackground||'').trim();return /^https?:\/\//i.test(u)?u:''}
function registeredDisplayName(){return String(userProfile?.display_name||store.get(userNameCacheKey,'')||'').trim()}
function updateUserChrome(){const name=registeredDisplayName(),lab=$('#userMenuLabel'),hello=$('#userGreeting'),head=$('#userHeaderAvatar'),headImg=$('#userHeaderAvatarImg'),a=avatarById(selectedAvatarId());if(lab)lab.textContent=registeredUser?(name||'Usuario Registrado'):'Usuario No Registrado';const show=!!registeredUser&&!!name&&$('#sectionIndicator')?.dataset.section==='user';if(hello){hello.textContent=show?`Hola ${name}`:'';hello.classList.toggle('hidden',!show)}if(head){head.classList.toggle('hidden',!show);if(headImg&&show&&a){headImg.src=avatarImageUrl(a);headImg.alt=avatarDisplayName(a)||name||'Avatar'}}}
...
function renderUser(){currentStory=null;ensureFreeAvatars();evaluateAchievements(false);const registered=!!registeredUser,avatar=avatarById(selectedAvatarId())||avatarRows()[0],name=registeredDisplayName(),heroBg=registered?registeredHeroBackground():'';view.innerHTML=`<section class="section user-page"><div class="user-hero ${registered?'registered':''}">${heroBg?`<img class="user-hero-background" src="${escAttr(heroBg)}" alt="" referrerpolicy="no-referrer">`:''}<div class="user-avatar-current">${avatar?avatarImg(avatar,'user-avatar-img'):'<span>?</span>'}</div><div class="user-hero-copy"><div class="eyebrow">${registered?'USUARIO REGISTRADO':'Usuario No Registrado'}</div><h1>${escapeHtml(registered?(name||'Usuario Registrado'):'Usuario No Registrado')}</h1>${registered?`<p class="user-email">${escapeHtml(registered.email||'')}</p>`:'<p>Tu progreso permanece en este dispositivo. Regístrate para guardarlo y sincronizarlo con tu cuenta.</p>'}</div></div>${registered?achievementsPanelHtml():''}<div class="user-avatar-section"><h2>AVATAR</h2><div class="avatar-grid">${avatarRows().map(avatarChoiceHtml).join('')||'<div class="empty">Los avatares se cargan desde GitHub y se configuran desde el Importer.</div>'}</div></div>${registered?`<div class="user-account-panel"><strong class="sync-active-title">SINCRONIZACIÓN ACTIVA</strong><p>Tu progreso, desbloqueos y preferencias se sincronizan automáticamente con tu cuenta cada vez que cambian.</p><button id="userLogout" class="secondary-btn" type="button">CERRAR SESIÓN</button><p id="userStatus" class="note"></p></div>`:registrationPanelHtml()}</section>`;$$('[data-avatar-id]').forEach(b=>b.onclick=()=>chooseAvatar(b.dataset.avatarId));if(registered){$('#userLogout').onclick=logoutUser}else bindRegistrationPanel();bindBrokenImages();updateUserChrome()}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.2.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
