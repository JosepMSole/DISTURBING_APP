...
function updateInstallButton(){
  ...
  const btn=$('#installBtn');if(btn)btn.classList.toggle('hidden',standalone);
  const panel=$('#homeInstallPanel');if(panel)panel.classList.toggle('hidden',standalone);
}
function closeInstallGuide(){document.querySelector('.install-guide')?.remove()}
function showInstallGuide(kind='ios'){
  closeInstallGuide();
  const ios=kind==='ios',desktop=kind==='desktop',android=kind==='android';
  const canPrompt=!ios&&!!installPrompt;
  const modal=document.createElement('div');
  modal.className='install-guide';
  const steps=ios
    ? [`<div><b>1</b><span>Abre esta página en <strong>Safari</strong>.</span></div>`,`<div><b>2</b><span>Pulsa el botón <strong>COMPARTIR</strong> <em>↥</em>.</span></div>`,`<div><b>3</b><span>Busca y toca <strong>AÑADIR A PANTALLA DE INICIO</strong>.</span></div>`,`<div><b>4</b><span>Confirma con <strong>AÑADIR</strong>. Verás el icono en tu pantalla de inicio.</span></div>`]
    : android
      ? [`<div><b>1</b><span>Pulsa el botón <strong>INSTALAR AHORA</strong> que verás abajo.</span></div>`,`<div><b>2</b><span>Si tu navegador no abre el cuadro, abre el menú <strong>⋮</strong>.</span></div>`,`<div><b>3</b><span>Toca <strong>Instalar app</strong> o <strong>Añadir a pantalla de inicio</strong>.</span></div>`,`<div><b>4</b><span>Acepta y abre la app desde su nuevo icono.</span></div>`]
      : desktop
        ? [`<div><b>1</b><span>Pulsa <strong>INSTALAR AHORA</strong> si tu navegador lo permite.</span></div>`,`<div><b>2</b><span>Si no aparece el cuadro, abre el menú del navegador.</span></div>`,`<div><b>3</b><span>Elige <strong>Instalar app</strong>.</span></div>`,`<div><b>4</b><span>La app quedará instalada con su propio icono y ventana.</span></div>`]
        : [`<div><b>1</b><span>Abre el menú de tu navegador.</span></div>`,`<div><b>2</b><span>Busca <strong>Instalar app</strong> o <strong>Añadir a pantalla de inicio</strong>.</span></div>`,`<div><b>3</b><span>Confirma la instalación.</span></div>`,`<div><b>4</b><span>Después abre Disturbing Stories App desde su icono.</span></div>`];
  const helper=ios
    ? 'En iPhone y iPad Apple exige hacerlo desde el menú Compartir. Solo se hace una vez.'
    : canPrompt
      ? 'Tu navegador parece compatible con la instalación directa. Puedes hacerlo desde aquí.'
      : 'Si no ves la opción exacta, prueba dentro del menú principal del navegador.';
  modal.innerHTML=`<div class="install-guide-scrim" data-install-close></div><section class="install-guide-card hud-static" role="dialog" aria-modal="true" aria-label="Instalar Disturbing Stories APP"><button class="install-guide-close" data-install-close aria-label="Cerrar">×</button><div class="eyebrow">INSTALAR APP</div><h2>Cómo instalar Disturbing Stories App</h2><p>Esta app puede instalarse como una app real en tu dispositivo. Sigue estos pasos:</p><div class="install-steps">${steps.join('')}</div><small>${helper}</small>${canPrompt?`<div class="install-guide-actions"><button id="installGuideNow" class="cta">INSTALAR AHORA</button></div>`:''}</section>`;
...
async function handleInstallApp(){
  ...
  if(isIOSInstall()){
    ...
    showInstallGuide(isAndroid?'android':(desktop?'desktop':'other'));
  }
}
...
function renderHome(){
  ...
  const installPanelBtn=$('#homeInstallPanel');if(installPanelBtn)installPanelBtn.onclick=()=>{
    const ua=navigator.userAgent||'';
    if(isIOSInstall())showInstallGuide('ios');
    else if(/android/i.test(ua))showInstallGuide('android');
    else if(!/mobile|iphone|ipad|ipod|android/i.test(ua))showInstallGuide('desktop');
    else showInstallGuide('other');
  };
...
async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=2.18.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
