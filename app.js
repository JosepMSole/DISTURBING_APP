...
/* ===== v4.4 · USER / AVATARES / LOGROS / SINCRONIZACIÓN AUTOMÁTICA ===== */
...
function achievementRequirementProgress(a){
  const r=a?.requirement||{};
  if(r.type==='read_book'){const rows=achievementBookStories(r.book),target=rows.length,current=rows.filter(st=>isRead(String(st.id))).length,book=normalizeBookId(r.book);return{current,target,label:book?`LEE EL LIBRO ${book}`:'LIBRO PENDIENTE'}}
  const target=Math.max(1,Number(r.count)||1);
  if(r.type==='complete_sagas')return{current:completedSagaCount(),target,label:`COMPLETA ${target} ${target===1?'SAGA':'SAGAS'}`};
  return{current:publishedReadCount(),target,label:`LEE ${target} ${target===1?'STORY':'STORIES'}`}
}
function achievementRequirementMet(a){const p=achievementRequirementProgress(a);return p.target>0&&p.current>=p.target}
...
function achievementsPanelHtml(){const rows=earnedAchievements().sort((a,b)=>(Number(a.number)||0)-(Number(b.number)||0));return `<div class="user-achievements-section"><div class="user-box-title">LOGROS CONSEGUIDOS</div><div class="achievement-earned-list">${rows.length?rows.map(achievementCardHtml).join(''):'<p class="achievement-empty">Todavía no has conseguido ningún logro.</p>'}</div></div>`}
...
function updateUserChrome(){const name=registeredDisplayName(),lab=$('#userMenuLabel'),hello=$('#userGreeting'),head=$('#userHeaderAvatar'),headImg=$('#userHeaderAvatarImg'),a=avatarById(selectedAvatarId());if(lab)lab.textContent=registeredUser?(name||'Usuario Registrado'):'Usuario No Registrado';const show=!!registeredUser&&!!name;if(hello){hello.textContent=show?`Hola ${name}`:'';hello.classList.toggle('hidden',!show);bindUserHeaderShortcut(hello)}if(head){head.classList.toggle('hidden',!show);bindUserHeaderShortcut(head);if(headImg&&show&&a){headImg.src=avatarImageUrl(a);headImg.alt=avatarDisplayName(a)||name||'Avatar'}}}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.4.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
