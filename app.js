...
function ensurePlayerFullscreenLayer(){let layer=$('#playerFullscreenLayer');if(layer)return layer;layer=document.createElement('div');layer.id='playerFullscreenLayer';layer.className='player-fullscreen-layer hidden';layer.setAttribute('role','dialog');layer.setAttribute('aria-modal','true');layer.setAttribute('aria-hidden','true');layer.setAttribute('aria-label','Disturbing Player a pantalla completa');layer.innerHTML=`<button id="playerFullscreenClose" class="player-fullscreen-close" type="button" aria-label="Cerrar pantalla completa" title="Cerrar">×</button><div id="playerFullscreenStage" class="player-fullscreen-stage"></div>`;document.body.append(layer);$('#playerFullscreenClose',layer).onclick=()=>closePlayerFullscreen();layer.addEventListener('click',e=>{if(e.target===layer)closePlayerFullscreen()});return layer}
function openPlayerFullscreen(){const consoleEl=$('#playerConsole'),host=$('#playerConsoleHost');if(!consoleEl||!host)return;const layer=ensurePlayerFullscreenLayer(),stage=$('#playerFullscreenStage',layer);if(!stage)return;stage.append(consoleEl);layer.classList.remove('hidden');layer.setAttribute('aria-hidden','false');document.body.classList.add('player-fullscreen-open');requestAnimationFrame(()=>{$$('.player-artwork-slide img',consoleEl).forEach(playerArtworkFit);syncPlayerUI();$('#playerFullscreenClose',layer)?.focus()})}
function closePlayerFullscreen(force=false){const layer=$('#playerFullscreenLayer');if(!layer||layer.classList.contains('hidden'))return;const consoleEl=$('#playerConsole'),host=$('#playerConsoleHost');if(consoleEl&&host)host.append(consoleEl);layer.classList.add('hidden');layer.setAttribute('aria-hidden','true');document.body.classList.remove('player-fullscreen-open');if(!force)requestAnimationFrame(()=>{$$('.player-artwork-slide img',consoleEl||document).forEach(playerArtworkFit);$('#playerFullscreenBtn')?.focus()})}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.32.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
