...
function cleanupPlayerVolumeArtifacts(){const box=$('.player-console .player-volume');if(!box)return;$$('.player-console .player-mobile-volume-note,.player-console .player-volume-note,.player-console [data-player-volume-note],.player-console .volume-output,.player-console output').forEach(n=>n.remove())}
...
function animateRegisteredUserHero(){
  ...
  hero.classList.add('user-hero-v421');
  const avatar=hero.querySelector('.user-avatar-current'),level=hero.querySelector('.user-hero-level'),name=hero.querySelector('.user-hero-name'),status=hero.querySelector('.user-hero-status'),email=hero.querySelector('.user-email'),edit=hero.querySelector('.user-edit-name-btn'),levelNumber=hero.querySelector('.user-hero-level-number');
  const textRows=[name,status,email].filter(Boolean).map(el=>({el,text:String(el.textContent||'')}));
  for(const row of textRows)row.el.textContent='';
  const target=Math.max(0,parseInt(levelNumber?.textContent||'0',10)||0);if(levelNumber)levelNumber.textContent='0';
  if(edit)edit.classList.add('user-edit-name-v421-wait');
  const type=(el,value,speed)=>new Promise(resolve=>{let i=0;if(!value){resolve();return}const tick=()=>{i++;el.textContent=value.slice(0,i);if(i<value.length)setTimeout(tick,speed);else resolve()};tick()});
  const count=()=>new Promise(resolve=>{if(!levelNumber||target<=0){if(levelNumber)levelNumber.textContent=String(target);resolve();return}const t0=performance.now(),dur=Math.min(850,420+target*35);const frame=now=>{const p=Math.min(1,(now-t0)/dur),e=1-Math.pow(1-p,3);levelNumber.textContent=String(Math.min(target,Math.round(target*e)));if(p<1)requestAnimationFrame(frame);else{levelNumber.textContent=String(target);resolve()}};requestAnimationFrame(frame)});
  setTimeout(async()=>{for(let i=0;i<textRows.length;i++){await type(textRows[i].el,textRows[i].text,i===0?34:22);await new Promise(r=>setTimeout(r,65))}await count();if(edit){edit.classList.remove('user-edit-name-v421-wait');edit.classList.add('user-edit-name-v421-show')}},280);
}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.21.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
