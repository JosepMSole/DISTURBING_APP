...
function renderStories(initial='all'){
  ...
  $$('[data-library-stat]').forEach(btn=>btn.onclick=()=>{const mode=btn.dataset.libraryStat;if(mode==='available'){sessionStorage.setItem('disturbing_my_jump_v25','unread');go('unlocked');return}currentFilter=mode==='read'?'read':'all';$('#statusFilter').value=currentFilter;draw();scrollStoryFiltersToHeader()});
...
function renderMyStories(){
  ...
  const externalJump=sessionStorage.getItem('disturbing_my_jump_v25');if(externalJump){sessionStorage.removeItem('disturbing_my_jump_v25');draw(externalJump);scrollResultsToHeader()}else draw('unread');bindBrokenImages();
}
...
function showUnlock(prefill=''){
  ...
  view.innerHTML=`<section class="unlock-panel section"><div class="eyebrow">Acceso mediante QR</div><h1>Desbloquear Story${prefill?' '+escapeHtml(prefill):''}</h1><div class="scanner-instruction"><strong>SÓLO EN EL LIBRO ${escapeHtml(book)}</strong><p>Esta Story requiere el QR que aparece junto a este relato en el libro físico.</p><p>Escanéalo para desbloquearla.</p></div><div class="scan-zone"><div id="qr-reader" class="qr-reader qr-reader-live"><div class="qr-placeholder">CÁMARA</div></div><div id="qr-file-reader" class="qr-file-reader" aria-hidden="true"></div><button class="cta" id="startQr">ACTIVAR ESCÁNER QR</button><button class="secondary-btn" id="photoQr" style="margin-top:10px">CARGAR IMAGEN DESDE TU ÁLBUM</button><input id="qrFile" class="qr-native-input" type="file" accept="image/*"><p id="unlockStatus" class="note" style="margin-top:12px"></p></div>${meta?lockedTeaserHtml(meta):''}</section>`;
...
async function startQrCamera(prefill=''){
  ...
  await stopQrScanner();qrScanBusy=true;unlockStatus('Solicitando acceso a la cámara…');
  const btn=$('#startQr');if(btn){btn.disabled=true;btn.textContent='ABRIENDO CÁMARA…'}
  ...
  try{
    ...
    catch(first){const cams=await Html5Qrcode.getCameras();if(!cams?.length)throw first;const preferred=cams.find(c=>/back|rear|environment|trasera|posterior/i.test(c.label||''))||cams[cams.length-1];await qrScanner.start(preferred.id,config,onDecoded,()=>{})}
    unlockStatus('ESCÁNER ACTIVO · pasa el QR por delante de la cámara. La detección es automática.');if(btn){btn.textContent='ESCÁNER ACTIVO';btn.disabled=true}
  }catch(e){console.warn('QR live scanner',e);try{await stopQrScanner()}catch{}unlockStatus('No se pudo abrir el escáner en directo. Revisa el permiso de cámara y pulsa REINTENTAR.');if(btn){btn.textContent='REINTENTAR ESCÁNER QR';btn.disabled=false}qrScanBusy=false}
}
...
function spinRandom(pool){
  if(!pool.length)return;let candidates=pool.filter(s=>s.id!==lastRandomStoryId);if(!candidates.length)candidates=pool;const picked=candidates[Math.floor(Math.random()*candidates.length)];lastRandomStoryId=picked.id;
  const stage=$('#randomPosterRoll'),result=$('#randomResult'),btn=$('#spinRandom');if(!stage)return;result.innerHTML='';btn.disabled=true;stage.classList.add('rolling');let ticks=0;const max=20;
  const timer=setInterval(()=>{const s=pool[Math.floor(Math.random()*pool.length)];stage.innerHTML=randomPosterOnly(s);bindBrokenImages();ticks++;if(ticks>=max){clearInterval(timer);stage.innerHTML=randomPosterOnly(picked);stage.classList.remove('rolling');stage.classList.add('winner');setTimeout(()=>{stage.classList.remove('winner');showRandomResult(picked);btn.disabled=false},720)}},70)
}
function showRandomResult(s){const el=$('#randomResult');if(!el)return;el.innerHTML=`<article class="random-result random-result-v25"><div><div class="eyebrow">RESULTADO</div><h2>STORY ${escapeHtml(s.id)} · ${escapeHtml(s.title||'')}</h2><p>${escapeHtml(timelineYearLabel(parseStoryYear(s)))}</p></div><div class="random-actions"><button class="cta" id="randomRead">LEER STORY</button><button class="secondary-btn" id="randomAgain">VOLVER A GIRAR</button></div></article>`;el.classList.remove('reward-in');void el.offsetWidth;el.classList.add('reward-in');$('#randomRead').onclick=()=>storyGo(s.id);$('#randomAgain').onclick=()=>$('#spinRandom').click()}
