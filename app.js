...
let sagasData={schema:1,sagas:[]},timelineData={schema:1,orders:{}},extrasData={schema:1,extras:[]},playerCatalogData={schema:1,tracks:[]},homeData={schema:1,nextStoryDate:''},avatarsData={schema:1,avatars:[]};
...
const localAvatarKey='disturbing_user_avatar_v1', unlockedAvatarsKey='disturbing_unlocked_avatars_v1', avatarRewardSeenKey='disturbing_avatar_reward_seen_v1';
...
async function boot(){
  ...
  try{
    ...
    [sagasData,timelineData,extrasData,playerCatalogData,homeData,avatarsData]=await Promise.all([
...
function bookTrailerHtml(url,bookId){
  ...
  return `<section class="book-trailer"><div class="book-trailer-label">BOOKTRAILER</div><div class="book-trailer-video"><iframe src="${escAttr(src)}" title="Booktrailer ${escAttr(bookLabel(bookId))}" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen loading="lazy" referrerpolicy="strict-origin-when-cross-origin"></iframe></div></section>`
}
...
/* ===== v4.0 · USER / REGISTERED USER / AVATARES ===== */
function initUserSystem(){
  try{const cfg=window.DS_SUPABASE_CONFIG;if(window.supabase?.createClient&&cfg?.url&&cfg?.publishableKey){supabaseClient=window.supabase.createClient(cfg.url,cfg.publishableKey,{auth:{persistSession:true,autoRefreshToken:true,detectSessionInUrl:true}});supabaseClient.auth.onAuthStateChange((_event,session)=>{registeredUser=session?.user||null;updateUserMenu();if(registeredUser)setTimeout(()=>loadRegisteredProfileAndMerge(),0);else{userProfile=null;updateUserMenu()}});supabaseClient.auth.getSession().then(({data})=>{registeredUser=data?.session?.user||null;updateUserMenu();if(registeredUser)loadRegisteredProfileAndMerge()}).catch(()=>{})}}
  catch(e){console.warn('USER/Supabase',e)}
  ensureFreeAvatars();setTimeout(()=>evaluateAvatarUnlocks(false),700)
}
...
function avatarRequirementMet(a){if(!a||a.free)return true;const r=a.requirement||{};if(r.type==='read_story')return !!r.storyId&&isRead(String(r.storyId));if(r.type==='complete_saga'){const saga=sagaById(r.sagaId);if(!saga)return false;const d=sagaProgressData(saga);return d.total>0&&d.read===d.total}return false}
async function evaluateAvatarUnlocks(showReward=true){ensureFreeAvatars();const got=unlockedAvatarIds(),newOnes=[];for(const a of avatarRows()){if(got.has(a.id)||a.free)continue;if(avatarRequirementMet(a)){got.add(a.id);newOnes.push(a)}}if(!newOnes.length)return;store.set(unlockedAvatarsKey,[...got]);queueUserVaultSync();if(showReward){for(const a of newOnes)await showAvatarReward(a)}if(routeName()==='user')renderUser()}
function avatarImg(a,cls=''){return `<img class="${cls}" src="${escAttr(a?.file||'')}" alt="${escAttr(a?.id||'Avatar')}" onerror="this.classList.add('avatar-missing')">`}
function avatarRequirementText(a){if(a?.free)return 'FREE';const r=a?.requirement||{};if(r.type==='read_story')return r.storyId?`LEE LA STORY ${escapeHtml(r.storyId)}`:'REQUISITO PENDIENTE';if(r.type==='complete_saga'){const sg=sagaById(r.sagaId);return r.sagaId?`COMPLETA AL 100% ${escapeHtml(sg?.name||r.sagaId)}`:'REQUISITO PENDIENTE'}return 'BLOQUEADO'}
function updateUserMenu(){const lab=$('#userMenuLabel');if(!lab)return;lab.textContent=registeredUser?(userProfile?.display_name||'REGISTERED USER'):'UNREGISTERED USER'}
async function chooseAvatar(id){const a=avatarById(id);if(!avatarAvailable(a)){toast(avatarRequirementText(a));return}store.set(localAvatarKey,id);if(registeredUser&&supabaseClient){try{await supabaseClient.from('profiles').update({avatar_id:id,updated_at:new Date().toISOString()}).eq('id',registeredUser.id);userProfile={...(userProfile||{}),avatar_id:id}}catch(e){console.warn(e)}}renderUser();queueUserVaultSync()}
function renderUser(){currentStory=null;ensureFreeAvatars();const registered=!!registeredUser,avatar=avatarById(selectedAvatarId())||avatarRows()[0];view.innerHTML=`<section class="section user-page"><div class="user-hero"><div class="user-avatar-current">${avatar?avatarImg(avatar,'user-avatar-img'):'<span>?</span>'}</div><div><div class="eyebrow">${registered?'REGISTERED USER':'UNREGISTERED USER'}</div><h1>${escapeHtml(registered?(userProfile?.display_name||registered.email||'USER'):'USER')}</h1>${registered?`<p class="user-email">${escapeHtml(registered.email||'')}</p>`:'<p>Tu progreso permanece en este dispositivo. Regístrate para asociarlo a tu User Vault.</p>'}</div></div><div class="user-avatar-section"><h2>AVATAR</h2><div class="avatar-grid">${avatarRows().map(a=>{const ok=avatarAvailable(a),sel=a.id===selectedAvatarId();return `<button class="avatar-choice ${ok?'':'locked'} ${sel?'selected':''}" data-avatar-id="${escAttr(a.id)}" type="button">${avatarImg(a,'avatar-choice-img')}${ok?'':`<span class="avatar-lock">🔒</span>`}<strong>${escapeHtml(a.id.toUpperCase())}</strong><small>${avatarRequirementText(a)}</small></button>`}).join('')||'<div class="empty">Añade imágenes en assets/avatar/ y configúralas desde el Importer.</div>'}</div></div>${registered?`<div class="user-account-panel"><strong>USER VAULT ACTIVO</strong><p>El progreso local y los desbloqueos se sincronizan con tu cuenta.</p><button id="userSyncNow" class="cta" type="button">SINCRONIZAR AHORA</button><button id="userLogout" class="secondary-btn" type="button">CERRAR SESIÓN</button><p id="userStatus" class="note"></p></div>`:registrationPanelHtml()}</section>`;$$('[data-avatar-id]').forEach(b=>b.onclick=()=>chooseAvatar(b.dataset.avatarId));if(registered){$('#userSyncNow').onclick=()=>loadRegisteredProfileAndMerge(true);$('#userLogout').onclick=logoutUser}else bindRegistrationPanel();bindBrokenImages()}
function registrationPanelHtml(){return `<div class="user-account-panel"><h2>REGISTRAR</h2><p>Crea tu REGISTERED USER sin perder el progreso que ya tienes.</p><div class="user-form"><label>NOMBRE<input id="regName" autocomplete="name" maxlength="50"></label><label>EMAIL<input id="regEmail" type="email" autocomplete="email"></label><label>PASSWORD<input id="regPassword" type="password" autocomplete="new-password" minlength="6"></label><button id="registerUser" class="cta" type="button">REGISTRAR</button></div><details class="user-login-existing"><summary>YA TENGO CUENTA</summary><div class="user-form"><label>EMAIL<input id="loginEmail" type="email" autocomplete="email"></label><label>PASSWORD<input id="loginPassword" type="password" autocomplete="current-password"></label><button id="loginUser" class="secondary-btn" type="button">INICIAR SESIÓN</button></div></details><p id="userStatus" class="note"></p></div>`}
...
async function registerUser(){if(!supabaseClient)return userStatus('Supabase no está disponible.',true);const name=$('#regName')?.value.trim(),email=$('#regEmail')?.value.trim(),password=$('#regPassword')?.value||'';if(!name||!email||password.length<6)return userStatus('Completa nombre, email y una contraseña de al menos 6 caracteres.',true);userStatus('Creando REGISTERED USER…');const {data,error}=await supabaseClient.auth.signUp({email,password,options:{data:{display_name:name,avatar_id:selectedAvatarId()},emailRedirectTo:location.origin+location.pathname+'#/user'}});if(error)return userStatus(error.message,true);if(data?.session){registeredUser=data.user;await loadRegisteredProfileAndMerge(true);renderUser()}else userStatus('Cuenta creada. Confirma el email recibido y después inicia sesión aquí.')}
async function loginUser(){if(!supabaseClient)return userStatus('Supabase no está disponible.',true);const email=$('#loginEmail')?.value.trim(),password=$('#loginPassword')?.value||'';if(!email||!password)return userStatus('Introduce email y password.',true);userStatus('Iniciando sesión…');const {data,error}=await supabaseClient.auth.signInWithPassword({email,password});if(error)return userStatus(error.message,true);registeredUser=data.user;await loadRegisteredProfileAndMerge(true);renderUser()}
async function logoutUser(){if(!supabaseClient)return;await supabaseClient.auth.signOut();registeredUser=null;userProfile=null;updateUserMenu();renderUser()}
...
async function loadRegisteredProfileAndMerge(manual=false){if(!registeredUser||!supabaseClient||userSyncBusy)return;userSyncBusy=true;try{const [{data:profile,error:pe},{data:vaultRow,error:ve}]=await Promise.all([supabaseClient.from('profiles').select('*').eq('id',registeredUser.id).single(),supabaseClient.from('user_vaults').select('*').eq('user_id',registeredUser.id).single()]);if(pe)throw pe;if(ve)throw ve;userProfile=profile;const merged=mergeVault(vaultRow?.vault||{},localVaultSnapshot());applyVault(merged);const chosen=userProfile?.avatar_id||store.get(localAvatarKey,'avatar1');if(chosen)store.set(localAvatarKey,chosen);await supabaseClient.from('user_vaults').update({vault:localVaultSnapshot(),vault_schema:1,updated_at:new Date().toISOString()}).eq('user_id',registeredUser.id);await evaluateAvatarUnlocks(false);updateUserMenu();if(manual)userStatus('✓ User Vault sincronizado.')}catch(e){console.warn('User Vault',e);if(manual)userStatus('No se pudo sincronizar el User Vault: '+(e.message||e),true)}finally{userSyncBusy=false}}
let userSyncTimer=0;function queueUserVaultSync(){if(!registeredUser||!supabaseClient)return;clearTimeout(userSyncTimer);userSyncTimer=setTimeout(()=>syncUserVault(),800)}
async function syncUserVault(){if(!registeredUser||!supabaseClient||userSyncBusy)return;userSyncBusy=true;try{await supabaseClient.from('user_vaults').update({vault:localVaultSnapshot(),vault_schema:1,updated_at:new Date().toISOString()}).eq('user_id',registeredUser.id)}catch(e){console.warn('Vault sync',e)}finally{userSyncBusy=false}}
async function showAvatarReward(a){const seen=new Set(store.get(avatarRewardSeenKey,[]));if(seen.has(a.id))return;seen.add(a.id);store.set(avatarRewardSeenKey,[...seen]);return new Promise(resolve=>{document.querySelector('.avatar-reward')?.remove();const el=document.createElement('div');el.className='avatar-reward';el.innerHTML=`<div class="avatar-reward-card"><button class="avatar-reward-close" aria-label="Cerrar">×</button>${avatarImg(a,'avatar-reward-img')}<small>RECOMPENSA</small><strong>NUEVO AVATAR DESBLOQUEADO</strong><em>${escapeHtml(a.id.toUpperCase())}</em></div>`;document.body.append(el);requestAnimationFrame(()=>el.classList.add('show'));const close=()=>{el.classList.remove('show');setTimeout(()=>{el.remove();resolve()},260)};el.onclick=e=>{if(e.target===el||e.target.closest('.avatar-reward-close'))close()};setTimeout(close,5200)})}
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=4.0.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
