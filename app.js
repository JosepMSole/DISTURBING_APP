...
let lastRandomStoryId='',tutorialActive=null,introPlayed=false,suppressNewClearOnceId='';
...
function markStoryOpened(id){const st=indexData?.stories?.find(x=>x.id===id);if(!st)return;const latest=indexData?.stories?.[0];if(!accessibleStory(st)&&st.id!==latest?.id)return;const a=acquiredMap(),o=openedMap();if(!a[id])a[id]=Date.now();if(!o[id])o[id]=Date.now();store.set(newAcquiredKey,a);store.set(firstOpenedKey,o)}
function isNewStory(s){if(!s)return false;const latest=indexData?.stories?.[0],a=acquiredMap(),o=openedMap();if(s.id===latest?.id)return !!a[s.id]&&!o[s.id];if(!accessibleStory(s))return false;return !!a[s.id]&&!o[s.id]}
...
function renderHome(){
  ...
  view.innerHTML=`<section class="hero latest-storm">${heroMedia(latest)}<div class="storm-fx" aria-hidden="true"><span class="storm-cloud storm-cloud-a"></span><span class="storm-cloud storm-cloud-b"></span><span class="storm-cloud storm-cloud-c"></span><span class="storm-glow"></span><span class="storm-flash storm-flash-a"></span><span class="storm-flash storm-flash-b"></span><span class="lightning lightning-a"></span><span class="lightning lightning-b"></span></div><div class="hero-content"><div class="eyebrow">ÚLTIMA PUBLICACIÓN · ${formatDate(latest.published)} · #${escapeHtml(latest.id)}</div><h1>${escapeHtml(latest.title)}</h1><div class="meta-row">${latest.access==='exclusive'?'<span class="pill red">EXCLUSIVA</span>':'<span class="pill">GRATIS</span>'}${latest.readTime?`<span class="pill">${escapeHtml(latest.readTime)} min</span>`:''}${isRead(latest.id)?'<span class="pill read-pill">✓ LEÍDA</span>':''}</div><button class="cta" id="openLatest">${isLocked(latest)?'VER STORY':'ENTRAR'}</button></div></section>${continueHtml}<section class="home-progress"><div class="home-progress-copy"><div class="eyebrow">Tu progreso</div><strong>${readCount} / ${total} STORIES LEÍDAS</strong></div><div class="collection-progress"><span style="width:${pct(global)}"></span></div><button id="openMyStories" class="link-btn">VER MIS STORIES ›</button></section><section class="section home-previous"><div class="section-title"><h2>Publicadas anteriormente</h2></div><div class="story-list">${indexData.stories.slice(1,3).map(storyCard).join('')}</div><button id="openAllStories" class="cta secondary-home-cta">ACCEDE A TODAS LAS STORIES</button></section>`;
...
function renderMyStories(){
  ...
  const jumpToResults=mode=>{draw(mode);requestAnimationFrame(()=>$('#myResultsGroup')?.scrollIntoView({behavior:'smooth',block:'start'}))};
  $$('.my-filter').forEach(b=>b.onclick=()=>draw(b.dataset.my));
  $$('[data-my-jump]').forEach(el=>{const run=()=>{const mode=el.dataset.myJump;if(mode==='progress'){const c=$('#myContinueGroup');if(c)c.scrollIntoView({behavior:'smooth',block:'center'});return}jumpToResults(mode)};el.onclick=run;el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();run()}}});
...
function storyCard(s,mode='list',sortMode=''){
  const locked=isLocked(s),read=isRead(s.id),cover=displayCover(s),pr=storyProgress(s.id),ex=exclusiveBadge(s,'cover'),media=storyMediaBadges(s),sortBadge=storySortBadge(s,sortMode),readCorner=readCornerBadge(s),fresh=newStoryBadge(s);
  ...
  const thumb=`<div class="story-thumb-wrap">${image}${readCorner}${sortBadge}${fresh}${ex}${media}</div>`;
  if(mode==='covers')return `<article class="story-cover-card" data-story-id="${escAttr(s.id)}" aria-label="Story ${escAttr(s.id)} · ${escAttr(s.title||'')}">${image}${readCorner}${sortBadge}${fresh}${read?'<span class="cover-read">✓ LEÍDA</span>':pr>.015?`<span class="cover-progress">${pct(pr)}</span>`:''}${ex}${media}</article>`;
  if(mode==='grid')return `<article class="story-grid-card" data-story-id="${escAttr(s.id)}"><div class="story-grid-poster">${image}${readCorner}${sortBadge}${fresh}${ex}${media}</div><div class="story-grid-copy"><div class="story-number">STORY ${escapeHtml(s.id)}</div><h3>${escapeHtml(s.title)}</h3><p>${s.access==='exclusive'?(locked?'EXCLUSIVA · BLOQUEADA':'EXCLUSIVA · DESBLOQUEADA'):'GRATIS'}${read?' · LEÍDA':''}</p></div></article>`;
...
function renderCollection(bookId=''){
  ...
  if(bookId&&books.has(bookId)){
    ...
    $$('.book-story-filter').forEach(b=>b.onclick=()=>draw(b.dataset.bookFeature));$$('[data-book-feature-jump]').forEach(b=>b.onclick=()=>{const feature=b.dataset.bookFeatureJump;draw(feature);requestAnimationFrame(()=>$('#bookStoriesGrid')?.scrollIntoView({behavior:'smooth',block:'start'}))});$('#collectionBack').onclick=()=>go('collection');draw();return;
  }
...
async function startQrCamera(prefill=''){if(qrScanBusy)return;if(!await ensureQrLibrary()){unlockStatus('No se ha podido cargar el lector QR. Comprueba la conexión e inténtalo de nuevo.');return}await stopQrScanner();qrScanBusy=true;unlockStatus('Solicitando acceso a la cámara…');const onDecoded=async decoded=>{if(!qrScanBusy)return;qrScanBusy=false;await stopQrScanner();await tryUnlock(decoded,prefill)};const config={fps:10,qrbox:{width:220,height:220},aspectRatio:1.0,disableFlip:false};try{qrScanner=new Html5Qrcode('qr-reader');try{await qrScanner.start({facingMode:'environment'},config,onDecoded,()=>{})}catch(first){const cams=await Html5Qrcode.getCameras();if(!cams?.length)throw first;const preferred=cams.find(c=>/back|rear|environment|trasera/i.test(c.label||''))||cams[cams.length-1];await qrScanner.start(preferred.id,config,onDecoded,()=>{})}unlockStatus('Apunta la cámara al QR del relato.')}catch(e){try{await stopQrScanner()}catch{}unlockStatus('No se pudo abrir el escáner en directo. Revisa el permiso de cámara o usa «Cargar imagen desde tu álbum».');qrScanBusy=false}}
...
function gamepadPortalSvg(){return `<svg class="portal-custom-svg gamepad-portal-svg" viewBox="0 0 40 24" aria-hidden="true"><path d="M11 7h18c4 0 7 3 7 7l-1 4c-.4 2-2.6 2.7-4 1.3L27 16H13l-4 3.3C7.6 20.7 5.4 20 5 18l-1-4c0-4 3-7 7-7Z"/><path d="M11 11v6M8 14h6"/><circle cx="29" cy="12" r="1.2"/><circle cx="32" cy="15" r="1.2"/></svg>`}
function storiesExplorerHtml(){return `<section class="stories-portals"><div class="stories-portals-group"><div class="portal-label">EXPLORAR</div><div class="portal-grid"><button class="portal-btn" data-portal="sagas"><span>◈</span><strong>SAGAS</strong></button><button class="portal-btn" data-portal="timeline">${timelinePortalSvg()}<strong>TIMELINE</strong></button><button class="portal-btn" data-portal="random"><span>◎</span><strong>RANDOM</strong></button><button class="portal-btn" data-portal="extras"><span>＋</span><strong>EXTRAS</strong></button></div></div><div class="stories-portals-group multimedia"><div class="portal-label">MULTIMEDIA</div><div class="portal-grid portal-grid-4"><button class="portal-btn cassette" data-portal="cassettes">${mediaIconSvg('audio')}<strong>DISTURBING CASSETTES</strong></button><button class="portal-btn tape" data-portal="tapes">${mediaIconSvg('video')}<strong>DISTURBING TAPES</strong></button><button class="portal-btn games" data-portal="games">${gamepadPortalSvg()}<strong>GAMES</strong><small>PRÓXIMAMENTE</small></button><button class="portal-btn nightmares" data-portal="micro-pesadillas"><span class="nightmare-diamond"></span><strong>MICRO-PESADILLAS</strong><small>PRÓXIMAMENTE</small></button></div></div></section>`}
...
function sagaCard(saga){const total=Math.max(Number(saga.totalParts)||0,(saga.parts||[]).length),published=(saga.parts||[]).filter(p=>p.storyId&&indexData.stories.some(s=>s.id===String(p.storyId))).length,percent=total?Math.round(published/total*100):0;return `<button class="saga-card" data-saga-id="${escAttr(saga.id)}">${saga.banner?`<img src="${escAttr(saga.banner)}" alt="" loading="lazy" referrerpolicy="no-referrer">`:'<div class="saga-banner-placeholder">SAGA</div>'}<div class="saga-card-shade"></div><div class="saga-card-copy"><span>${published}/${total} PARTES PUBLICADAS</span><h2>${escapeHtml(saga.name||'Saga')}</h2><div class="saga-progress"><i style="width:${percent}%"></i></div><strong>${percent}% PUBLICADA</strong></div></button>`}
function renderSaga(id){currentStory=null;const saga=sagaById(id);if(!saga){view.innerHTML='<div class="empty">Saga no encontrada.</div>';return}const total=Math.max(Number(saga.totalParts)||0,(saga.parts||[]).length),parts=Array.from({length:total},(_,i)=>(saga.parts||[])[i]||{order:i+1}),published=parts.filter(p=>p.storyId&&indexData.stories.some(s=>s.id===String(p.storyId))).length,percent=total?Math.round(published/total*100):0;view.innerHTML=`<section class="section v2-section saga-detail">${saga.banner?`<div class="saga-detail-banner"><img src="${escAttr(saga.banner)}" alt="${escAttr(saga.name||'')}"></div>`:''}<div class="eyebrow">SAGA · ${published}/${total} PUBLICADAS</div><h1>${escapeHtml(saga.name||'Saga')}</h1><p class="saga-synopsis">${escapeHtml(saga.synopsis||'')}</p><div class="saga-percent"><strong>${percent}%</strong><span>PUBLICADA</span><div><i style="width:${percent}%"></i></div></div>${saga.extraText?`<p class="saga-extra-text">${escapeHtml(saga.extraText)}</p>`:''}${saga.extraImage?`<img class="saga-extra-image" src="${escAttr(saga.extraImage)}" alt="" loading="lazy">`:''}<div class="saga-parts">${parts.map((part,i)=>sagaPartRow(part,i+1,total)).join('')}</div></section>`;bindStoryCards();bindBrokenImages();maybeAutoTutorial('sagas')}
function sagaPartRow(part,n,total){const st=part.storyId?indexData.stories.find(s=>s.id===String(part.storyId)):null;if(!st)return `<div class="saga-part future-part"><span>PARTE ${n}</span><strong>PRÓXIMAMENTE</strong><em>${escapeHtml(part.label||'Esperando publicación')}</em></div>`;const cover=displayCover(st);return `<article class="saga-part published-part" data-story-id="${escAttr(st.id)}"><span>PARTE ${n} DE ${total}</span>${cover?`<div class="saga-part-poster"><img src="${escAttr(cover)}" alt="">${exclusiveBadge(st,'cover')}</div>`:''}<div><strong>STORY ${escapeHtml(st.id)} · ${escapeHtml(st.title||'')}</strong><em>${bookLabel(st.book||'—')}</em></div></article>`}
...
function renderTimeline(desc=false){currentStory=null;const groups=timelineOrderedStories(desc);let prevYear=null,prevEra='';const rows=[];for(const g of groups){const era=eraForYear(g.year);if(era!==prevEra){rows.push(`<div class="timeline-era"><span>${escapeHtml(era)}</span></div>`);prevEra=era}if(prevYear!==null&&g.year!==null&&Math.abs(g.year-prevYear)>150)rows.push('<div class="timeline-break"><i></i><span>SALTO TEMPORAL</span><i></i></div>');rows.push(`<div class="timeline-year-group"><div class="timeline-year">${escapeHtml(timelineYearLabel(g.year))}</div><div class="timeline-items">${g.stories.map(timelineItem).join('')}</div></div>`);prevYear=g.year}view.innerHTML=`<section class="section v2-section timeline-section"><div class="section-title"><div><div class="eyebrow">CRONOLOGÍA DE FICCIÓN</div><h1>Timeline</h1></div></div><button class="timeline-order" id="timelineOrder">${desc?'↑ MÁS ANTIGUAS PRIMERO':'↓ MÁS RECIENTES PRIMERO'}</button><div class="timeline-list">${rows.join('')||'<div class="empty">No hay Stories disponibles para el Timeline.</div>'}</div></section>`;$('#timelineOrder').onclick=()=>renderTimeline(!desc);bindStoryCards();bindBrokenImages()}
function timelineItem(s){const cover=displayCover(s),icons=[];if(storyHasFeature(s,'audio'))icons.push(`<span class="timeline-icon audio">${mediaIconSvg('audio')}</span>`);if(storyHasFeature(s,'video'))icons.push(`<span class="timeline-icon video">${mediaIconSvg('video')}</span>`);return `<article class="timeline-item"><span class="timeline-node"></span><button class="timeline-poster" data-story-id="${escAttr(s.id)}">${cover?`<img src="${escAttr(cover)}" alt="Story ${escAttr(s.id)}">`:`<span>${escapeHtml(s.id)}</span>`}${exclusiveBadge(s,'cover')}</button><div class="timeline-icons">${icons.join('')}</div></article>`}

function renderRandom(filter='unread'){currentStory=null;const available=(indexData.stories||[]).filter(accessibleStory);const pool=filter==='read'?available.filter(s=>isRead(s.id)):filter==='all'?available:available.filter(s=>!isRead(s.id));view.innerHTML=`<section class="section v2-section random-section"><div class="eyebrow">DEJA QUE DISTURBING STORIES DECIDA POR TI</div><h1>Random</h1><div class="random-filters"><button data-random-filter="all" class="${filter==='all'?'active':''}">TODAS</button><button data-random-filter="unread" class="${filter==='unread'?'active':''}">NO LEÍDAS</button><button data-random-filter="read" class="${filter==='read'?'active':''}">LEÍDAS</button></div><div class="random-wheel-wrap"><div id="randomWheel" class="random-wheel"><i></i><b>RANDOM</b></div><span class="random-pointer">▼</span></div><button id="spinRandom" class="cta" ${pool.length?'':'disabled'}>GIRAR RULETA</button><div id="randomResult"></div></section>`;$$('[data-random-filter]').forEach(b=>b.onclick=()=>renderRandom(b.dataset.randomFilter));$('#spinRandom').onclick=()=>spinRandom(pool)}
function spinRandom(pool){if(!pool.length)return;let candidates=pool.filter(s=>s.id!==lastRandomStoryId);if(!candidates.length)candidates=pool;const picked=candidates[Math.floor(Math.random()*candidates.length)];lastRandomStoryId=picked.id;const wheel=$('#randomWheel');wheel.classList.remove('spin');void wheel.offsetWidth;wheel.classList.add('spin');setTimeout(()=>showRandomResult(picked),1050)}
function showRandomResult(s){const cover=displayCover(s);$('#randomResult').innerHTML=`<article class="random-result">${cover?`<div class="random-result-poster"><img src="${escAttr(cover)}" alt="">${newStoryBadge(s)}${exclusiveBadge(s,'cover')}${storyMediaBadges(s)}</div>`:''}<div class="eyebrow">RESULTADO</div><h2>STORY ${escapeHtml(s.id)} · ${escapeHtml(s.title||'')}</h2><p>${escapeHtml(timelineYearLabel(parseStoryYear(s)))}</p><div class="random-actions"><button class="cta" id="randomRead">LEER STORY</button><button class="secondary-btn" id="randomAgain">VOLVER A GIRAR</button></div></article>`;$('#randomRead').onclick=()=>storyGo(s.id);$('#randomAgain').onclick=()=>$('#spinRandom').click();bindBrokenImages()}
...
function renderFuturePlaceholder(title,tone){currentStory=null;view.innerHTML=`<section class="section v2-section future-placeholder ${escAttr(tone)}"><div class="eyebrow">PREPARADO PARA FUTURA ITERACIÓN</div><h1>${escapeHtml(title)}</h1><div class="future-placeholder-box"><strong>PRÓXIMAMENTE</strong><p>La entrada visual ya forma parte de Disturbing Stories App v2.1. Su contenido se desarrollará en una iteración posterior.</p></div></section>`}

async function playEntryIntro(){if(introPlayed)return;introPlayed=true;const layer=$('#introLayer'),video=$('#introVideo');if(!layer||!video)return;layer.classList.remove('hidden');layer.setAttribute('aria-hidden','false');let finished=false;return new Promise(resolve=>{const done=()=>{if(finished)return;finished=true;clearTimeout(fallback);layer.classList.add('intro-out');setTimeout(()=>{layer.classList.add('hidden');layer.classList.remove('intro-out');layer.setAttribute('aria-hidden','true');resolve()},220)};const start=()=>{clearTimeout(fallback);const p=video.play();if(p&&typeof p.catch==='function')p.catch(done)};const fallback=setTimeout(done,1800);layer.onclick=done;video.addEventListener('ended',done,{once:true});video.addEventListener('error',done,{once:true});if(video.readyState>=3)start();else video.addEventListener('canplay',start,{once:true})})}

async function registerSW(){if('serviceWorker'in navigator){try{let reloading=false;navigator.serviceWorker.addEventListener('controllerchange',()=>{if(reloading)return;reloading=true;location.reload()});const reg=await navigator.serviceWorker.register('./sw.js?v=2.1.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
