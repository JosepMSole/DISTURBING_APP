...
function restoreReadPosition(id){
  ...
  const state=readingStateMap()[id]||{progress:storyProgress(id),position:storyProgress(id)};
  const stored=Math.max(0,Math.min(1,Number(state.position??state.progress)||0));
  const images=[...r.querySelectorAll('img')];images.forEach(img=>{try{img.loading='eager';img.setAttribute('fetchpriority','high')}catch{}});
  let cancelled=false,raf=0,resizeObserver=null,animationDone=false;
  const targetY=()=>{const rect=r.getBoundingClientRect(),top=scrollY+rect.top,h=Math.max(1,r.offsetHeight-innerHeight*.65);return Math.max(0,top+stored*h-innerHeight*.3)};
  const cancel=()=>{cancelled=true;if(raf)cancelAnimationFrame(raf);try{resizeObserver?.disconnect()}catch{}};
  const userOpts={passive:true,once:true};window.addEventListener('wheel',cancel,userOpts);window.addEventListener('touchstart',cancel,userOpts);window.addEventListener('pointerdown',cancel,userOpts);
  const begin=()=>{
    if(cancelled)return;const started=performance.now(),from=scrollY,duration=720;
    const step=now=>{if(cancelled)return;const t=Math.min(1,(now-started)/duration),ease=1-Math.pow(1-t,3),dest=targetY();scrollTo({top:from+(dest-from)*ease,behavior:'auto'});if(t<1)raf=requestAnimationFrame(step);else{animationDone=true;scrollTo({top:targetY(),behavior:'auto'});requestAnimationFrame(updateProgress)}};
    raf=requestAnimationFrame(step);
    if('ResizeObserver'in window){resizeObserver=new ResizeObserver(()=>{if(!cancelled&&animationDone&&performance.now()-started<2300)requestAnimationFrame(()=>scrollTo({top:targetY(),behavior:'auto'}))});resizeObserver.observe(r);setTimeout(()=>{try{resizeObserver?.disconnect()}catch{}},2400)}
    images.filter(i=>!i.complete).forEach(i=>{const settle=()=>{if(!cancelled&&animationDone)requestAnimationFrame(()=>scrollTo({top:targetY(),behavior:'auto'}))};i.addEventListener('load',settle,{once:true});i.addEventListener('error',settle,{once:true})});
    setTimeout(()=>{if(!cancelled){scrollTo({top:targetY(),behavior:'auto'});requestAnimationFrame(updateProgress)}},1800);
  };
...
async function registerSW(){if('serviceWorker'in navigator){try{const reg=await navigator.serviceWorker.register('./sw.js?v=6.12.0',{updateViaCache:'none'});try{await reg.update()}catch{} }catch(e){console.warn('SW',e)}}}
boot();
